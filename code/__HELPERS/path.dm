 ...
 */
#define CAN_STEP(cur_turf, next) (next && !next.density && cur_turf.Adjacent(next) && !(simulated_only && (SSpathfinder.space_type_cache[next.type] || SSpathfinder.openspace_type_cache[next.type])) && !cur_turf.LinkBlockedWithAccess(next,caller, id) && (next != avoid))
