...
/datum/wires/particle_accelerator/control_box/on_pulse(wire)
	...
	switch(wire)
		...
		if(WIRE_LIMIT)
			C.visible_message("<span class='notice'>[icon2html(C, viewers(holder))]<b>[C]</b> издает сильный шум.</span>")
...
/datum/wires/particle_accelerator/control_box/can_reveal_wires(mob/user)
