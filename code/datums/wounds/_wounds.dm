...
/datum/wound/proc/try_treating(obj/item/I, mob/user)
	...
	if(QDELETED(I) || limb.body_zone != user.zone_selected)
		return FALSE
...
/datum/wound/proc/receive_damage(wounding_type, wounding_dmg, wound_bonus, attack_direction = attack_direction)
	return
