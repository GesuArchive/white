...
/datum/wound/blunt/wound_injury(datum/wound/old_wound = null, attack_direction = null)
	...
	if(limb.held_index && victim.get_item_for_held_index(limb.held_index) && (disabling || prob(30 * severity)))
		...
		if(I && victim.dropItemToGround(I))
			victim.visible_message("<span class='smalldanger'><b>[victim]</b> бросает <b>[I]</b> в приступе боли!</span>", "<span class='warning'>Боль в моей <b>[ru_gde_zone(limb.name)]</b> заставляет меня бросить <b>[I]</b>!</span>", vision_distance=COMBAT_MESSAGE_RANGE)
...
/datum/wound/blunt/handle_process(delta_time, times_fired)
	...
	if(DT_PROB(severity * 1.5, delta_time))
		...
		if(prob(33))
			to_chat(victim, "<span class='smalldanger'>Ощущаю острую боль от перелома!</span>")
...
/datum/wound/blunt/proc/attack_with_hurt_hand(mob/M, atom/target, proximity)
	...
	if(prob((severity - 1) * 15))
		...
		if(prob(70 - 20 * (severity - 1)))
			to_chat(victim, "<span class='userdanger'>Перелом в моей [ru_gde_zone(limb.name)] стреляет болью при ударе <b>[target]</b>!</span>")
			limb.receive_damage(brute=rand(1,5))
		else
			victim.visible_message("<span class='smalldanger'><b>[victim]</b> слабо бьёт <b>[target]</b> своей сломаной рукой, отскакивая в приступе боли!</span>", \
			"<span class='userdanger'>Перелом в моей [ru_gde_zone(limb.name)] загорается от невыносимой боли, когда я пытаюсь ударить <b>[target]</b>!</span>", vision_distance=COMBAT_MESSAGE_RANGE)
...
/datum/wound/blunt/receive_damage(wounding_type, wounding_dmg, wound_bonus)
	...
	if(limb.body_zone == BODY_ZONE_CHEST && victim.blood_volume && prob(internal_bleeding_chance + wounding_dmg))
		...
		switch(blood_bled)
			...
			if(7 to 13)
				victim.visible_message("<span class='smalldanger'><b>[victim]</b> кашляет кровью от удара в грудь.</span>", "<span class='danger'>Выплёвываю немного крови от удара в грудь.</span>", vision_distance=COMBAT_MESSAGE_RANGE)
				victim.bleed(blood_bled, TRUE)
			if(14 to 19)
				victim.visible_message("<span class='smalldanger'><b>[victim]</b> выплевывает струю крови от удара в грудь!</span>", "<span class='danger'>Выплёвываю струю крови от удара в грудь!</span>", vision_distance=COMBAT_MESSAGE_RANGE)
				new /obj/effect/temp_visual/dir_setting/bloodsplatter(victim.loc, victim.dir)
				victim.bleed(blood_bled)
			if(20 to INFINITY)
				victim.visible_message("<span class='smalldanger'><b>[victim]</b> заблёвывает всё кровью от удара в грудь!</span>", "<span class='danger'><b>Заблёвываю всё кровью от удара в грудь!</b></span>", vision_distance=COMBAT_MESSAGE_RANGE)
				victim.bleed(blood_bled)
				new /obj/effect/temp_visual/dir_setting/bloodsplatter(victim.loc, victim.dir)
...
/datum/wound/blunt/moderate/proc/door_crush()
	if(prob(33))
		victim.visible_message("<span class='smalldanger'><b>[victim]</b> выворачивает [ru_parse_zone(limb.name)] и ставит на место!</span>", "<span class='userdanger'>Выправляю [ru_parse_zone(limb.name)] на место! Ух!</span>")
		remove_wound()
...
/datum/wound/blunt/moderate/try_handling(mob/living/carbon/human/user)
	...
	if(user.grab_state >= GRAB_AGGRESSIVE)
		user.visible_message("<span class='smalldanger'><b>[user]</b> начинает скручивать и напрягать [ru_parse_zone(limb.name)] <b>[victim]</b>!</span>", "<span class='notice'>Начинаю скручивать и напрягать [ru_parse_zone(limb.name)] <b>[victim]</b>...</span>", ignored_mobs=victim)
...
/datum/wound/blunt/moderate/proc/chiropractice(mob/living/carbon/human/user)
	...
	if(prob(65))
		user.visible_message("<span class='smalldanger'><b>[user]</b> вправляет [ru_parse_zone(limb.name)] <b>[victim]</b>!</span>", "<span class='notice'>Вправляю [ru_parse_zone(limb.name)] <b>[victim]</b> на место!</span>", ignored_mobs=victim)
		...
		user.visible_message("<span class='smalldanger'><b>[user]</b> крутит [ru_parse_zone(limb.name)] <b>[victim]</b> довольно неправильно!</span>", "<span class='danger'>Кручу [ru_parse_zone(limb.name)] <b>[victim]</b> неправильно!</span>", ignored_mobs=victim)
...
/datum/wound/blunt/moderate/proc/malpractice(mob/living/carbon/human/user)
	...
	if(prob(65))
		user.visible_message("<span class='smalldanger'><b>[user]</b> вправляет [ru_parse_zone(limb.name)] <b>[victim]</b> с отвратительным хрустом!</span>", "<span class='danger'>Вправляю [ru_parse_zone(limb.name)] <b>[victim]</b> с отвратительным хрустом!</span>", ignored_mobs=victim)
		...
		user.visible_message("<span class='smalldanger'><b>[user]</b> крутит [ru_parse_zone(limb.name)] <b>[victim]</b> неправильно!</span>", "<span class='danger'>Кручу [ru_parse_zone(limb.name)] <b>[victim]</b> неправильно!</span>", ignored_mobs=victim)
		to_chat(victim, "<span class='userdanger'><b>[user]</b> крутит мою [ru_parse_zone(limb.name)] неправильно!</span>")
		limb.receive_damage(brute=10, wound_bonus=CANT_WOUND)
		malpractice(user)
...
/datum/wound/blunt/moderate/treat(obj/item/I, mob/user)
	if(victim == user)
		victim.visible_message("<span class='smalldanger'><b>[user]</b> начинает вправлять [victim.ru_ego()] [ru_parse_zone(limb.name)] используя [I].</span>", "<span class='warning'>Начинаю вправлять свою [ru_parse_zone(limb.name)] используя [I]...</span>")
	else
		user.visible_message("<span class='smalldanger'><b>[user]</b> начинает вправлять [ru_parse_zone(limb.name)] <b>[victim]</b> используя [I].</span>", "<span class='notice'>Начинаю вправлять [ru_parse_zone(limb.name)] <b>[victim]</b> используя [I]...</span>")
	...
	if(victim == user)
		...
		victim.visible_message("<span class='smalldanger'><b>[user]</b> успешно вправляет [victim.ru_ego()] [ru_parse_zone(limb.name)]!</span>", "<span class='userdanger'>Вправляю свою [ru_parse_zone(limb.name)]!</span>")
	else
		limb.receive_damage(brute=10, wound_bonus=CANT_WOUND)
		user.visible_message("<span class='smalldanger'><b>[user]</b> успешно вправляет [ru_parse_zone(limb.name)] <b>[victim]</b>!</span>", "<span class='nicegreen'>Вправляю [ru_parse_zone(limb.name)] <b>[victim]</b>!</span>", victim)
...
/datum/wound/blunt/proc/gel(obj/item/stack/medical/bone_gel/I, mob/user)
	...
	user.visible_message("<span class='smalldanger'><b>[user]</b> начинает примеенять [I] на [ru_parse_zone(limb.name)] <b>[victim]</b>...</span>", "<span class='warning'>Начинаю применять [I] на [ru_parse_zone(limb.name)] [user == victim ? " " : "<b>[victim]</b> "], игнорируя предупреждение на этикетке...</span>")
	...
	if(user != victim)
		...
		if(prob(25 + (20 * (severity - 2)) - painkiller_bonus)) // 25%/45% chance to fail self-applying with severe and critical wounds, modded by painkillers
			victim.visible_message("<span class='smalldanger'><b>[victim]</b> проваливает попытку нанести [I] на [victim.ru_ego()] [ru_parse_zone(limb.name)], теряя сознание от боли!</span>", "<span class='notice'>Теряю сознание от боли пытаясь применить [I] на мою [ru_parse_zone(limb.name)] перед тем как закончить!</span>")
...
/datum/wound/blunt/proc/tape(obj/item/stack/sticky_tape/surgical/I, mob/user)
	...
	user.visible_message("<span class='smalldanger'><b>[user]</b> начинает применять [I] на [ru_parse_zone(limb.name)] <b>[victim]</b>...</span>", "<span class='warning'>Начинаю применять [I] на [ru_parse_zone(limb.name)] [user == victim ? " " : "<b>[victim]</b> "]...</span>")
