...
/datum/wound/pierce/receive_damage(wounding_type, wounding_dmg, wound_bonus)
	...
	if(victim.blood_volume && prob(internal_bleeding_chance + wounding_dmg))
		...
		switch(blood_bled)
			...
			if(20 to INFINITY)
				victim.visible_message("<span class='smalldanger'>Неконтроллируемая струя крови начинает хлестать из [ru_otkuda_zone(limb.name)] [victim]!</span>", "<span class='danger'><b>Из моей [ru_otkuda_zone(limb.name)] начинает выходить кровь ужасным темпом!</b></span>" , vision_distance=COMBAT_MESSAGE_RANGE)
				victim.bleed(blood_bled)
				new /obj/effect/temp_visual/dir_setting/bloodsplatter(victim.loc, victim.dir)
...
/datum/wound/pierce/proc/tool_cauterize(obj/item/I, mob/user)
	...
	user.visible_message("<span class='smalldanger'><b>[user]</b> начинает прижигать [ru_parse_zone(limb.name)] <b>[victim]</b> используя [I.name]...</span>", "<span class='danger'>Начинаю прижигать [ru_parse_zone(limb.name)] [user == victim ? "" : "<b>[victim]</b> "]используя [I.name]...</span>")
