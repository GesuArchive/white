...
/datum/wound/slash/handle_process(delta_time, times_fired)
	...
	if(blood_flow < minimum_flow)
		if(demotes_to)
			...
			to_chat(victim, "<span class='green'>Порез на моей [ru_gde_zone(limb.name)] перестаёт кровоточить!</span>")
			qdel(src)
...
/datum/wound/slash/proc/lick_wounds(mob/living/carbon/human/user)
	...
	user.visible_message("<span class='notice'><b>[user]</b> начинает зализывать рану на [ru_gde_zone(limb.name)] <b>[victim]</b>.</span>" , "<span class='notice'>Начинаю зализывать рану на [ru_gde_zone(limb.name)] <b>[victim]</b>...</span>" , ignored_mobs=victim)
...
/datum/wound/slash/proc/las_cauterize(obj/item/gun/energy/laser/lasgun, mob/user)
	...
	user.visible_message("<span class='smalldanger'><b>[user]</b> начинает наводить [lasgun] прямо на [ru_gde_zone(limb.name)] <b>[victim]</b>...</span>" , "<span class='userdanger'>Начинаю наводить [lasgun] прямо на [user == victim ? "свою " : " "][ru_parse_zone(limb.name)][user == victim ? "" : " <b>[victim]</b>"]...</span>")
	...
	victim.visible_message("<span class='smalldanger'>Порезы на [ru_gde_zone(limb.name)] <b>[victim]</b> превращаются в ужасные шрамы!</span>")
...
/datum/wound/slash/proc/tool_cauterize(obj/item/I, mob/user)
	...
	user.visible_message("<span class='smalldanger'><b>[user]</b> начинает прижигать порезы на [ru_gde_zone(limb.name)] <b>[victim]</b> используя [I]...</span>" , "<span class='danger'>Начинаю прижигать порезы на [user == victim ? "своей" : " "][ru_gde_zone(limb.name)][user == victim ? "" : " <b>[victim]</b>"] используя [I]...</span>")
