...
/datum/mutation/human/self_amputation
	...
	quality = POSITIVE
	text_gain_indication = "<span class='notice'>У меня такое ощущение что моя рука вот-вот отвалится...</span>"
	instability = 30
...
/datum/action/cooldown/spell/self_amputation/proc/grown(user)
	var/mob/living/carbon/C = user
	to_chat(C, "<span class='notice'>Конечность вырастает прямо на глазах, но это очень больно!</span>")
...
/datum/action/cooldown/spell/self_amputation/cast(mob/living/carbon/cast_on)
	...
	switch(choice)
		if("Подсказка")	//	На самом деле тут не нужна подсказка, просто я не знаю как повернуть это меню на 45 градусов
			to_chat(C, "<span class='notice'>Благодаря геному ящера и позволяет сбросить конечность в случае опасности, а также вырастить новую. Так же создает мембранные клапаны в системе кровообращения, предотвращающие кровотечение при потере конечностей. Для выращивания новой конечности требуется много сил, питательного материала и крови, а так же это весьма болезненно.</span>")
	switch(choice)
		if("Правая рука")
