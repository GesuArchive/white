...
/datum/component/plumbing/Initialize(start=TRUE, ducting_layer, turn_connects=TRUE, datum/reagents/custom_receiver, extend_pipe_to_edge = FALSE)
	...
	if(start)
		...
		RegisterSignals(parent, list(COMSIG_COMPONENT_ADDED), PROC_REF(enable))
...
/datum/component/plumbing/RegisterWithParent()
	...
	RegisterSignals(parent, list(COMSIG_OBJ_HIDE), PROC_REF(hide))
	RegisterSignals(parent, list(COMSIG_ATOM_UPDATE_OVERLAYS), PROC_REF(create_overlays)) //called by lateinit on startup
	RegisterSignals(parent, list(COMSIG_ATOM_DIR_CHANGE), PROC_REF(on_parent_dir_change)) //called when placed on a shuttle and it moves, and other edge cases
	RegisterSignals(parent, list(COMSIG_MOVABLE_CHANGE_DUCT_LAYER), PROC_REF(change_ducting_layer))
