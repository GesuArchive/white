...
*/
		...
		if(fullness <= (600 * (1 + eater.overeatduration / (2000 SECONDS))))
			eater.visible_message("<span class='danger'>[feeder] пытается дать [eater] попробовать [parent].</span>" , \
...
/datum/component/edible/proc/checkLiked(fraction, mob/M)
	...
	switch(food_taste_reaction)
		if(FOOD_TOXIC)
			to_chat(H,"<span class='warning'>ГОСПОДИ, ЭТО ОТВРАТИТЕЛЬНО!</span>")
			H.adjust_disgust(25 + 30 * fraction)
		...
		if(FOOD_LIKED)
			to_chat(H,"<span class='notice'>Вкусно!</span>")
...
/datum/component/edible/proc/On_Consume(mob/living/eater, mob/living/feeder)
	...
	to_chat(feeder, "<span class='warning'>Совсем не осталось [parent], о нет!</span>")
	if(isturf(parent))
		var/turf/T = parent
