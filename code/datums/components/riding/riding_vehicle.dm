...
/datum/component/riding/vehicle/riding_can_z_move(atom/movable/movable_parent, direction, turf/start, turf/destination, z_move_flags, mob/living/rider)
	...
	if(!keycheck(rider))
		if(z_move_flags & ZMOVE_FEEDBACK)
			to_chat(rider, "<span class='warning'>[movable_parent] не имеет ключа внутри!</span>")
		return COMPONENT_RIDDEN_STOP_Z_MOVE
	if(HAS_TRAIT(rider, TRAIT_INCAPACITATED))
		if(z_move_flags & ZMOVE_FEEDBACK)
			to_chat(rider, "<span class='warning'>Я не могу управлять [movable_parent] в данный момент!</span>")
		return COMPONENT_RIDDEN_STOP_Z_MOVE
	if(ride_check_flags & RIDER_NEEDS_LEGS && HAS_TRAIT(rider, TRAIT_FLOORED))
		if(z_move_flags & ZMOVE_FEEDBACK)
			to_chat(rider, "<span class='warning'>Кажется, я не могу справиться с этим, будучи не в состоянии встать достаточно, чтобы двигаться [movable_parent]...</span>")
		return COMPONENT_RIDDEN_STOP_Z_MOVE
	if(ride_check_flags & RIDER_NEEDS_ARMS && HAS_TRAIT(rider, TRAIT_HANDS_BLOCKED))
		if(z_move_flags & ZMOVE_FEEDBACK)
			to_chat(rider, "<span class='warning'>Кажется, я не могу удержаться на [movable_parent] чтобы управлять этим...</span>")
		return COMPONENT_RIDDEN_STOP_Z_MOVE
...
/datum/component/riding/vehicle/driver_move(atom/movable/movable_parent, mob/living/user, direction)
	...
	if(!keycheck(user))
		if(COOLDOWN_FINISHED(src, message_cooldown))
			to_chat(user, "<span class='warning'>[vehicle_parent] не имеет ключа внутри!</span>")
	...
	if(HAS_TRAIT(user, TRAIT_INCAPACITATED))
		if(ride_check_flags & UNBUCKLE_DISABLED_RIDER)
			...
			user.visible_message("<span class='danger'>[user] падает с [vehicle_parent].</span>",\
			"<span class='danger'>Я соскальзываю с [vehicle_parent] когда ваше тело падает!</span>")
			user.Stun(3 SECONDS)

		if(COOLDOWN_FINISHED(src, message_cooldown))
			to_chat(user, "<span class='warning'>Я не могу управлять [vehicle_parent] В данный момент!</span>")
			COOLDOWN_START(src, message_cooldown, 5 SECONDS)
		return COMPONENT_DRIVER_BLOCK_MOVE
	...
	if(ride_check_flags & RIDER_NEEDS_LEGS && HAS_TRAIT(user, TRAIT_FLOORED))
		if(ride_check_flags & UNBUCKLE_DISABLED_RIDER)
			vehicle_parent.unbuckle_mob(user, TRUE)
			user.visible_message("<span class='danger'>[user] падает с  [vehicle_parent].</span>",\
			"<span class='danger'>Я падаю с  [vehicle_parent] при попытке управлять им, будучи не в состоянии стоять!</span>")
			user.Stun(3 SECONDS)

		if(COOLDOWN_FINISHED(src, message_cooldown))
			to_chat(user, "<span class='warning'>Кажется, я не могу справиться с этим, будучи не в состоянии встать достаточно, чтобы двигаться  [vehicle_parent]...</span>")
			COOLDOWN_START(src, message_cooldown, 5 SECONDS)
		return COMPONENT_DRIVER_BLOCK_MOVE
	...
	if(ride_check_flags & RIDER_NEEDS_ARMS && HAS_TRAIT(user, TRAIT_HANDS_BLOCKED))
		if(ride_check_flags & UNBUCKLE_DISABLED_RIDER)
			vehicle_parent.unbuckle_mob(user, TRUE)
			user.visible_message("<span class='danger'>[user] падает с  [vehicle_parent].</span>",\
			"<span class='danger'>Я падаю с  [vehicle_parent] при попытке управлять им, не будучи в состоянии удержаться!</span>")
			user.Stun(3 SECONDS)

		if(COOLDOWN_FINISHED(src, message_cooldown))
			to_chat(user, "<span class='warning'>Кажется, я не могу удержаться  [vehicle_parent] чтобы управлять этим...</span>")
			COOLDOWN_START(src, message_cooldown, 5 SECONDS)
		return COMPONENT_DRIVER_BLOCK_MOVE
...
/datum/component/riding/vehicle/secway/driver_move(mob/living/user, direction)
	...
	if(keycheck(user) && the_secway.eddie_murphy)
		if(COOLDOWN_FINISHED(src, message_cooldown))
			the_secway.visible_message("<span class='warning'>[src] брызжет слюной и отказывается двигаться!</span>")
