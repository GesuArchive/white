	...
	switch(excess)
		...
		if((NANITE_EXCESS_VOMIT + 0.1) to NANITE_EXCESS_BURST) //Nanites getting rejected in massive amounts, but still enough to make a semi-orderly exit through vomit
			if(iscarbon(host_mob))
				...
				host_mob.visible_message(span_warning("A metallic grey slurry bursts out of [host_mob] skin!") , span_userdanger("A metallic grey slurry violently bursts out of your skin!") , null);
