...
/datum/component/cell/proc/examine_cell(atom/A, mob/user, list/examine_list)
	...
	if(!inserted_cell)
		...
		examine_list += "<span class='notice'>It has [inserted_cell] inserted. It has <b>[inserted_cell.percent()]%</b> charge left. Ctrl+Shift+Click to remove the [inserted_cell].</span>"
	else
