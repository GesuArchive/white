...
/datum/component/phylactery/proc/on_examine(datum/source, mob/user, list/examine_list)
	...
	if(IS_WIZARD(user) || isobserver(user))
		if(user.mind == lich_mind)
			...
			examine_list += "<span class='green'>Your phylactery. The next time you meet an untimely demise, you will revive at this object in <b>[time_to_revive / 10 / 60] minute\s</b>.</span>"
		else
			examine_list += "<span class='green'>A lich's phylactery. This one belongs to [lich_mind].</span>"

		if(num_resurrections > 0)
			examine_list += "<span class='green'><i>There's [num_resurrections] notches in the side of it.</i></span>"

	else
		examine_list += "<span class='green'>A terrible aura surrounds this item. Its very existence is offensive to life itself...</span>"
...
/datum/component/phylactery/proc/check_if_lich_died(datum/source, mob/living/died, gibbed)
	...
	to_chat(died, "<span class='green'>You feel your soul being dragged back to this world... <b>you will revive at your phylactery in [time_to_revive / 10 / 60] minute\s.</b></span>")
...
/datum/component/phylactery/proc/revive_lich(mob/living/corpse)
	...
	if(!QDELETED(corpse))
		...
		if(wheres_wizdo)
			corpse.visible_message("<span class='warning'>Suddenly, [corpse.name]'s corpse falls to pieces! You see a strange energy rise from the remains, and speed off towards the [wheres_wizdo]!</span>")
