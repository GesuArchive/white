...
/datum/component/pellet_cloud/proc/finalize()
	...
	for(var/atom/target in targets_hit)
		...
		if(num_hits > 1)
			target.visible_message("<span class='danger'>В[hit_part ? " [ru_parse_zone(hit_part.name)]" : ""] <b>[target]</b> попадают [num_hits] [proj_name][did_damage ? ", но не оставляют даже царапины" : ""]!</span>", "<span class='userdanger'>В[hit_part ? " [ru_parse_zone(hit_part.name)]" : ""] попадают [num_hits] [proj_name]!</span>", null, COMBAT_MESSAGE_RANGE, visible_message_flags = SPAM_MESSAGE)
		else
			target.visible_message("<span class='danger'>В[hit_part ? " [ru_parse_zone(hit_part.name)]" : ""] <b>[target]</b> попадает [proj_name][did_damage ? ", но не оставляет даже царапины" : ""]!</span>", "<span class='userdanger'>В[hit_part ? " [ru_parse_zone(hit_part.name)]" : ""] попадает [proj_name]!</span>", null, COMBAT_MESSAGE_RANGE, visible_message_flags = SPAM_MESSAGE)
