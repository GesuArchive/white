...
/datum/component/storage/proc/preattack_intercept(datum/source, obj/O, mob/M, params)
	...
	if(!len)
		to_chat(M, "<span class='warning'>Ничего не смог собрать в [parent]!</span>")
		return
...
/datum/component/storage/proc/mob_item_insertion_feedback(mob/user, mob/M, obj/item/I, override = FALSE)
	...
	for(var/mob/viewing in viewers(user, null))
		if(M == viewing)
			to_chat(usr, "<span class='notice'>Кладу [sklonenie(I.name, VINITELNI, I.gender)] [insert_preposition] [sklonenie(parent.name, VINITELNI, parent.gender)].</span>")
		else if(in_range(M, viewing)) //If someone is standing close enough, they can tell what it is...
			viewing.show_message("<span class='notice'>[M] кладёт [sklonenie(I.name, VINITELNI, I.gender)] [insert_preposition] [sklonenie(parent.name, VINITELNI, parent.gender)].</span>", MSG_VISUAL)
...
/datum/component/storage/proc/gather_mode_switch(mob/user)
	...
	switch(collection_mode)
		if(COLLECT_SAME)
			to_chat(user, "<span class='notice'>[parent] теперь собирает предметы одного типа разом.</span>")
		if(COLLECT_EVERYTHING)
			to_chat(user, "<span class='notice'>[parent] теперь собирает предметы всех типов разом.</span>")
		if(COLLECT_ONE)
			to_chat(user, "<span class='notice'>[parent] теперь собирает один предмет один раз.</span>")
