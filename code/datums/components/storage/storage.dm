...
/datum/component/storage/proc/can_be_inserted(obj/item/I, stop_messages = FALSE, mob/M)
	...
	if(length(can_hold))
		if(!is_type_in_typecache(I, can_hold))
			...
			return FALSE
	if(is_type_in_typecache(I, cant_hold) || HAS_TRAIT(I, TRAIT_NO_STORAGE_INSERT) || (can_hold_trait && !HAS_TRAIT(I, can_hold_trait))) //Items which this cont
		if(!stop_messages)
			to_chat(M, "<span class='warning'>[capitalize(host.name)] не может хранить [I.name]!</span>")
		return FALSE
