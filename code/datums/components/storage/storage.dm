...
/datum/component/storage/proc/mob_item_insertion_feedback(mob/user, mob/M, obj/item/I, override = FALSE)
	...
	for(var/mob/viewing in viewers(user, null))
		if(M == viewing)
			to_chat(usr, "<span class='notice'>Кладу [sklonenie(I.name, VINITELNI, I.gender)] [insert_preposition] [sklonenie(parent, VINITELNI)].</span>")
		else if(in_range(M, viewing)) //If someone is standing close enough, they can tell what it is...
			viewing.show_message("<span class='notice'>[M] кладёт [sklonenie(I.name, VINITELNI, I.gender)] [insert_preposition] [sklonenie(parent, VINITELNI)].</span>", MSG_VISUAL)
