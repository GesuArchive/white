...
/datum/component/food_storage/proc/try_inserting_item(datum/source, obj/item/inserted_item, mob/user, params)
	...
	if(inserted_item.w_class > minimum_weight_class)
		to_chat(user, "<span class='warning'>\The [inserted_item.name] won't fit in [parent].</span>")
		return

	if(!QDELETED(stored_item))
		to_chat(user, "<span class='warning'>There's something in [parent].</span>")
		return

	if(HAS_TRAIT(inserted_item, TRAIT_NODROP))
		to_chat(user, "<span class='warning'>\the [inserted_item] is stuck to your hand, you can't put into [parent]!</span>")
		return

	user.visible_message("<span class='notice'>[user.name] begins inserting [inserted_item.name] into [parent].</span>", \
					"<span class='notice'>You start to insert the [inserted_item.name] into [parent].</span>")
...
/datum/component/food_storage/proc/try_removing_item(datum/source, mob/user)
	...
	user.visible_message("<span class='notice'>[user.name] begins tearing at [parent].</span>", \
					"<span class='notice'>You start to rip into [parent].</span>")
...
/datum/component/food_storage/proc/insert_item(obj/item/inserted_item, mob/user)
	if(do_after(user, 1.5 SECONDS, target = parent))
		var/atom/food = parent
		to_chat(user, "<span class='notice'>You slip [inserted_item.name] inside [parent].</span>")
...
/datum/component/food_storage/proc/remove_item(mob/user)
	if(user.put_in_hands(stored_item))
		user.visible_message("<span class='warning'>[user.name] slowly pulls [stored_item.name] out of [parent].</span>", \
							"<span class='warning'>You slowly pull [stored_item.name] out of [parent].</span>")
	else
		stored_item.dropped()
		stored_item.visible_message("<span class='warning'>[stored_item.name] falls out of [parent].</span>")
...
/datum/component/food_storage/proc/consume_food_storage(datum/source, mob/living/target, mob/living/user, bitecount, bitesize)
	...
	if(prob(good_chance_of_discovery)) //finding the item, without biting it
		discovered = TRUE
		to_chat(target, "<span class='warning'>It feels like there's something in [parent]...!</span>")
