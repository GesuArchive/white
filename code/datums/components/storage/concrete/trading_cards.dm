...
/datum/component/storage/concrete/tcg/show_to(mob/M)
	. = ..()
	M.visible_message(span_notice("[M] starts to look through the contents of <b>[parent]</b>!"), \
					span_notice("You begin looking into the contents of <b>[parent]</b>!"))

/datum/component/storage/concrete/tcg/close(mob/M)
	. = ..()
	var/list/card_contents = contents()
	var/obj/temp_parent = parent
	temp_parent.visible_message(span_notice("<b>[capitalize(parent)]</b> is shuffled after looking through it."))
