...
/datum/ai_controller/dog/PerformIdleBehavior(delta_time)
	...
	if(blackboard[BB_SIMPLE_CARRY_ITEM] && DT_PROB(5, delta_time))
		...
		living_pawn.visible_message(span_notice("[living_pawn] gently teethes on <b>[carry_item]</b> in [living_pawn.p_their()] mouth."), vision_distance=COMBAT_MESSAGE_RANGE)
