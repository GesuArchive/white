...
/datum/status_effect/cult_master/proc/deathrattle()
	...
	for(var/datum/mind/B in SSticker.mode.cult)
		if(isliving(B.current))
			...
			to_chat(M, "<span class='cultlarge'>The Cult's Master, [owner], has fallen in [A]!</span>")
				...
				if(!itemUser.has_hand_for_held_index(hand))
					...
					if(((hand % 2) == 0))
						...
						if(L.attach_limb(itemUser))
							...
							return
					to_chat(itemUser, "<span class='notice'>Моя рука мгновенно отрастает и в ней появляется жезл Асклепия!</span>")
				else
					//Otherwise get rid of whatever else is in their hand and return the rod to said hand
					itemUser.put_in_hand(newRod, hand, forced = TRUE)
					to_chat(itemUser, "<span class='notice'>Посох Асклепия возвращается в мою руку!</span>")
