...
/datum/status_effect/high_fiving/on_creation(mob/living/new_owner, obj/item/slap)
	...
	slap_item = slap
	owner.visible_message(span_notice("[owner] raises [owner.ru_ego()] arm, looking for a high-five!") , \
	...
	if(!possible_takers) // in case we tried high-fiving with only a dead body around or something
		owner.visible_message(span_danger("[owner] realizes no one within range is actually capable of high-fiving, lowering [owner.ru_ego()] arm in shame...") , \
...
/datum/status_effect/high_fiving/proc/fail()
	owner.visible_message(span_danger("[owner] slowly lowers [owner.ru_ego()] arm, realizing no one will high-five [owner.ru_na()]! How embarassing...") , \
