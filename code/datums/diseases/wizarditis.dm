...
/datum/disease/wizarditis/stage_act(delta_time, times_fired)
	...
	switch(stage)
		if(2)
			...
			if(DT_PROB(0.25, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю, что [pick("у меня недостаточно маны", "потоки магии закончились", "надо кого-то призвать")].</span>")
		if(3)
			if(DT_PROB(0.25, delta_time))
				affected_mob.say(pick("NEC CANTIO!","AULIE OXIN FIERA!", "STI KALY!", "TARCOL MINTI ZHERI!"), forced = "wizarditis")
			if(DT_PROB(0.25, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю [pick("кипящую магию в моих венах", "эта местность даёт мне +1 к INT", "надобность кого-то призвать")].</span>")
		if(4)
			if(DT_PROB(0.5, delta_time))
				affected_mob.say(pick("NEC CANTIO!","AULIE OXIN FIERA!","STI KALY!","EI NATH!"), forced = "wizarditis")
				return
			if(DT_PROB(0.25, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю [pick("приливную волну новой силы внутри меня", "эта местность даёт мне +2 к INT и +1 к WIS", "надобность телепортироваться")].</span>")
