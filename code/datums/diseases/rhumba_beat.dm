	...
	switch(stage)
		if(2)
			...
			if(DT_PROB(0.5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Чувствую себя странно...</span>")
		if(3)
			if(DT_PROB(2.5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Пора танцевать...</span>")
			else if(DT_PROB(2.5, delta_time))
				affected_mob.emote("gasp")
			else if(DT_PROB(5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Нужно срочно чики-чики-бум...</span>")
		if(4)
			if(DT_PROB(10, delta_time))
				if(prob(50))
					...
					affected_mob.emote("gasp")
					to_chat(affected_mob, "<span class='danger'>Ощущаю горящий бит внутри...</span>")
		if(5)
			to_chat(affected_mob, "<span class='danger'>Моё тело не может сдержать ритм Румбы...</span>")
