	...
	switch(stage)
		if(2)
			if(DT_PROB(1, delta_time))
				...
				to_chat(affected_mob, "<span class='danger'>Грудь болит.</span>")
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='danger'>Желужок жужжит!</span>")
			if(DT_PROB(2.5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Потею.</span>")
		if(4)
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='userdanger'>Вижу всего по четыре!</span>")
			...
			if(DT_PROB(5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Воздух больно выходит из моих лёгких.</span>")
				affected_mob.adjustOxyLoss(25, FALSE)
				affected_mob.emote("gasp")
		if(5)
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='userdanger'>[pick("Сердце замедляется...", "Расслабляюсь вместе с сердечным ритмом.")]</span>")
				affected_mob.adjustStaminaLoss(70, FALSE)
			if(DT_PROB(5, delta_time))
				affected_mob.adjustStaminaLoss(100, FALSE)
				affected_mob.visible_message("<span class='warning'>[affected_mob] ослабевает!</span>" , "<span class='userdanger'>Сдаюсь и ощущаю умиротворение...</span>")
				affected_mob.AdjustSleeping(100)
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='userdanger'>Мой разум расслабляется и мысли текут!</span>")
				affected_mob.set_confusion(min(100, affected_mob.get_confusion() + 8))
			if(DT_PROB(5, delta_time))
				affected_mob.vomit(20)
			if(DT_PROB(1.5, delta_time))
				to_chat(affected_mob, "<span class='warning'><i>[pick("Желудок тихо жужжит...", "Живот содрогается в последний раз, безжизненный взгляд застывает...", "Хочу съесть мелок")]</i></span>")
				affected_mob.overeatduration = max(affected_mob.overeatduration - (200 SECONDS), 0)
				affected_mob.adjust_nutrition(-100)
			if(DT_PROB(7.5, delta_time))
				to_chat(affected_mob, "<span class='danger'>[pick("Слишком жарко...", "Надо расстегнуть костюм...", "Надо снять одежду...")]</span>")
