	...
	switch(stage)
		if(2)
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю удар током всем своим телом.</span>")
			if(DT_PROB(1, delta_time))
		...
		if(3)
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю удар током всем своим телом.</span>")
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю себя как клоуна.</span>")
		...
		if(4)
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю сильный удар током всем своим телом.</span>")
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='danger'>Природа чудесная.</span>")
