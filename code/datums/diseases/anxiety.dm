	...
	switch(stage)
		if(2) //also changes say, see say.dm
			if(DT_PROB(2.5, delta_time))
				to_chat(affected_mob, "<span class='notice'>Беспокойно.</span>")
		if(3)
			if(DT_PROB(5, delta_time))
				to_chat(affected_mob, "<span class='notice'>Живот трепещет.</span>")
			if(DT_PROB(2.5, delta_time))
				to_chat(affected_mob, "<span class='notice'>Паника накатывает.</span>")
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='danger'>ПАНИКУЮ!</span>")
				affected_mob.add_confusion(rand(2,3))
		if(4)
			if(DT_PROB(5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю бабочек в своём животе.</span>")
			if(DT_PROB(2.5, delta_time))
				affected_mob.visible_message("<span class='danger'>[affected_mob] спотыкается в панике.</span>" , \
												"<span class='userdanger'>ПАНИЧЕСКАЯ АТАКА!</span>")
			...
			if(DT_PROB(1, delta_time))
				affected_mob.visible_message("<span class='danger'>[affected_mob] выкашливает бабочек!</span>" , \
													"<span class='userdanger'>Выкашливаю бабочек!</span>")
