	...
	switch(stage)
		if(1)
			if(DT_PROB(5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю себя немного глупо.</span>")
		if(2)
			if(DT_PROB(5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Вижу радуги.</span>")
		if(3)
			if(DT_PROB(5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Мысли прерываются громким <b>ХОНКОМ!</b></span>")
		if(4)
