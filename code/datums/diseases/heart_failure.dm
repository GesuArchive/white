	...
	switch(stage)
		if(1 to 2)
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='warning'>Ощущаю [pick("дискомфорт", "давление", "жжение", "боль")] в груди.</span>")
			if(DT_PROB(1, delta_time))
				to_chat(affected_mob, "<span class='warning'>Ощущаю головокружение.</span>")
				affected_mob.add_confusion(6)
			if(DT_PROB(1.5, delta_time))
				to_chat(affected_mob, "<span class='warning'>Ощущаю [pick("полноту", "тошноту", "пот", "слабость", "усталость", "нехватку кислорода", "себя плохо")].</span>")
		if(3 to 4)
			if(!sound)
				affected_mob.playsound_local(affected_mob, 'sound/health/slowbeat.ogg', 40, FALSE, channel = CHANNEL_HEARTBEAT, use_reverb = FALSE)
				sound = TRUE
			if(DT_PROB(1.5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю острую боль в груди!</span>")
				if(prob(25))
			...
			if(DT_PROB(1.5, delta_time))
				to_chat(affected_mob, "<span class='danger'>Ощущаю сильную слабость и головокружение...</span>")
		...
		if(5)
			...
			if(affected_mob.stat == CONSCIOUS)
				affected_mob.visible_message("<span class='danger'>[affected_mob] хватается за [affected_mob.ru_ego()] грудь, так как [affected_mob.ru_ego()] сердце остановилось!</span>" , \
					"<span class='userdanger'>Ощущаю ужасную боль в груди, так как сердечко всё!</span>")
