	...
	switch(A.stage)
		if(3, 4)
			if(prob(base_message_chance) && !suppress_warning)
				to_chat(M, "<span class='warning'>[pick("Слышу звон в ушах.", "В ушах стреляет.")]</span>")
		if(5)
			if(power >= 2)
				if(ears.damage < ears.maxHealth)
					to_chat(M, "<span class='userdanger'>Уши болезненно стреляют и начинают кровоточить!</span>")
					ears.damage = max(ears.damage, ears.maxHealth)
					M.emote("agony")
			else
				to_chat(M, "<span class='userdanger'>В УШАХ ЗВЕНИТ!</span>")
