	...
	switch(A.stage)
		if(4, 5)
			...
			if(HAS_TRAIT_FROM(M, TRAIT_BLIND, EYE_DAMAGE))
				if(prob(20))
					to_chat(M, "<span class='warning'>Зрение возвращается...</span>")
					M.cure_blind(EYE_DAMAGE)
					M.cure_nearsighted(EYE_DAMAGE)
					M.blur_eyes(35)
			else if(HAS_TRAIT_FROM(M, TRAIT_NEARSIGHT, EYE_DAMAGE))
				to_chat(M, "<span class='warning'>Темнота уходит из периферийного зрения.</span>")
				M.cure_nearsighted(EYE_DAMAGE)
				M.blur_eyes(10)
		else
			if(prob(base_message_chance))
				to_chat(M, "<span class='notice'>[pick("Глазам стало лучше.","Глаза теперь могут видеть лучше.", "Можно не моргать.","Ушам стало лучше.","Слышу всё гораздо лучше.")]</span>")
