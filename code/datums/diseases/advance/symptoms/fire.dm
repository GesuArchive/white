	...
	switch(A.stage)
		if(3)
			if(prob(base_message_chance) && !suppress_warning)
				to_chat(M, "<span class='warning'>[pick("Жарко.", "Что-то щёлкает.", "Пахнет дымом.")]</span>")
		if(4)
			Firestacks_stage_4(M, A)
			M.ignite_mob()
			to_chat(M, "<span class='userdanger'>КОЖА ГОРИТ!</span>")
			M.emote("agony")
		if(5)
	...
	switch(A.stage)
		if(3)
			if(prob(base_message_chance))
				to_chat(M, "<span class='warning'>[pick("Вены кипят.", "Жарко.", "Кто-то готовит мясо.")]</span>")
		if(4)
			if(M.fire_stacks < 0)
				M.visible_message("<span class='warning'>Потная кожа [M] шипит и трескается при контакте с водой!</span>")
			...
			M.ignite_mob()
			to_chat(M, "<span class='userdanger'>Ваш пот загорается!</span>")
			M.emote("agony")
		if(5)
			if(M.fire_stacks < 0)
				M.visible_message("<span class='warning'>Потная кожа [M] шипит и трескается при контакте с водой!</span>")
