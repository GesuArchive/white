...
/datum/disease/parasite/stage_act(delta_time, times_fired)
	...
	if(!affected_liver)
		affected_mob.visible_message("<span class='notice'><B>Печень [affected_mob] покрыта крошечными личинками! Они быстро сморщиваются и умирают после пребывания на открытом воздухе.</B></span>")
		cure()
		return FALSE
	...
	switch(stage)
		...
		if(2)
			if(DT_PROB(5, delta_time))
				if(prob(50))
					to_chat(affected_mob, "<span class='notice'>Ощущаю потерю веса!</span>")
				affected_mob.adjust_nutrition(-3)
		if(3)
			if(DT_PROB(10, delta_time))
				if(prob(20))
					to_chat(affected_mob, "<span class='notice'>Я... ОЧЕНЬ лёгкий.</span>")
				affected_mob.adjust_nutrition(-6)
		if(4)
			if(DT_PROB(16, delta_time))
				if(affected_mob.nutrition >= 100)
					if(prob(10))
						to_chat(affected_mob, "<span class='warning'>Теряю вес на глазах!</span>")
					affected_mob.adjust_nutrition(-12)
				else
					to_chat(affected_mob, "<span class='warning'>Ощущаю НЕВЕРОЯТНУЮ лёгкость тела!</span>")
