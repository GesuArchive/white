...
/datum/skill/proc/try_skill_reward(datum/mind/mind, new_level)
	...
	if (!ispath(skill_cape_path))
		to_chat(mind.current, "<span class='nicegreen'>Мой легендарный навык [name] весьма крут, однако Ассоциация Профессионалов [title] не имеет особых символов чтобы отметить это.</span>")
		return
	if (LAZYFIND(mind.skills_rewarded, src.type))
		to_chat(mind.current, "<span class='nicegreen'>Похоже,  Ассоциация Профессионалов [title] не хочет давать мне больше символов навыка.</span>")
		return
	podspawn(list(
	...
	to_chat(mind.current, "<span class='nicegreen'>Мой легендарный навык привлёк Ассоциацию Профессионалов [title]. Похоже, они выслали мне особый символ, чтобы отметить это.</span>")
