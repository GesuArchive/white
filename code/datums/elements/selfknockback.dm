...
/datum/element/selfknockback/proc/Item_SelfKnockback(obj/item/I, atom/attacktarget, mob/usertarget, proximity_flag)
	...
	if(proximity_flag || (get_dist(attacktarget, usertarget) <= I.reach))
		...
		usertarget.visible_message("<span class='warning'>[usertarget] отлетает благодаря силе [I] ударившей [attacktarget]!</span>", "<span class='warning'>Сила удара [I] по [attacktarget] принуждает меня отлететь!</span>")
