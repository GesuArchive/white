...
/atom/movable/screen/plane_master/frill/backdrop(mob/mymob)
	...
	if(!client_prefs.frills_over_floors)
		add_filter(FRILL_FLOOR_CUT, 1, list("type" = "alpha", "render_source" = FLOOR_PLANE_RENDER_TARGET, "flags" = MASK_SWAP))
	if(client_prefs.ambientocclusion)
		add_filter(FRILL_GAME_CUT, 1, list("type" = "alpha", "x" = 6, "y" = -7, "render_source" = GAME_PLANE_RENDER_TARGET, "flags" = MASK_SWAP))
	else
		add_filter(FRILL_GAME_CUT, 1, list("type" = "alpha", "render_source" = GAME_PLANE_RENDER_TARGET, "flags" = MASK_SWAP))
