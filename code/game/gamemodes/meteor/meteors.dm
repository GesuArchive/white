...
/proc/spawn_meteor(list/meteortypes)
	...
	while(!isspaceturf(pickedstart) || !istype(pickedstart, /turf/open/openspace))
...
/obj/effect/meteor/proc/ram_turf(turf/T)
	...
	for(var/thing in T)
		...
		if(isliving(thing))
			...
			living_thing.visible_message("<span class='warning'>[capitalize(src.name)] slams into [living_thing].</span>", "<span class='userdanger'>[capitalize(src.name)] slams into you!.</span>")
		switch(hitpwr)
