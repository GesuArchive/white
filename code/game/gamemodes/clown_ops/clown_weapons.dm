...
/obj/item/melee/transforming/energy/sword/bananium/suicide_act(mob/user)
	...
	user.visible_message("<span class='suicide'>[user] is [pick("slitting [user.ru_ego()] stomach open with", "falling on")] [src]! It looks like [user.p_theyre()] trying to commit seppuku, but the blade slips off of [user.ru_na()] harmlessly!</span>")
	var/datum/component/slippery/slipper = GetComponent(/datum/component/slippery)
...
/obj/item/shield/energy/bananium/throw_at(atom/target, range, speed, mob/thrower, spin=1, diagonals_first = 0, datum/callback/callback, force, gentle = FALSE, quickstart = TRUE, params)
	if(active)
		if(iscarbon(thrower))
