...
/obj/item/melee/transforming/energy/sword/bananium/suicide_act(mob/user)
	...
	user.visible_message(span_suicide("[user] is [pick("slitting [user.ru_ego()] stomach open with", "falling on")] [src]! It looks like [user.p_theyre()] trying to commit seppuku, but the blade slips off of [user.ru_na()] harmlessly!"))
	var/datum/component/slippery/slipper = GetComponent(/datum/component/slippery)
