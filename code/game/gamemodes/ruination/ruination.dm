...
/datum/game_mode/ruination/process()
	if(!started_at && GLOB.pulse_engines.len)
		for(var/obj/structure/pulse_engine/PE in GLOB.pulse_engines)
			if(PE.engine_active)
				...
				priority_announce("На вашей станции был обнаружен запуск одного или нескольких импульсных двигателей. Не паникуйте, это всего лишь запланированное перемещение вашей станции на новую орбиту.", null, 'white/valtos/sounds/trevoga2.ogg', "Priority")
	...
	if(started_at)
		if((started_at + (win_time - 16 MINUTES)) < world.time && announce_stage == 0)
			...
			priority_announce("Осталось 15 минут до активации блюспейс-транслокатора.", null, 'white/valtos/sounds/trevoga2.ogg', sender_override = "Синдикат")
		...
		if((started_at + (win_time - 10 MINUTES)) < world.time && announce_stage == 1)
			...
			priority_announce("Осталось 10 минут до активации блюспейс-транслокатора.", null, 'white/valtos/sounds/trevoga2.ogg', sender_override = "Синдикат")
		if((started_at + (win_time - 5 MINUTES)) < world.time && announce_stage == 2)
			...
			priority_announce("Осталось 5 минут до активации блюспейс-транслокатора. Держитесь.", null, 'white/valtos/sounds/trevoga2.ogg', sender_override = "Синдикат")
