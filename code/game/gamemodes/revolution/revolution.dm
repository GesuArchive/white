...
/datum/game_mode/revolution
	...
	restricted_jobs = list(JOB_PRISONER, JOB_RUSSIAN_OFFICER, JOB_TRADER, JOB_HACKER, JOB_VETERAN, JOB_SECURITY_OFFICER, JOB_WARDEN, JOB_DETECTIVE, JOB_AI, JOB_CYBORG,JOB_CAPTAIN, JOB_HEAD_OF_PERSONNEL, JOB_HEAD_OF_SECURITY, JOB_CHIEF_ENGINEER, JOB_RESEARCH_DIRECTOR, JOB_CHIEF_MEDICAL_OFFICER, JOB_FIELD_MEDIC, JOB_SPECIALIST, JOB_INTERN)
	required_jobs = list(list(JOB_CAPTAIN=1),list(JOB_HEAD_OF_PERSONNEL=1),list(JOB_HEAD_OF_SECURITY=1),list(JOB_CHIEF_ENGINEER=1),list(JOB_RESEARCH_DIRECTOR=1),list(JOB_CHIEF_MEDICAL_OFFICER=1)) //Any head present
	required_players = 30
	required_enemies = 2
...
/datum/game_mode/revolution/generate_report()
	return "Беспорядки среди сотрудников резко возросли в последние недели, когда руководители сотрудников предприняли несколько попыток мятежа. \
	Было замечено, что некоторые члены экипажа используют устройства вспышки, чтобы ослепить своих коллег, которые затем беспрекословно выполняют \
	их приказы и работают над свержением руководителей отделов. С осторожностью наблюдайте за таким поведением. Если экипаж попытается поднять мятеж, \
	вы и руководители полностью уполномочены казнить их, используя смертоносное оружие - позже они будут клонированы и допрошены в Центральном Командовании."
