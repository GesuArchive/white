...
/obj/machinery/syndicatebomb/attackby(obj/item/I, mob/user, params)
	if(HAS_TRAIT(user, TRAIT_PACIFISM))
		to_chat(user, "<span class='warning'>Не могу как-то взаимодействовать с бомбой!</span>")
		return FALSE
	if(I.tool_behaviour == TOOL_WRENCH && can_unanchor)
		...
		if(I.use_tool(src, user, 20, volume=50, amount=5)) //uses up 5 fuel
			to_chat(user, "<span class='notice'>Разрезал [src] на части.</span>")
...
/obj/machinery/syndicatebomb/proc/settings(mob/user)
	if(HAS_TRAIT(user, TRAIT_PACIFISM))
		to_chat(user, "<span class='warning'>Не могу заставить себя поставить бомбу! Это ведь навредит кому-то!</span>")
		return FALSE
