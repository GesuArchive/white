...
/obj/machinery/clonepod
	...
	fair_market_price = 1 // He nodded, because he knew I was right. Then he swiped his credit card to pay me for arresting him.
...
/obj/machinery/clonepod/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += "<hr><span class='notice'>Дисплей: Cloning speed at <b>[speed_coeff*50]%</b>.<br>Predicted amount of cellular damage: <b>[100-heal_level]%</b>.</span>"
		if(efficiency > 5)
...
/obj/machinery/clonepod/process()
	...
	if(!is_operational()) //Autoeject if power is lost
		...
		else if(SSeconomy.full_ancap && current_insurance && current_insurance.adjust_money(-fair_market_price))
			var/datum/bank_account/D = SSeconomy.get_dep_account(payment_department)
			if(D)
				D.adjust_money(fair_market_price)
