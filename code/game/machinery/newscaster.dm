	...
	if(ishuman(user) || issilicon(user))
		...
		switch(screen)
			...
			if(11)
				...
				dat+="Можно снять эту метку, если у вас есть соответствующий доступ.</FONT><HR>"
				if(!length(GLOB.news_network.network_channels))
...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && isturf(loc))) || issilicon(usr))
		...
		if(href_list["set_channel_name"])
			...
			channel_name = input(usr, "Выбрать бы канал", "Общая сеть") in sort_list(available_channels)
			updateUsrDialog()
