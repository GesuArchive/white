...
/obj/machinery/quantumpad/attackby(obj/item/I, mob/user, params)
	...
	if(panel_open)
		if(I.tool_behaviour == TOOL_MULTITOOL)
			...
			M.buffer = src
			to_chat(user, span_notice("You save the data in [I] buffer. It can now be saved to pads with closed panels."))
			return TRUE
		...
		if(istype(M.buffer, /obj/machinery/quantumpad))
			if(M.buffer == src)
				...
				linked_pad = M.buffer
				to_chat(user, span_notice("You link [src] to the one in [I] buffer."))
				return TRUE
		else
			to_chat(user, span_warning("There is no quantum pad data saved in [I] buffer!"))
			return TRUE

	else if(istype(I, /obj/item/quantum_keycard))
		var/obj/item/quantum_keycard/K = I
		if(K.qpad)
			to_chat(user, span_notice("You insert [K] into [src] card slot, activating it."))
			interact(user, K.qpad)
		else
			to_chat(user, span_notice("You insert [K] into [src] card slot, initiating the link procedure."))
...
/obj/machinery/quantumpad/interact(mob/user, obj/machinery/quantumpad/target_pad = linked_pad)
	...
	if(world.time < last_teleport + teleport_cooldown)
		to_chat(user, span_warning("[capitalize(src.name)] is recharging power. Please wait [DisplayTimeText(last_teleport + teleport_cooldown - world.time)]."))
		return

	if(teleporting)
		to_chat(user, span_warning("[capitalize(src.name)] is charging up. Please wait."))
		return
	...
	if(target_pad)
		...
		spawn(teleport_speed)
			...
			if(machine_stat & NOPOWER)
				to_chat(user, span_warning("[capitalize(src.name)] is unpowered!"))
				teleporting = FALSE
				return
