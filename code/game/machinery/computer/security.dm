...
//RECORD DELETE
			...
			if("Add Entry")
				...
				while(active2.fields[text("com_[]", counter)])
					counter++
				active2.fields[text("com_[]", counter)] = text("Сделано [] ([]) в [] []/[]<BR>[]", src.authenticated, src.rank, station_time_timestamp(), time2text(world.realtime, "DD/MM"), CURRENT_STATION_YEAR, t1)
