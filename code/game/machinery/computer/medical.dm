...
<BR>
<BR><b>Danger:</b> [initial(type.severity)]"}
				...
				while(active2.fields[text("com_[]", counter)])
					counter++
				active2.fields[text("com_[]", counter)] = text("Сделано [] ([]) в [] []/[]<BR>[]", authenticated, rank, station_time_timestamp(), time2text(world.realtime, "DD/MM"), CURRENT_STATION_YEAR, t1)
