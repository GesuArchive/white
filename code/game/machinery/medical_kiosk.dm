...
/obj/machinery/medical_kiosk/emag_act(mob/user)
	...
	if(obj_flags & EMAGGED)
		return
	if(user)
		user.visible_message(span_warning("[user] waves a suspicious card by the [src] biometric scanner!") ,
...
/obj/machinery/medical_kiosk/examine(mob/user)
	...
	if(scanner_wand == null)
		...
		. += "<hr><span class='notice'><b>[src.name]</b> has its scanner clipped to the side. ПКМ to remove.</span>"

/obj/machinery/medical_kiosk/ui_interact(mob/user, datum/tgui/ui)
	var/patient_distance = 0
	if(!ishuman(user))
		to_chat(user, span_warning("[capitalize(src.name)] is unable to interface with non-humanoids!"))
		if (ui)
			ui.close()
		return
...
/obj/machinery/medical_kiosk/ui_data(mob/living/carbon/human/user)
	...
	var/obj/item/organ/stomach/belly = altPatient.get_organ_slot(ORGAN_SLOT_STOMACH)
