	...
	switch(mode)
		if(DRONE_READY)
			...
			if(begin_create_message)
				visible_message(span_notice("[capitalize(src.name)] [begin_create_message]"))
			if(work_sound)
		...
		if(DRONE_PRODUCTION)
			...
			if(end_create_message)
				visible_message(span_notice("[capitalize(src.name)] [end_create_message]"))
		...
		if(DRONE_RECHARGING)
			...
			if(recharge_message)
				visible_message(span_notice("[capitalize(src.name)] [recharge_message]"))
...
/obj/machinery/drone_dispenser/attackby(obj/item/I, mob/living/user)
	if(I.tool_behaviour == TOOL_CROWBAR)
		...
		if(!(machine_stat & BROKEN))
			to_chat(user, span_warning("[capitalize(src.name)] doesn't need repairs."))
			return
...
/obj/machinery/drone_dispenser/obj_break(damage_flag)
	...
	if(!.)
		return
	if(break_message)
		audible_message(span_warning("[capitalize(src.name)] [break_message]"))
	if(break_sound)
