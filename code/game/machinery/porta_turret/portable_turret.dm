...
/obj/machinery/porta_turret/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		return
	to_chat(user, span_warning("You short out [src] threat assessment circuits."))
	audible_message(span_hear("[capitalize(src.name)] hums oddly..."))
	obj_flags |= EMAGGED
	controllock = TRUE
...
/obj/machinery/turretid/attackby(obj/item/I, mob/user, params)
	...
	if(I.tool_behaviour == TOOL_MULTITOOL)
		...
		if(M.buffer && istype(M.buffer, /obj/machinery/porta_turret))
			turrets |= M.buffer
			to_chat(user, span_notice("You link [M.buffer] with <b>[src.name]</b>."))
			return
