	...
	switch(state)
		...
		if(STATE_WIRED)	// Upgrades!
			if(istype(W, /obj/item/stack/sheet/mineral/plasma)) //emp upgrade
				if(emp_module)
					to_chat(user, "<span class='warning'>[capitalize(src.name)] уже содержит [emp_module]!</span>")
					return
				...
				to_chat(user, "<span class='notice'>Присоединяю [W] к микросхеме [src].</span>")
				return

			else if(istype(W, /obj/item/analyzer)) //xray upgrade
				if(xray_module)
					to_chat(user, "<span class='warning'>[capitalize(src.name)] уже содержит [xray_module]!</span>")
					return
				...
				if(proxy_module)
					to_chat(user, "<span class='warning'>[capitalize(src.name)] уже содержит [proxy_module]!</span>")
					return
				if(!user.transferItemToLoc(W, src))
					return
				to_chat(user, "<span class='notice'>Присоединяю [W] к микросхеме [src].</span>")
...
/obj/structure/camera_assembly/crowbar_act(mob/user, obj/item/tool)
	...
	if(!droppable_parts.len)
		return
	var/obj/item/choice = tgui_input_list(user, "Выберите деталь для демонтажа:", src, sort_names(droppable_parts))
	if(!choice || !user.canUseTopic(src, BE_CLOSE, FALSE, NO_TK))
		return
	to_chat(user, "<span class='notice'>Отключаю [choice] от [src].</span>")
	drop_upgrade(choice)
