...
/obj/machinery/dish_drive/process()
	...
	for(var/obj/item/I in view(4, src))
		if(is_type_in_list(I, collectable_items) && I.loc != src && (!I.reagents || !I.reagents.total_volume))
			if(I.Adjacent(src))
				LAZYADD(dish_drive_contents, I)
				visible_message("<span class='notice'>[capitalize(src.name)] beams up [I]!</span>")
				I.forceMove(src)
				playsound(src, 'sound/items/pshoom.ogg', 50, TRUE)
				flick("synthesizer_beam", src)
			else
...
/obj/machinery/dish_drive/proc/do_the_dishes(manual)
	...
	if(!bin)
		if(manual)
			visible_message("<span class='warning'>[capitalize(src.name)] buzzes. There are no disposal bins in range!</span>")
	...
	for(var/obj/item/I in dish_drive_contents)
		if(is_type_in_list(I, disposable_items))
			...
			disposed++
	if (disposed)
		visible_message("<span class='notice'>[capitalize(src.name)] [pick("whooshes", "bwooms", "fwooms", "pshooms")] and beams [disposed] stored item\s into the nearby [bin.name].</span>")
		playsound(src, 'sound/items/pshoom.ogg', 50, TRUE)
