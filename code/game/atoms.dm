		...
		if(reagents)
			...
			switch(tgui_alert(usr, "Choose a method.", "Add Reagents", list("Search", "Choose from a list", "I'm feeling lucky")))
				...
				if("Choose from a list")
					chosen_id = tgui_input_list(usr, "Choose a reagent to add.", "Choose a reagent.0", sort_list(subtypesof(/datum/reagent), GLOBAL_PROC_REF(cmp_typepaths_asc)))
...
/atom/proc/on_mouse_enter(client/client)
	...
	if(ishuman(user) && !(HAS_TRAIT(user, TRAIT_XRAY_VISION)) && !(src in view(user)))
