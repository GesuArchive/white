...
/turf/open/lava/attackby(obj/item/C, mob/user, params)
	...
	if(istype(C, /obj/item/clothing/mask/cigarette))
		...
		if(prob(25 * clumsy_modifier ))
			ciggie.light(span_warning("[user] expertly dips <b>[ciggie.name]</b> into [src], along with the rest of [user.p_their()] arm. What a dumbass."))
			var/obj/item/bodypart/affecting = user.get_active_hand()
			affecting?.receive_damage(burn = 90)
		else
			ciggie.light(span_rose("[user] expertly dips <b>[ciggie.name]</b> into [src], lighting it with the scorching heat of the planet. Witnessing such a feat is almost enough to make you cry."))
		return TRUE

/turf/open/lava/proc/is_safe()
	//if anything matching this typecache is found in the lava, we don't burn things
	var/static/list/lava_safeties_typecache = typecacheof(list(/obj/structure/lattice/catwalk, /obj/structure/stone_tile, /obj/structure/lattice/lava, /obj/structure/industrial_lift/tram/caves))
