...
/obj/item/paicard/suicide_act(mob/living/user)
	user.visible_message(span_suicide("[user] is staring sadly at [src]! [user.ru_who()] can't keep living without real human intimacy!"))
	return OXYLOSS
...
/obj/item/paicard/attack_self(mob/user)
	...
	var/dat = "<head><meta http-equiv='Content-Type' content='text/html; charset=utf-8' /></head><TT><B>Personal AI Device</B><BR>"
	if(pai)
...
/obj/item/paicard/proc/alertUpdate()
	...
	loc.visible_message(span_info("[capitalize(src.name)] flashes a message across its screen, \"Additional personalities available for download.\"") , blind_message = span_notice("[capitalize(src.name)] vibrates with an alert."))
