...
/proc/healthscan(mob/user, mob/living/M, mode = SCANNER_VERBOSE, advanced = FALSE, tochat = TRUE)
	if(!M.getorganslot(ORGAN_SLOT_BRAIN_BIOMONITOR))
		...
		if(user.is_blind())
			to_chat(user, "<span class='warning'>Этот сканер не адаптирован для слепых! Я буду жаловаться в профсоюз!</span>")
			return
...
/proc/chemscan(mob/living/user, mob/living/M)
	...
	if(user.is_blind())
		to_chat(user, "<span class='warning'>Я полностью слеп и не вижу показателей анализатора!</span>")
		return
...
/obj/item/proc/scangasses(mob/user)
	...
	if(total_moles)
		...
		if(abs(n2_concentration - N2STANDARD) < 20)
			to_chat(user, "<span class='info'>Азот: [round(n2_concentration*100, 0.01)] % ([round(environment.get_moles(GAS_N2), 0.01)] моль)</span>")
		else
			to_chat(user, "<span class='alert'>Азот: [round(n2_concentration*100, 0.01)] % ([round(environment.get_moles(GAS_N2), 0.01)] моль)</span>")

		if(abs(o2_concentration - O2STANDARD) < 2)
			to_chat(user, "<span class='info'>Кислород: [round(o2_concentration*100, 0.01)] % ([round(environment.get_moles(GAS_O2), 0.01)] моль)</span>")
		else
			to_chat(user, "<span class='alert'>Кислород: [round(o2_concentration*100, 0.01)] % ([round(environment.get_moles(GAS_O2), 0.01)] моль)</span>")

		if(co2_concentration > 0.01)
			to_chat(user, "<span class='alert'>CO2: [round(co2_concentration*100, 0.01)] % ([round(environment.get_moles(GAS_CO2), 0.01)] моль)</span>")
		else
			to_chat(user, "<span class='info'>CO2: [round(co2_concentration*100, 0.01)] % ([round(environment.get_moles(GAS_CO2), 0.01)] моль)</span>")

		if(plasma_concentration > 0.005)
			to_chat(user, "<span class='alert'>Плазма: [round(plasma_concentration*100, 0.01)] % ([round(environment.get_moles(GAS_PLASMA), 0.01)] моль)</span>")
		else
			to_chat(user, "<span class='info'>Плазма: [round(plasma_concentration*100, 0.01)] % ([round(environment.get_moles(GAS_PLASMA), 0.01)] моль)</span>")

		for(var/id in environment.get_gases())
			if(id in GLOB.hardcoded_gases)
				continue
			var/gas_concentration = environment.get_moles(id)/total_moles
			to_chat(user, "<span class='alert'>[GLOB.gas_data.names[id]]: [round(gas_concentration*100, 0.01)] % ([round(environment.get_moles(id), 0.01)] моль)</span>")
	...
	for(var/g in airs)
		...
		if(total_moles > 0)
			...
			for(var/id in air_contents.get_gases())
				...
				render_list += "<span class='notice'>[GLOB.gas_data.names[id]]: [round(gas_concentration*100, 0.01)] % ([round(air_contents.get_moles(id), 0.01)] моль)</span>"
...
/proc/slime_scan(mob/living/simple_animal/slime/T, mob/living/user)
	...
	if (T.nutrition < T.get_starve_nutrition())
		to_render += "<span class='warning'>\nВнимание: слайм голодает!</span>"
	else if (T.nutrition < T.get_hunger_nutrition())
		to_render += "<span class='warning'>\nВнимание: слайм хочет кушать</span>"
...
/obj/item/sequence_scanner/proc/gene_scan(mob/living/carbon/C, mob/living/user)
	...
	to_chat(user, "<span class='notice'>Генетическая стабильность: [C.dna.stability]%.</span>")
