...
/obj/item/aicard/suicide_act(mob/living/user)
	user.visible_message("<span class='suicide'>[user] пытается upload [user.ru_na()]self into [src]! That's not going to work out well!</span>")
	return BRUTELOSS
...
/obj/item/aicard/ui_act(action,params)
	...
	switch(action)
		...
		if("wireless")
			...
			to_chat(AI, "<span class='warning'>[capitalize(src.name)] wireless port has been [AI.control_disabled ? "disabled" : "enabled"]!</span>")
			. = TRUE
		if("radio")
