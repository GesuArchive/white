...
/obj/item/aicard/suicide_act(mob/living/user)
	user.visible_message(span_suicide("[user] пытается upload [user.ru_na()]self into [src]! That's not going to work out well!") )
	return BRUTELOSS
	...
	switch(action)
		if("wipe")
			if(flush)
				...
				var/confirm = tgui_alert(usr, "Are you sure you want to wipe this card's memory?", name, "Yes", "No")
				if(confirm == "Yes" && !..())
					flush = TRUE
					if(AI && AI.loc == src)
		...
		if("wireless")
			...
			to_chat(AI, span_warning("[capitalize(src.name)] wireless port has been [AI.control_disabled ? "disabled" : "enabled"]!") )
			. = TRUE
		if("radio")
