/obj/item/storage/portable_chem_mixer
	...
	desc = "Портативное устройство для хранения и смешивания химикатов. Изначально пуст и все необходимые вещества необходимо помещать внутрь при помощи хим-стаканов. В инструкции написано, что для получения доступа к химическому банку необходима отвертка. Переносится на поясе. На внутренней стороне видел логотип S&T."
...
/obj/item/storage/portable_chem_mixer/ui_data(mob/user)
	...
	for(var/re in dispensable_reagents)
		...
		if(temp)
			...
			chemicals.Add(list(list("title" = chemname, "id" = lowertext(temp.name), "volume" = total_volume )))
	data["chemicals"] = chemicals
	var/beakerContents[0]
	if(beaker)
		for(var/datum/reagent/R in beaker.reagents.reagent_list)
			beakerContents.Add(list(list("name" = R.name, "id" = lowertext(R.name), "volume" = R.volume))) // list in a list because Byond merges the first list...
