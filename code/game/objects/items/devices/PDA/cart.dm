			...
			if (cl)
				...
				for (var/obj/item/mop/M in world)
					...
					if(ml)
						...
						ldat += "Mop - <b>\[[ml.x],[ml.y] ([uppertext(dir2ru_text(direction))])\]</b> - [M.reagents.total_volume ? "Wet" : "Dry"]<br>"
				...
				for (var/obj/structure/janitorialcart/B in world)
					...
					if(bl)
						...
						ldat += "Cart - <b>\[[bl.x],[bl.y] ([uppertext(dir2ru_text(direction))])\]</b> - Water level: [B.reagents.total_volume]/100<br>"
				...
				for (var/mob/living/simple_animal/bot/cleanbot/B in GLOB.alive_mob_list)
					...
					if(bl)
						if (bl.z != cl.z)
							continue
						var/direction = get_dir(src, B)
						ldat += "Cleanbot - <b>\[[bl.x],[bl.y] ([uppertext(dir2ru_text(direction))])\]</b> - [B.on ? "Online" : "Offline"]<br>"
...
/obj/item/cartridge/Topic(href, href_list)
	...
	switch(href_list["choice"])
		...
		if("Newscaster Message")
			var/host_pda_owner_name = host_pda.id ? "[host_pda.id.registered_name] ([host_pda.id.assignment])" : "Неизвестный"
