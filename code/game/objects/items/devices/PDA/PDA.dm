...
/obj/item/pda/proc/send_message(mob/living/user, list/obj/item/pda/targets, everyone)
	...
	// Show it to ghosts
	var/ghost_message = "<span class='name'>[owner] </span><span class='game say'>Сообщение PDA</span> --> <span class='name'>[target_text]</span>: <span class='message'>[signal.format_message()]</span>"
	...
	if(istype(C))
		switch(scanmode)
			...
			if(PDA_SCANNER_MEDICAL)
				C.visible_message("<span class='notice'>[user] analyzes [C] vitals.</span>")
				healthscan(user, C, 1)
				add_fingerprint(user)

			if(PDA_SCANNER_HALOGEN)
				C.visible_message("<span class='notice'>[user] analyzes [C] radiation levels.</span>")
...
/obj/item/pda/proc/explode() //This needs tuning.
	...
	if (ismob(loc))
		...
		visible_message("<span class='danger'>[capitalize(src.name)] explodes!</span>", "<span class='warning'>You hear a loud *pop*!</span>")
