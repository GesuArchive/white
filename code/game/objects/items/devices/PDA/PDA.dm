...
/obj/item/pda/examine(mob/user)
	...
	if(id)
		. += "<hr><span class='notice'>ПКМ to remove the ID.</span>" //won't name ID on examine in case it's stolen
...
/obj/item/pda/proc/update_label()
	name = "PDA-[owner] ([ru_job_parse(ownjob)])" //Name generalisation
...
/obj/item/pda/interact(mob/user)
	...
	if (!owner)
		...
		switch (mode)
			...
			if(6)
				dat += "<h4>[PDAIMG(mail)] ExperTrakВ® Skill Tracker V4.26.2</h4>"
...
/obj/item/pda/proc/send_message(mob/living/user, list/obj/item/pda/targets, everyone)
	...
	// Show it to ghosts
	var/ghost_message = span_name("[owner] </span><span class='game say'>Сообщение PDA</span> --> <span class='name'>[target_text]</span>: <span class='message'>[signal.format_message()]")
...
/obj/item/pda/proc/receive_message(datum/signal/subspace/messaging/pda/signal)
	...
	if (!silent)
		...
		balloon_alert_to_viewers("*[ttone]*", vision_distance = 3)
	...
	if(istype(C, /obj/item/cartridge))
		...
		if(!owner)
			...
			if(!id_check(user, idcard))
				return
			to_chat(user, span_notice("You put the ID into <b>[src.name]</b>'s slot."))
		...
		if(inserted_item)
			to_chat(user, span_warning("There is already \a [inserted_item] in <b>[src.name]</b>!"))
		else
			if(!user.transferItemToLoc(C, src))
				return
			to_chat(user, span_notice("You slide [C] into <b>[src.name]</b>."))
			inserted_item = C
			update_icon()
			playsound(src, 'sound/machines/pda_button1.ogg', 50, TRUE)
		...
		picture = P.picture
		to_chat(user, span_notice("You scan [C]."))
	...
	if(istype(C))
		switch(scanmode)
			...
			if(PDA_SCANNER_MEDICAL)
				C.visible_message(span_notice("[user] analyzes [C] vitals."))
				healthscan(user, C, 1)
				add_fingerprint(user)

			if(PDA_SCANNER_HALOGEN)
				C.visible_message(span_notice("[user] analyzes [C] radiation levels."))
...
/obj/item/pda/proc/explode() //This needs tuning.
	...
	if (ismob(loc))
		...
		visible_message(span_danger("[capitalize(src.name)] explodes!") , span_warning("You hear a loud *pop*!"))
