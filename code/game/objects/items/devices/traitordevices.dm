...
/obj/item/storage/toolbox/emergency/turret/attackby(obj/item/attacking_item, mob/living/user, params)
	if(attacking_item.tool_behaviour == TOOL_WRENCH && user.a_intent == INTENT_HARM && attacking_item.use_tool(src, user, 2 SECONDS, volume = 50))
		user.visible_message(span_danger("[user] бьёт [src] используя [attacking_item]!"), \
			span_danger("Бью [src] используя [attacking_item]!"), null, COMBAT_MESSAGE_RANGE)
...
/obj/machinery/porta_turret/syndicate/toolbox/examine(mob/user)
	...
	if(faction_check(faction, user.faction))
		. += span_notice("<hr>Можно починить это используя <b>ЛКМ</b> и боевой ключ.")
		. += span_notice("\nМожно сложить это <b>ПКМ</b> и боевой ключ.")
...
/obj/machinery/porta_turret/syndicate/toolbox/attackby(obj/item/attacking_item, mob/living/user, params)
	if(istype(attacking_item, /obj/item/wrench/combat))
		if(user.a_intent == INTENT_HARM && attacking_item.toolspeed && attacking_item.use_tool(src, user, 5 SECONDS, volume = 20))
			deconstruct(TRUE)
			attacking_item.play_tool_sound(src, 50)
		else if(!user.a_intent == INTENT_HARM)
			to_chat(user, span_notice("Начинаю чинить [src]..."))
			while(obj_integrity != max_integrity && attacking_item.toolspeed && attacking_item.use_tool(src, user, 2 SECONDS, volume = 20))
				repair_damage(10)
		return
	return ..()
