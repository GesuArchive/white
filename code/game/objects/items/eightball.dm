...
/obj/item/toy/eightball/attack_self(mob/user)
	...
	if(on_cooldown)
		to_chat(user, span_warning("[capitalize(src.name)] was shaken recently, it needs time to settle."))
		return
...
/obj/item/toy/eightball/haunted/attack_ghost(mob/user)
	if(!shaking)
		to_chat(user, span_warning("[capitalize(src.name)] is not currently being shaken."))
		return
	interact(user)
	return ..()
