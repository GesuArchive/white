...
/obj/item/grenade/stingbang/detonate(mob/living/lanced_by)
	if(iscarbon(loc))
		...
		if(B)
			...
			C.visible_message("<b><span class='danger'>[capitalize(src.name)] goes off in [C]'s hand, blowing [C.ru_ego()] [B.name] to bloody shreds!</span></b>", "<span class='userdanger'>[capitalize(src.name)] goes off in your hand, blowing your [B.name] to bloody shreds!</span>")
			B.dismember()
