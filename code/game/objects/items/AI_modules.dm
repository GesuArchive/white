...
/obj/item/ai_module/proc/install(datum/ai_laws/law_datum, mob/user)
	...
	if(law_datum.owner)
		to_chat(user, "<span class='notice'>Загрузка завершена. Законы <b>[law_datum.owner]</b> были модифицированы.</span>")
		law_datum.owner.law_change_counter++
	else
		to_chat(user, "<span class='notice'>Загрузка завершена.</span>")
	...
	if(law_datum.owner)
		deadchat_broadcast("<b> меняет законы <span class='name'>[ainame]</span> в локации [get_area_name(user, TRUE)].</b>", "<span class='name'>[user]</span>" , follow_target=user, message_type=DEADCHAT_LAWCHANGE)
...
/obj/item/ai_module/supplied/safeguard/install(datum/ai_laws/law_datum, mob/user)
	if(!targetName)
		to_chat(user, "<span class='alert'>Ошибка ввода, введите имя.</span>")
		return 0
	..()
...
/obj/item/ai_module/supplied/freeform/attack_self(mob/user)
	...
	if(CHAT_FILTER_CHECK(targName))
		to_chat(user, "<span class='warning'>ОШИБКА! Некорректный текст!</span>") // AI LAW 2 SAY U W U WITHOUT THE SPACES
		return
	laws[1] = targName
	..()
...
/obj/item/ai_module/supplied/freeform/install(datum/ai_laws/law_datum, mob/user)
	if(laws[1] == "")
		to_chat(user, "<span class='alert'>Текст закона не задан.</span>")
		return 0
	..()
...
/obj/item/ai_module/remove/attack_self(mob/user)
	...
	if(lawpos == null)
		return
	if(lawpos <= 0)
		to_chat(user, "<span class='warning'>ОШИБКА! Номер [lawpos] является некорректным.</span>")
		lawpos = 1
		return
	to_chat(user, "<span class='notice'>Выбран закон № [lawpos].</span>")
	..()
...
/obj/item/ai_module/remove/install(datum/ai_laws/law_datum, mob/user)
	if(lawpos > (law_datum.get_law_amount(list(LAW_INHERENT = 1, LAW_SUPPLIED = 1))))
		to_chat(user, "<span class='warning'>Закона с № [lawpos] не существует!</span>")
		return
	..()
...
/obj/item/ai_module/core/freeformcore/attack_self(mob/user)
	...
	if(!targName)
		return
	if(CHAT_FILTER_CHECK(targName))
		to_chat(user, "<span class='warning'>ОШИБКА! Некорректный текст!</span>")
		return
	laws[1] = targName
	..()
