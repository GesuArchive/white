...
/obj/item/ai_module/proc/install(datum/ai_laws/law_datum, mob/user)
	...
	if(law_datum.owner)
		to_chat(user, span_notice("Upload complete. [law_datum.owner] laws have been modified.") )
...
/obj/item/ai_module/supplied/freeform/attack_self(mob/user)
	...
	if(newpos == null)
		return
	if(newpos < 15)
		var/response = tgui_alert(usr, "Error: The law priority of [newpos] is invalid,  Law priorities below 14 are reserved for core laws,  Would you like to change that that to 15?", "Invalid law priority", "Change to 15", "Cancel")
