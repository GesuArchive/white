...
/obj/item/book/granter/spell/recoil(mob/user)
	user.visible_message(span_warning("[capitalize(src.name)] glows in a black light!"))

/obj/item/book/granter/spell/onlearned(mob/user)
	..()
	if(oneuse)
		user.visible_message(span_warning("[capitalize(src.name)] glows dark for a second!"))
...
/obj/item/book/granter/spell/mindswap/recoil(mob/user)
	...
	if(swapper.cast(list(stored_swap), user, TRUE))
		...
		user.visible_message(span_warning("[capitalize(src.name)] fizzles slightly as it stops glowing!")) //if the mind_transfer failed to transfer mobs, likely due to the target being catatonic.
...
/obj/item/book/granter/spell/charge/recoil(mob/user)
	..()
	to_chat(user,span_warning("[capitalize(src.name)] suddenly feels very warm!"))
	empulse(src, 1, 1)
...
/obj/item/book/granter/spell/summonitem/recoil(mob/user)
	..()
	to_chat(user,span_warning("[capitalize(src.name)] suddenly vanishes!"))
	qdel(src)
...
/obj/item/book/granter/martial/cqc/onlearned(mob/living/carbon/user)
	..()
	if(oneuse == TRUE)
		to_chat(user, span_warning("[capitalize(src.name)] beeps ominously..."))

/obj/item/book/granter/martial/cqc/recoil(mob/living/carbon/user)
	to_chat(user, span_warning("[capitalize(src.name)] explodes!"))
...
/obj/item/book/granter/crafting_recipe/on_reading_finished(mob/user)
	...
	for(var/crafting_recipe_type in crafting_recipe_types)
		...
		to_chat(user,span_notice("Могу изготавливать [initial(R.name)]."))
