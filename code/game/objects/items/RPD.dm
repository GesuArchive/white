...
/obj/item/pipe_dispenser/suicide_act(mob/user)
	user.visible_message(span_suicide("[user] points the end of the RPD down [user.ru_ego()] throat and presses a button! It looks like [user.p_theyre()] trying to commit suicide..."))
		...
		switch(category) //if we've gotten this var, the target is valid
			...
			if(DISPOSALS_CATEGORY) //Making disposals pipes
				...
				if(isclosedturf(attack_target))
					to_chat(user, span_warning("[src] error light flickers; there's something in the way!"))
					return
