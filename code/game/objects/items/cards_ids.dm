...
/obj/item/card/suicide_act(mob/living/carbon/user)
	user.visible_message("<span class='suicide'>[user] begins to swipe [user.ru_ego()] neck with <b>[src.name]</b>! It looks like [user.p_theyre()] trying to commit suicide!</span>")
	return BRUTELOSS
...
/obj/item/card/id/proc/mass_insert_money(list/money, mob/user)
	if(!registered_account)
		to_chat(user, "<span class='warning'><b>[capitalize(src.name)]</b> не имеет привязанного аккаунта, чтобы вкладывать туда что-то!</span>")
		return FALSE
...
/obj/item/card/id/examine_more(mob/user)
	...
	if(registered_account)
		...
		msg += "<span class='info'>\nПКМ на ID-карте для снятия денег.</span>"
		msg += "<span class='info'>\nПохоже сюда можно вставлять голо-чипы, монетки и прочую валюту.</span>"
		if(registered_account.civilian_bounty)
			msg += "\n<span class='info'><b>Есть активный гражданский заказ.</b>"
			msg += "<span class='info'>\n<i>[registered_account.bounty_text()]</i></span>"
			msg += "<span class='info'>\nКоличество: [registered_account.bounty_num()]</span>"
			msg += "<span class='info'>\nНаграда: [registered_account.bounty_value()]</span>"
		if(registered_account.account_holder == user.real_name)
			msg += "<span class='boldnotice'>\nЕсли ты потеряешь эту ID-карту, ты можешь запросто переподключить свой счёт используя ПКМ на своей новой карте.</span>"
	else
