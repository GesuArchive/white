...
/obj/item/hot_potato/proc/detonate()
	...
	location.visible_message(span_userdanger("[capitalize(src.name)] [detonate_explosion? "explodes" : "activates"]!") , span_userdanger("[capitalize(src.name)] activates! You've ran out of time!"))
...
/obj/item/hot_potato/proc/force_onto(mob/living/victim, mob/user)
	if(!istype(victim) || user != loc || victim == user)
		return FALSE
	if(!victim.client)
		to_chat(user, span_boldwarning("[capitalize(src.name)] refuses to attach to a non-sapient creature!"))
	if(victim.stat != CONSCIOUS || !victim.usable_legs)
		to_chat(user, span_boldwarning("[capitalize(src.name)] refuses to attach to someone incapable of using it!"))
	...
	if(.)
		...
		user.visible_message(span_boldwarning("[user] tried to force [src] onto [victim], but it could not attach!") , span_boldwarning("Пытаюсь force [src] onto [victim], but it is unable to attach!") , span_boldwarning("You hear a mechanical click and two buzzes."))
