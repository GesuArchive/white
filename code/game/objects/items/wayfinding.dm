...
/obj/machinery/pinpointer_dispenser
	...
	var/list/slogan_list = list(
		"Find a wayfinding pinpointer? Give it to me! I'll make it worth your while. Please. Daddy needs his medicine.", //last sentence is a reference to Sealab 2021
...
/obj/machinery/pinpointer_dispenser/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/pinpointer/wayfinding))
		...
		to_chat(user, span_notice("You put [WP] in the return slot."))
...
/obj/machinery/pinpointer_dispenser/point_at(A)
	. = ..()
	visible_message("<span class='name'>[capitalize(src.name)]</span> points at [A].")
...
/obj/item/pinpointer/wayfinding/attack_self(mob/living/user)
	...
	var/A = tgui_input_list(user, "", "Pinpoint", sort_list(beacons))
