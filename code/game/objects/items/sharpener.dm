...
/obj/item/sharpener/attackby(obj/item/I, mob/user, params)
	...
	if(I.force >= max || I.throwforce >= max) //So the whetstone never reduces force or throw_force
		to_chat(user, "<span class='warning'>[I] уже слишком острый, чтобы точить дальше!</span>")
		return
	if(requires_sharpness && !I.get_sharpness())
		to_chat(user, "<span class='warning'>Могу заточить только уже острые предметы, например, ножи!</span>")
		return
	if(is_type_in_list(I, list(/obj/item/melee/transforming/energy, /obj/item/dualsaber))) //You can't sharpen the photons in energy meelee weapons
		to_chat(user, "<span class='warning'>Не думаю что [I] изменится, если я использую его на <b>[src.name]</b>!</span>")
		return
	...
	if(signal_out & COMPONENT_BLOCK_SHARPEN_BLOCKED)
		to_chat(user, "<span class='warning'> не могу заточить [I] сейчас!</span>")
		return
	if((signal_out & COMPONENT_BLOCK_SHARPEN_ALREADY) || (I.force > initial(I.force) && !signal_out)) //No sharpening stuff twice
		to_chat(user, "<span class='warning'>[I] уже был доработан раньше. Дальше затачивать нельзя!</span>")
		return
	...
	user.visible_message("<span class='notice'>[user] точит [I] на [src]!</span>", "<span class='notice'>Точу [I], делаю его более смертоносным.</span>")
