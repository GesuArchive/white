...
/obj/item/storage/box/gum/wake_up/examine_more(mob/user)
	var/list/msg = list("<span class='notice'><i>Читаю описание жвачки на упаковке...</i></span>")
	msg += "\t["<span class='info'>Для снятия усталости и сонливости во время работы.</span>"]"
	msg += "\t["<span class='info'>Не разжевывайте более одной полоски каждые 12 часов. Не используйте в качестве полной замены сна.</span>"]"
	msg += "\t["<span class='info'>Не давать детям до 16 лет. Не превышайте максимальную дозировку. Не принимать внутрь. Не принимать более 3 дней подряд. Не принимать в сочетании с другими лекарствами. Может вызывать побочные реакции у пациентов с уже существующими заболеваниями сердца.</span>"]"
	msg += "\t["<span class='info'>Побочные эффекты применения Активина могут включать подергивание усиков, гиперактивные крылья, потерю кератинового блеска, потерю покрытия волосков, аритмию, помутнение зрения и эйфорию. При появлении побочных эффектов прием препарата следует прекратить.</span>"]"
	msg += "\t["<span class='info'>Повторное нескольких полосок за раз может вызвать зависимость.</span>"]"
	msg += "\t["<span class='info'>В случае превышения максимальной дозы немедленно сообщите об этом члену медицинского персонала судна. Не вызывайте рвоту.</span>"]"
	msg += "\t["<span class='info'>Состав: каждая полоска содержит 500 мг Активина (декстро-метамфетамин). Другие ингредиенты включают зеленый краситель 450 (Verdant Meadow) и искусственный травяной ароматизатор.</span>"]"
	msg += "\t["<span class='info'>Хранение: хранить в сухом прохладном месте. Не использовать после истечения срока годности: 32/4/350.</span>"]"
	return msg
