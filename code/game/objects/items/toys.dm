...
/obj/item/toy/sword/attackby(obj/item/W, mob/living/user, params)
	if(istype(W, /obj/item/toy/sword))
		if(HAS_TRAIT(W, TRAIT_NODROP) || HAS_TRAIT(src, TRAIT_NODROP))
			to_chat(user, span_warning("\the [HAS_TRAIT(src, TRAIT_NODROP) ? src : W] is stuck to your hand, you can't attach it to [HAS_TRAIT(src, TRAIT_NODROP) ? W : src]!"))
			return
		else
...
/obj/item/toy/cards/suicide_act(mob/living/carbon/user)
	user.visible_message(span_suicide("[user] is slitting [user.ru_ego()] wrists with <b>[src.name]</b>! It looks like [user.ru_who()] [user.p_have()] a crummy hand!"))
...
/obj/item/toy/figure/attack_self(mob/user as mob)
	if(cooldown <= world.time)
		...
		to_chat(user, span_notice("[capitalize(src.name)] says \"[toysay]\""))
