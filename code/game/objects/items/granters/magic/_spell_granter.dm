...
/obj/item/book/granter/action/spell/proc/on_magic_charge(datum/source, datum/action/cooldown/spell/spell, mob/living/caster)
	...
	if(uses >= INFINITY - 2000)
		to_chat(caster, "<span class='notice'>This book is infinite use and can't be recharged, yet the magic has improved it somehow...</span>")
...
/obj/item/book/granter/action/spell/can_learn(mob/living/user)
	...
	if(locate(granted_action) in user.actions)
		if(IS_WIZARD(user))
			to_chat(user, "<span class='warning'>You're already far more versed in the spell [action_name] than this flimsy how-to book can provide!</span>")
		else
