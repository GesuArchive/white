...
/obj/item/pinpointer/attack_self(mob/living/user)
	if(!process_scan) //since it's not scanning on process, it scans here.
		scan_for_target()
	toggle_on()
	user.visible_message(span_notice("[user] [active ? "" : "de"]activates [user.ru_ego()] pinpointer.") , span_notice("You [active ? "" : "de"]activate your pinpointer."))
...
/obj/item/pinpointer/crew/attack_self(mob/living/user)
	if(active)
		toggle_on()
		user.visible_message(span_notice("[user] deactivates [user.ru_ego()] pinpointer.") , span_notice("You deactivate your pinpointer."))
		return
	...
	if(!names.len)
		user.visible_message(span_notice("[user] pinpointer fails to detect a signal.") , span_notice("Your pinpointer fails to detect a signal."))
		return

	var/A = tgui_input_list(user, "Person to track", "Pinpoint", sort_list(names))
	if(!A || QDELETED(src) || !user || !user.is_holding(src) || user.incapacitated())
		return

	target = names[A]
	toggle_on()
	user.visible_message(span_notice("[user] activates [user.ru_ego()] pinpointer.") , span_notice("You activate your pinpointer."))
