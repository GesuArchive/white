...
/obj/item/toy/prize/examine()
	...
	if(in_combat)
		. += "<span class='notice'>\nThis toy has a maximum health of [max_combat_health]. Currently, it's [combat_health].</span>"
		. += "<span class='notice'>\nIts special move light is [special_attack_cooldown? "flashing red." : "green and is ready!"]</span>"
	else
		. += "<span class='notice'>\nThis toy has a maximum health of [max_combat_health].</span>"
