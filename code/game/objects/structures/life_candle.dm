...
/obj/structure/life_candle/attack_hand(mob/user)
	...
	if(user.mind in linked_minds)
		...
		user.visible_message("<span class='notice'>[user] touches [src]. It seems to respond to [user.ru_ego()] presence!</span>", "<span class='warning'>You create a connection between you and [src].</span>")
		linked_minds |= user.mind
