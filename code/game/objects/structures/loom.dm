...
/obj/structure/loom/proc/weave(obj/item/stack/sheet/cotton/W, mob/user)
	...
	if(W.amount < FABRIC_PER_SHEET)
		...
		return FALSE
	user.show_message("<span class='notice'>You start weaving [W.name] through the loom..</span>" , MSG_VISUAL)
	while(W.use_tool(src, user, W.pull_effort) && W.use(FABRIC_PER_SHEET))
		new W.loom_result(drop_location())
		user.show_message("<span class='notice'>You weave [W.name] into a workable fabric.</span>" , MSG_VISUAL)
	return TRUE
