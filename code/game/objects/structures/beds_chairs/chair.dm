...
/obj/structure/chair/MouseDrop(over_object, src_location, over_location)
	...
	if(over_object == usr && Adjacent(usr))
		...
		if(!usr.canUseTopic(src, BE_CLOSE, NO_DEXTERITY, FALSE, TRUE))
			return
		usr.visible_message("<span class='notice'>[usr] хватает [skloname(src.name, VINITELNI, src.gender)].</span>" , "<span class='notice'>Хватаю [skloname(src.name, VINITELNI, src.gender)].</span>")
...
/obj/item/chair/hit_reaction(mob/living/carbon/human/owner, atom/movable/hitby, attack_text = "атаку", final_block_chance = 0, damage = 0, attack_type = MELEE_ATTACK)
	if(attack_type == UNARMED_ATTACK && prob(hit_reaction_chance))
		owner.visible_message("<span class='danger'>[owner] отражает [attack_text] [skloname(src.name, TVORITELNI, src.gender)]!</span>")
		return TRUE
	return FALSE
...
/obj/machinery/painmachine/AltClick(mob/user)
	if(occupant)
		to_chat(user, "<span class='danger'>Не могу переключать режим, когда внутри кто-то есть!</span>")
		return
	if(mode == "стансферы")
		mode = "пончики"
	else
		mode = "стансферы"
	user.visible_message("<span class='notice'>[user] нажимает на переключатель.</span>" , "<span class='notice'>Переключаю режим машины боли.</span>")
	to_chat(user, "<span class='notice'>Теперь будет производить [mode]</span>")
