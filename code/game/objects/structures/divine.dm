...
/obj/structure/sacrificealtar/AltClick(mob/living/user)
	...
	if(!L)
		return
	to_chat(user, "<span class='notice'>Призывая священный ритуал, вы жертвуете [L].</span>")
	L.gib()
...
/obj/structure/healingfountain/attack_hand(mob/living/user)
	...
	if(last_process + time_between_uses > world.time)
		to_chat(user, "<span class='notice'>Фонтан кажется пуст.</span>")
		return
	last_process = world.time
	to_chat(user, "<span class='notice'>Поток теплой и успокаивающей воды струится между вашими пальцами. Вскоре после этого фонтан иссыхает.</span>")
