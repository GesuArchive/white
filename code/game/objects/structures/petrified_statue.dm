...
/obj/structure/statue/petrified/New(loc, mob/living/L, statue_timer)
	...
	if(L)
		...
		L.visible_message(span_warning("[L] skin rapidly turns to marble!") , span_userdanger("Your body freezes up! Can't... move... can't... think..."))
		L.forceMove(src)
...
/obj/structure/statue/petrified/deconstruct(disassembled = TRUE)
	if(!disassembled)
		if(petrified_mob)
			petrified_mob.dust()
	visible_message(span_danger("[capitalize(src.name)] shatters!."))
	qdel(src)
