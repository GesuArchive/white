...
/obj/effect/mob_spawn/human/seed_vault
	...
	short_desc = "Вы - разумная экосистема, пример господства над жизнью, которым обладали создатели."
...
/obj/effect/mob_spawn/human/ash_walker/allow_spawn(mob/user)
	if(!(user.key in team.players_spawned))//one per person unless you get a bonus spawn
		return TRUE
	to_chat(user, span_warning("<b>Исчерпываю свою полезность для Некрополя</b>."))
	return FALSE
...
/obj/effect/mob_spawn/human/exile/special(mob/living/new_spawn)
	...
	switch(wish)
		if(1)
			to_chat(new_spawn, "<b>Хотел убивать, и я это делал. Счёт давно потерян, но искра возбуждения, которая когда-то была в убийстве, угасла. Чувствую только сожаление.</b>")
		if(2)
			to_chat(new_spawn, "<b>Желал бесконечного богатства, но никакие деньги не стоили этого существования. Может быть, благотворительность может выкупить мою душу?</b>")
		if(3)
			to_chat(new_spawn, "<b>Желал власти. Мало пользы от этого, теперь я изгнанный из света, а также [gender == MALE ? "король" : "королева"] пустого царства. Ощущаю только раскаяние.</b>")
		if(4)
			to_chat(new_spawn, "<b>Хотел бессмертия, даже когда мои друзья умирали позади меня. Независимо от того, сколько раз я бросался в лаву, я снова просыпаюсь в этой комнате в течение нескольких дней. Выхода нет.</b>")
...
/obj/effect/mob_spawn/human/golem/attack_hand(mob/user)
	...
	if(isgolem(user) && can_transfer)
		...
		H.visible_message(span_notice("Внезапно [H.name] высвобождает тусклый поток света, который направляется к [src.name] и оживляет её!") ,span_notice("Покидаю старое и перемещаюсь в [src.name]!"))
		show_flavour = FALSE
...
/obj/effect/mob_spawn/human/hermit/Initialize(mapload)
	...
	switch(arrpee)
		if(1)
			flavour_text += "you were a [pick("arms dealer", "shipwright", "docking manager")] assistant on a small trading station several sectors from here. Raiders attacked, and there was \
...
/obj/effect/mob_spawn/human/demonic_friend/Initialize(mapload, datum/mind/owner_mind, obj/effect/proc_holder/spell/targeted/summon_friend/summoning_spell)
	...
	var/area/A = get_area(src)
	if(!mapload && A)
		notify_ghosts("\A friendship shell has been completed in [A.name].", source = src, action=NOTIFY_ATTACK, flashwindow = FALSE)
