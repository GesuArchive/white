...
/obj/structure/training_machine/ui_act(action, params)
	...
	if (moving && obj_flags & EMAGGED)
		visible_message(span_warning("The [src] control panel fizzles slightly."))
		return
	switch(action)
		if("toggle")
			toggle()
...
/obj/structure/training_machine/examine(mob/user)
	...
	if(has_buckled_mob)
		. += span_notice("<b>ПКМ to unbuckle [buckled_mobs[1]]</b>")
	if (obj_flags & EMAGGED)
		...
		. += span_notice("<b>ПКМ to remove [attached_item]</b>")
...
/obj/item/training_toolbox/examine(mob/user)
	...
	. += span_notice("<b>ПКМ to 'Lap' the hit counter.</b>")
