...
/obj/structure/toilet/attack_hand(mob/living/user)
	...
	if(swirlie)
		...
		swirlie.visible_message("<span class='danger'>[user] slams the toilet seat onto [swirlie] head!</span>" , "<span class='userdanger'>[user] slams the toilet seat onto your head!</span>" , "<span class='hear'>You hear reverberating porcelain.</span>")
...
/obj/structure/toilet/attackby(obj/item/I, mob/living/user, params)
	...
	if(I.tool_behaviour == TOOL_CROWBAR)
		...
		if(!open)
			return
		user.visible_message("<span class='notice'><b>[user]</b> элегантно чистит <b>[src.name]</b>.</span>" ,
			"<span class='notice'>Элегантно чищу <b>[src.name]</b>.</span>" , "<span class='hear'>Слышу как что-то ёрзает.</span>")
		return TRUE

	else if (istype(I, /obj/item/kitchen/fork))
		if(!open)
			return
		user.visible_message("<span class='warning'><b>[user]</b> шкрябает <b>парашу</b> вилкой.</span>" ,
			"<span class='userdanger'>Шкрябаю парашу вилкой.</span>" , "<span class='hear'>Слышу как кто-то скребёт вилкой по чему-то твёрдому.</span>")
