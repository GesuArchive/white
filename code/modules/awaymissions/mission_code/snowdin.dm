...
/turf/open/lava/plasma/attackby(obj/item/I, mob/user, params)
	...
	user.visible_message("<span class='notice'>[user] scoops some plasma from the [src] with [C].</span>", "<span class='notice'>You scoop out some plasma from the [src] using [C].</span>")
				...
				if(ishuman(L))//are they a carbon?
					...
					if(prob(35)) //checking if the delay is over & if the victim actually has any parts to nom
						...
						if(plasma_parts.len)
							...
							PP.visible_message("<span class='warning'>[L] screams in pain as [L.ru_ego()] [NB] melts down to the bone!</span>", \
						...
						if(!plasma_parts.len && !robo_parts.len) //a person with no potential organic limbs left AND no robotic limbs, time to turn them into a plasmaman
							...
							PP.visible_message("<span class='warning'>[L] bursts into a brilliant purple flame as [L.ru_ego()] entire body is that of a skeleton!</span>", \
