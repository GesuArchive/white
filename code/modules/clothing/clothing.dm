	...
	if(damaged_clothes && istype(W, repairable_by))
		...
		switch(damaged_clothes)
			...
			if(CLOTHING_SHREDDED)
				if(S.amount < 3)
					to_chat(user, "<span class='warning'>Мне потребуется 3 единицы [S.name] для починки [src].</span>")
					return
				to_chat(user, "<span class='notice'>Начинаю чинить повреждения [src] используя [S]...</span>")
