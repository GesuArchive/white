...
/datum/action/chameleon_outfit/proc/select_outfit(mob/user)
	if(!user || !IsAvailable())
		return FALSE
	var/selected = tgui_input_list(usr, "Select outfit to change into", "Chameleon Outfit", outfit_options)
...
/datum/action/item_action/chameleon/change/proc/select_look(mob/user)
	...
	var/picked_name
	picked_name = tgui_input_list(usr, "Select [chameleon_name] to change into", "Chameleon [chameleon_name]", sort_list(chameleon_list, GLOBAL_PROC_REF(cmp_typepaths_asc)), picked_name)
	if(!picked_name)
		return
...
/obj/item/clothing/shoes/chameleon
	...
	resistance_flags = NONE
	armor = list(MELEE = 10, BULLET = 10, LASER = 10, ENERGY = 0, BOMB = 0, BIO = 90, RAD = 0, FIRE = 50, ACID = 50)
