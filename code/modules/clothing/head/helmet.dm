...
/obj/item/clothing/head/helmet/proc/rolldown()
	if(ishuman(usr))
		if(flags_inv == HIDEHAIR)
			to_chat(usr, "<span class='notice'>Распускаю волосы.</span>")
			flags_inv = null
		else
			to_chat(usr, "<span class='notice'>Заправляю волосы под шлем.</span>")
...
/obj/item/clothing/head/helmet/monkey_sentience/examine(mob/user)
	...
	. += "<span class='warning'>\nЖАЖДЕ КРОВИ</span>"
	. += "<span class='warning'>\nСМЕРТИ МОЗГА</span>"
	. += "<span class='warning'>\nАКТИВАЦИИ ПЕРВОБЫТНОГО ГЕНА</span>"
	. += "<span class='warning'>\nМАССОВОЙ УСТОЙЧИВОСТИ ГЕНЕТИЧЕСКОГО СОСТАВА</span>"
	. += "<span class='boldnotice'>\nПеред применением устройства проконсультируйтесь с вашим главным врачом.</span>"
...
/obj/item/clothing/head/helmet/proc/toggle_helmlight()
	...
	to_chat(user, "<span class='notice'>[attached_light.on ? "Включаю":"Выключаю"] фонарик на шлеме.</span>")
