...
/obj/machinery/atmospherics/components/trinary/filter/AltClick(mob/user)
	if(can_interact(user))
		...
		to_chat(user, "<span class='notice'>Выкручиваю мощность [src] на [transfer_rate] Л/с.</span>")
		update_icon()
	return ..()
...
/obj/machinery/atmospherics/components/trinary/filter/process_atmos(delta_time)
	...
	if(filter_type && air2.return_pressure() <= 900000)
		air1.scrub_into(air2, transfer_ratio, list(filter_type))
	if(air3.return_pressure() <= 900000)
