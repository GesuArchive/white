...
/obj/machinery/atmospherics/components/unary/hypertorus/core/process_atmos()
	...
	for(var/gas_type in selected_fuel.requirements)
		internal_fusion.merge(linked_input.airs[1].remove_specific(gas_type, linked_input.airs[1].get_moles(gas_type) * fuel_injection_rate * 0.1))
...
//7 reworks
	...
	for(var/gas_id in selected_fuel.requirements)
		fuel_list |= gas_id
		fuel_list[gas_id] = internal_fusion.get_moles(gas_id)
	...
	for(var/gas_id in moderator_internal.get_gases())
		...
		moderator_list[gas_id] = moderator_internal.get_moles(gas_id)
	...
	if(check_fuel())
		...
		for(var/gas_id in selected_fuel.requirements)
			internal_fusion.adjust_moles(gas_id, -min(fuel_list[gas_id], fuel_consumption * 0.85 / selected_fuel.fuel_consumption_multiplier))
		for(var/gas_id in selected_fuel.primary_products)
		...
		if(power_output)
			switch(power_level)
				if(1)
					...
					moderator_internal.adjust_moles(selected_fuel.secondary_products[1], scaled_production * 0.95)
					moderator_internal.adjust_moles(selected_fuel.secondary_products[2], scaled_production *0.75)
				...
				if(2)
					...
					moderator_internal.adjust_moles(selected_fuel.secondary_products[1], scaled_production * 1.65)
					moderator_internal.adjust_moles(selected_fuel.secondary_products[2], scaled_production)
					if(moderator_list[/datum/gas/plasma] > 50)
						internal_output.adjust_moles(/datum/gas/bz, scaled_production * 1.8)
						moderator_internal.adjust_moles(selected_fuel.secondary_products[3], scaled_production * 1.15)
				...
				if(3, 4)
					...
					if(power_level == 3)
						moderator_internal.adjust_moles(selected_fuel.secondary_products[2], scaled_production * 0.5)
						moderator_internal.adjust_moles(selected_fuel.secondary_products[3], scaled_production * 0.45)
					if(power_level == 4)
						moderator_internal.adjust_moles(selected_fuel.secondary_products[3], scaled_production * 1.65)
						moderator_internal.adjust_moles(selected_fuel.secondary_products[4], scaled_production * 1.25)
				...
				if(5)
					...
					moderator_internal.adjust_moles(selected_fuel.secondary_products[4], scaled_production * 0.65)
					moderator_internal.adjust_moles(selected_fuel.secondary_products[5], scaled_production)
					moderator_internal.adjust_moles(selected_fuel.secondary_products[6], scaled_production * 0.75)
					if(moderator_list[/datum/gas/plasma] > 15)
						internal_output.get_moles(/datum/gas/freon, scaled_production *0.25)
						moderator_internal.get_moles(/datum/gas/plasma, -min(moderator_internal.get_moles(/datum/gas/plasma), scaled_production * 1.45))
					...
					if(moderator_internal.return_temperature() < 1e7 || (moderator_list[/datum/gas/plasma] > 100 && moderator_list[/datum/gas/bz] > 50))
						internal_output.adjust_moles(/datum/gas/antinoblium, 0.9 * (scaled_fuel_list[scaled_fuel_list[3]] / (fuel_injection_rate * 0.0065)) * delta_time)
				if(6)
					var/scaled_production = clamp(heat_output * 1e-7, 0, fuel_consumption_rate) * delta_time
					moderator_internal.adjust_moles(selected_fuel.secondary_products[5], scaled_production * 0.35)
					moderator_internal.adjust_moles(selected_fuel.secondary_products[6], scaled_production)
					...
					if(moderator_list[/datum/gas/bz])
						...
						moderator_internal.adjust_moles(/datum/gas/antinoblium, clamp((scaled_fuel_list[scaled_fuel_list[3]] / (fuel_injection_rate * 0.0045)), 0, 10) * delta_time)
					if(moderator_list[/datum/gas/healium] > 100)
						if(critical_threshold_proximity > 400)
							critical_threshold_proximity = max(critical_threshold_proximity - (moderator_list[/datum/gas/healium] / 100 * delta_time ), 0)
							moderator_internal.adjust_moles(/datum/gas/healium, -min(moderator_internal.get_moles(/datum/gas/healium), scaled_production * 20))
					internal_fusion.adjust_moles(/datum/gas/antinoblium, 0.01 * (scaled_fuel_list[scaled_fuel_list[3]] / (fuel_injection_rate * 0.0095)) * delta_time)
	...
	if(moderator_list[/datum/gas/oxygen] > 150)
		if(iron_content > 0)
			...
			moderator_internal.adjust_moles(/datum/gas/oxygen, 10)
	...
	if(waste_remove && power_level <= 5)
		...
		if(internal_fusion.get_moles(/datum/gas/antinoblium) > 0)
			internal_remove = internal_fusion.remove_specific(/datum/gas/antinoblium, internal_fusion.get_moles(/datum/gas/antinoblium) * (1 - (1 - 0.25) ** delta_time))
			linked_output.airs[1].merge(internal_remove)
