...
/datum/pipeline/proc/temperature_interact(turf/target, share_volume, thermal_conductivity)
	...
	var/turf_temperature = target.return_temperature()
	var/turf_heat_capacity = target.heat_capacity
	...
	target.temperature += -1 * heat / turf_heat_capacity
	target.air_update_turf(FALSE, FALSE)
