...
/datum/pipeline/proc/addMachineryMember(obj/machinery/atmospherics/components/C)
	...
	if(!G)
		log_runtime("addMachineryMember: Null gasmix added to pipeline datum from [C] which is of type [C.type]. Nearby: ([C.x], [C.y], [C.z])")
		return
	other_airs |= G
...
/datum/pipeline/proc/reconcile_air()
	...
	for(var/mixture in gas_mixture_list)
		...
		for(var/giver_id in giver_gases)
			...
			total_heat_capacity += giver_gas_data[MOLES] * giver_gas_data[GAS_META]
