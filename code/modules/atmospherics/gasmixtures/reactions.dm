...
/datum/gas_reaction/nitrylformation/react(datum/gas_mixture/air)
	...
	if ((air.get_moles(/datum/gas/oxygen) - heat_efficency < 0 ) || (air.get_moles(/datum/gas/nitrogen) - heat_efficency < 0) || (air.get_moles(/datum/gas/bz) - heat_efficency < 0)) //Shouldn't produce gas from nothing.
		return NO_REACTION
...
/datum/gas_reaction/metalhydrogen/react(datum/gas_mixture/air, datum/holder)
	...
	if(pressure >= METAL_HYDROGEN_MINIMUM_PRESSURE && temperature >= METAL_HYDROGEN_MINIMUM_HEAT)
		...
		if (prob(20 * increase_factor))
			air.adjust_moles(/datum/gas/hydrogen, -heat_efficency * 10)
...
/datum/gas_reaction/freonformation/react(datum/gas_mixture/air)
	...
	if ((air.get_moles(/datum/gas/plasma) - heat_efficency < 0 ) || (air.get_moles(/datum/gas/carbon_dioxide) - heat_efficency < 0) || (air.get_moles(/datum/gas/bz) - heat_efficency < 0)) //Shouldn't produce gas from nothing.
		return NO_REACTION
...
/datum/gas_reaction/stim_ball/react(datum/gas_mixture/air, datum/holder)
	...
	air.adjust_moles(/datum/gas/plasma, air.get_moles(/datum/gas/plasma) * 0.5) //Consumes half the plasma each time.
	if(energy_released)
		var/new_heat_capacity = air.heat_capacity()
		if(new_heat_capacity > MINIMUM_HEAT_CAPACITY)
