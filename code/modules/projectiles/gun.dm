...
/obj/item/gun
	...
	var/automatic = 1 //can gun use it, 0 is no, anything above 0 is the delay between clicks in ds
	var/pb_knockback = 0
	...
	if(user.a_intent == INTENT_HARM)
		...
		if(!gun_light)
			if(!user.transferItemToLoc(I, src))
				return
			to_chat(user, "<span class='notice'>Защёлкиваю [S] прямо в слот [src].</span>")
			set_gun_light(S)
			update_gunlight()
			alight = new(src)
			if(loc == user)
