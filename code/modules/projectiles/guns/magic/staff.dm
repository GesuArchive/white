...
/obj/item/gun/magic/staff/change/pickup(mob/user)
	. = ..()
	if(!is_wizard_or_friend(user))
		to_chat(user, "<span class='hypnophrase'><span style='font-size: 24px'>You don't feel strong enough to properly wield this staff!</span></span>")
...
/obj/item/gun/magic/staff/healing/pickup(mob/user)
	. = ..()
	if(!is_wizard_or_friend(user))
		to_chat(user, "<span class='hypnophrase'><span style='font-size: 24px'>The staff feels weaker as you touch it</span></span>")
