...
/obj/item/gun/syringe/attackby(obj/item/A, mob/user, params, show_msg = TRUE)
	if(istype(A, /obj/item/reagent_containers/syringe/bluespace))
		to_chat(user, "<span class='notice'>[A] слишком большой и не помещается в [src].</span>")
		return TRUE
	if(istype(A, /obj/item/reagent_containers/syringe))
		if(syringes.len < max_syringes)
			if(!user.transferItemToLoc(A, src))
				return FALSE
			to_chat(user, "<span class='notice'>Зарядил [A] в <b>[src.name]</b>.</span>")
			syringes += A
			recharge_newshot()
			update_icon()
			...
			to_chat(user, "<span class='warning'>В [capitalize(src.name)] не поместятся еще шприцы!</span>")
	return FALSE
...
/obj/item/gun/syringe/dna/attackby(obj/item/A, mob/user, params, show_msg = TRUE)
	if(istype(A, /obj/item/dnainjector))
		...
		if(D.used)
			to_chat(user, "<span class='warning'>Данный инжектор израсходован!</span>")
			return
		if(syringes.len < max_syringes)
			if(!user.transferItemToLoc(D, src))
				return FALSE
			to_chat(user, "<span class='notice'>Зарядил [D] в <b>[src.name]</b>.</span>")
			syringes += D
			recharge_newshot()
			return TRUE
		else
			to_chat(user, "<span class='warning'>[capitalize(src.name)] не вместит больше шприцов!</span>")
	return FALSE
...
/obj/item/gun/syringe/blowgun/process_fire(atom/target, mob/living/user, message = TRUE, params = null, zone_override = "", bonus_spread = 0)
	visible_message("<span class='danger'>[user] прицеливается своим blowgun!</span>")
