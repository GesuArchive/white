...
/obj/item/gun/chem/process_chamber(empty_chamber = TRUE, from_firing = TRUE, chamber_next_round = TRUE, atom/shooter = null)
...
/obj/item/gun/chem/process()
	...
	if(world.time < last_synth+time_per_syringe)
		return
	to_chat(loc, span_warning("Слышу щелчок с которым [src] синтезировал новый дротик."))
	syringes_left++
