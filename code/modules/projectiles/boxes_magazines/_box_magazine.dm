...
/obj/item/ammo_box/proc/add_notes_box()
	...
	if(caliber && max_ammo) // Text references a 'magazine' as only magazines generally have the caliber variable initialized
		readout += "До ["<span class='warning'>[max_ammo] патронов калибра [caliber]</span>"] может быть найдено в магазине. \
		\nСлучайный разряд любого из этих снарядов может привести к аннулированию страхового договора."
...
/obj/item/ammo_box/proc/top_off(load_type, starting=FALSE)
	...
	if(!starting && (caliber && initial(round_check.caliber) != caliber && round_check.caliber != CALIBER_UNIVERSAL) || (!caliber && load_type != ammo_type && round_check.caliber != CALIBER_UNIVERSAL))
...
/obj/item/ammo_box/proc/give_round(obj/item/ammo_casing/R, replace_spent = 0)
	...
	if(!R || (caliber && R.caliber != caliber && R.caliber != CALIBER_UNIVERSAL) || (!caliber && R.type != ammo_type && R.caliber != CALIBER_UNIVERSAL))
		return FALSE
...
/obj/item/ammo_box/attackby(obj/item/A, mob/user, params, silent = FALSE, replace_spent = 0)
	...
	if(num_loaded)
		if(!silent)
			to_chat(user, "<span class='notice'>You load [num_loaded] shell\s into <b>[src.name]</b>!</span>")
