...
/obj/machinery/nanite_programmer/update_overlays()
	...
	SSvis_overlays.add_vis_overlay(src, icon, "nanite_programmer_on", plane = EMISSIVE_PLANE)
...
/obj/machinery/nanite_programmer/ui_act(action, params)
	...
	switch(action)
		...
		if("toggle_active")
			...
			investigate_log("[key_name(usr)] edited [program.name] initial activation status into [program.activated ? "Activated" : "Deactivated"]", INVESTIGATE_NANITES)
			. = TRUE
		if("set_code")
			...
			switch(target_code)
				if("activation")
					...
					investigate_log("[key_name(usr)] edited [program.name] activation code into [program.activation_code]", INVESTIGATE_NANITES)
				if("deactivation")
					program.deactivation_code = clamp(round(new_code, 1),0,9999)
					investigate_log("[key_name(usr)] edited [program.name] deactivation code into [program.deactivation_code]", INVESTIGATE_NANITES)
				if("kill")
					program.kill_code = clamp(round(new_code, 1),0,9999)
					investigate_log("[key_name(usr)] edited [program.name] kill code into [program.kill_code]", INVESTIGATE_NANITES)
				if("trigger")
					program.trigger_code = clamp(round(new_code, 1),0,9999)
					investigate_log("[key_name(usr)] edited [program.name] trigger code into [program.trigger_code]", INVESTIGATE_NANITES)
			. = TRUE
		if("set_extra_setting")
			program.set_extra_setting(params["target_setting"], params["value"])
			investigate_log("[key_name(usr)] edited [program.name] extra setting '[params["target_setting"]]' into [params["value"]]", INVESTIGATE_NANITES)
			playsound(src, "terminal_type", 25, FALSE)
			. = TRUE
		if("set_restart_timer")
			...
			if(!isnull(timer))
				...
				investigate_log("[key_name(usr)] edited [program.name] restart timer into [timer/10] s", INVESTIGATE_NANITES)
			. = TRUE
		if("set_shutdown_timer")
			...
			if(!isnull(timer))
				...
				investigate_log("[key_name(usr)] edited [program.name] shutdown timer into [timer/10] s", INVESTIGATE_NANITES)
			. = TRUE
		if("set_trigger_timer")
			...
			if(!isnull(timer))
				...
				investigate_log("[key_name(usr)] edited [program.name] trigger timer into [timer/10] s", INVESTIGATE_NANITES)
			. = TRUE
