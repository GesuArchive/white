...
/obj/item/slimecross/regenerative/afterattack(atom/target,mob/user,prox)
	...
	if(H != user)
		...
		user.visible_message(span_notice("[user] crushes [src] over [user.ru_na()]self, the milky goo quickly regenerating all of [user.ru_ego()] injuries!") ,
...
/obj/item/slimecross/regenerative/darkblue/core_effect(mob/living/target, mob/user)
	...
	if(H.get_item_by_slot(ITEM_SLOT_HEAD))
		...
		fireproof(C)
	if(fireproofed)
		target.visible_message(span_notice("Some of [target] clothing gets coated in the goo, and turns blue!"))
...
/obj/item/slimecross/regenerative/sepia/core_effect_before(mob/living/target, mob/user)
	to_chat(target, "<span class=notice>Пытаюсь forget how you feel.</span>")
...
/obj/item/slimecross/regenerative/pyrite/core_effect(mob/living/target, mob/user)
	target.visible_message(span_warning("The milky goo coating [target] leaves [target.ru_na()] a different color!"))
