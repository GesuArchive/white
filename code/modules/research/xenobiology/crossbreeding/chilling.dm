...
/obj/item/slimecross/chilling/grey/do_effect(mob/user)
	user.visible_message("<span class='notice'>[capitalize(src.name)] produces a few small, grey cubes</span>")
	for(var/i in 1 to 3)
...
/obj/item/slimecross/chilling/orange/do_effect(mob/user)
	user.visible_message("<span class='danger'>[capitalize(src.name)] shatters, and lets out a jet of heat!</span>")
...
/obj/item/slimecross/chilling/purple/do_effect(mob/user)
	...
	if(A.outdoors)
		to_chat(user, "<span class='warning'>[capitalize(src.name)] can't affect such a large area.</span>")
		return
	user.visible_message("<span class='notice'>[capitalize(src.name)] shatters, and a healing aura fills the room briefly.</span>")
...
/obj/item/slimecross/chilling/blue/do_effect(mob/user)
	user.visible_message("<span class='notice'>[capitalize(src.name)] cracks, and spills out a liquid goo, which reforms into a mask!</span>")
...
/obj/item/slimecross/chilling/metal/do_effect(mob/user)
	user.visible_message("<span class='danger'>[capitalize(src.name)] melts like quicksilver, and surrounds [user] in a wall!</span>")
...
/obj/item/slimecross/chilling/yellow/do_effect(mob/user)
	var/area/A = get_area(get_turf(user))
	user.visible_message("<span class='notice'>[capitalize(src.name)] shatters, and a the air suddenly feels charged for a moment.</span>")
...
/obj/item/slimecross/chilling/darkpurple/do_effect(mob/user)
	...
	if(filtered)
		...
		user.visible_message("<span class='notice'>[capitalize(src.name)] cracks, but nothing happens.</span>")
	..()
...
/obj/item/slimecross/chilling/silver/do_effect(mob/user)
	user.visible_message("<span class='notice'>[capitalize(src.name)] crumbles into icy powder, leaving behind several emergency food supplies!</span>")
...
/obj/item/slimecross/chilling/bluespace/do_effect(mob/user)
	if(allies.len <= 0)
		to_chat(user, "<span class='warning'>[capitalize(src.name)] is not linked to anyone!</span>")
		return
	...
	if(do_after(user, 100, target=src))
		to_chat(user, "<span class='notice'>[capitalize(src.name)] shatters as it tears a hole in reality, snatching the linked individuals from the void!</span>")
		for(var/mob/living/M in allies)
			var/datum/status_effect/slimerecall/S = M.has_status_effect(/datum/status_effect/slimerecall)
			M.remove_status_effect(S)
	else
		to_chat(user, "<span class='warning'>[capitalize(src.name)] falls dark, dissolving into nothing as the energies fade away.</span>")
		for(var/mob/living/M in allies)
			var/datum/status_effect/slimerecall/S = M.has_status_effect(/datum/status_effect/slimerecall)
			if(istype(S))
				S.interrupted = TRUE
...
/obj/item/slimecross/chilling/sepia/do_effect(mob/user)
	user.visible_message("<span class='warning'>[capitalize(src.name)] shatters, freezing time itself!</span>")
...
/obj/item/slimecross/chilling/cerulean/do_effect(mob/user)
	if(isliving(user))
		user.visible_message("<span class='warning'>[capitalize(src.name)] creaks and shifts into a clone of [user]!</span>")
		var/mob/living/M = user
...
/obj/item/slimecross/chilling/pyrite/do_effect(mob/user)
	user.visible_message("<span class='notice'>[capitalize(src.name)] crystallizes into a pair of spectacles!</span>")
...
/obj/item/slimecross/chilling/red/do_effect(mob/user)
	...
	for(var/mob/living/simple_animal/slime/S in view(get_turf(user), 7))
		...
		S.docile = TRUE
	if(slimesfound)
		user.visible_message("<span class='notice'>[capitalize(src.name)] lets out a peaceful ring as it shatters, and nearby slimes seem calm.</span>")
	else
		user.visible_message("<span class='notice'>[capitalize(src.name)] lets out a peaceful ring as it shatters, but nothing happens...</span>")
	return ..()
...
/obj/item/slimecross/chilling/pink/do_effect(mob/user)
	user.visible_message("<span class='notice'>[capitalize(src.name)] cracks like an egg, and an adorable puppy comes tumbling out!</span>")
...
/obj/item/slimecross/chilling/gold/do_effect(mob/user)
	user.visible_message("<span class='notice'>[capitalize(src.name)] lets off golden light as it melts and reforms into an egg-like device!</span>")
...
/obj/item/slimecross/chilling/oil/do_effect(mob/user)
	user.visible_message("<span class='danger'>[capitalize(src.name)] begins to shake with muted intensity!</span>")
...
/obj/item/slimecross/chilling/lightpink/do_effect(mob/user)
	user.visible_message("<span class='notice'>[capitalize(src.name)] blooms into a beautiful flower!</span>")
...
/obj/item/slimecross/chilling/adamantine/do_effect(mob/user)
	user.visible_message("<span class='notice'>[capitalize(src.name)] creaks and breaks as it shifts into a heavy set of armor!</span>")
...
/obj/item/slimecross/chilling/rainbow/do_effect(mob/user)
	...
	if(area.outdoors)
		to_chat(user, "<span class='warning'>[capitalize(src.name)] can't affect such a large area.</span>")
		return
	user.visible_message("<span class='warning'>[capitalize(src.name)] reflects an array of dazzling colors and light, energy rushing to nearby doors!</span>")
