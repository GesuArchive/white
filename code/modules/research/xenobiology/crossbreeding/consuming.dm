...
/obj/item/slimecross/consuming/attackby(obj/item/O, mob/user)
	if(IS_EDIBLE(O))
		if(last_produced + cooldown > world.time)
			to_chat(user, span_warning("[capitalize(src.name)] is still digesting after its last meal!"))
			return
		var/datum/reagent/N = O.reagents.has_reagent(/datum/reagent/consumable/nutriment)
		if(N)
			nutriment_eaten += N.volume
			to_chat(user, span_notice("[capitalize(src.name)] opens up and swallows [O] whole!"))
			qdel(O)
			playsound(src, 'sound/items/eatfood.ogg', 20, TRUE)
		else
			to_chat(user, span_warning("[capitalize(src.name)] burbles unhappily at the offering."))
		if(nutriment_eaten >= nutriment_required)
			nutriment_eaten = 0
			user.visible_message(span_notice("[capitalize(src.name)] swells up and produces a small pile of cookies!"))
