...
/obj/machinery/rnd/experimentor/Topic(href, href_list)
	...
	if(href_list["close"])
		...
		if(recentlyExperimented)
			to_chat(usr, "<span class='warning'>[capitalize(src.name)] has been used too recently!</span>")
		else if(!loaded_item)
			to_chat(usr, "<span class='warning'>[capitalize(src.name)] is not currently loaded!</span>")
...
/obj/machinery/rnd/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_POKE)
		visible_message("<span class='notice'>[capitalize(src.name)] prods at [exp_on] with mechanical arms.</span>")
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			visible_message("<span class='notice'>[exp_on] is gripped in just the right way, enhancing its focus.</span>")
			badThingCoeff++
		else if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
			visible_message("<span class='danger'>[capitalize(src.name)] malfunctions and destroys [exp_on], lashing its arms out at nearby people!</span>")
			...
			visible_message("<span class='warning'>[capitalize(src.name)] malfunctions!</span>")
			exp = SCANTYPE_OBLITERATE
		else if(prob(EFFECT_PROB_MEDIUM-badThingCoeff))
			visible_message("<span class='danger'>[capitalize(src.name)] malfunctions, throwing the [exp_on]!</span>")
	...
	if(exp == SCANTYPE_IRRADIATE)
		visible_message("<span class='danger'>[capitalize(src.name)] reflects radioactive rays at [exp_on]!</span>")
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			visible_message("<span class='danger'>[capitalize(src.name)] malfunctions, melting [exp_on] and leaking radiation!</span>")
			radiation_pulse(src, 500)
			ejectItem(TRUE)
		else if(prob(EFFECT_PROB_LOW-badThingCoeff))
			visible_message("<span class='warning'>[capitalize(src.name)] malfunctions, spewing toxic waste!</span>")
			...
			visible_message("<span class='warning'>[capitalize(src.name)] malfunctions, transforming [savedName] into [loaded_item]!</span>")
	...
	////////////////////////////////////////////////////////////////////////////////////////////////
	if(exp == SCANTYPE_GAS)
		visible_message("<span class='warning'>[capitalize(src.name)] fills its chamber with gas, [exp_on] included.</span>")
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			visible_message("<span class='danger'>[capitalize(src.name)] destroys [exp_on], leaking dangerous gas!</span>")
			...
			visible_message("<span class='danger'>[capitalize(src.name)]'s chemical chamber has sprung a leak!</span>")
			...
			visible_message("<span class='warning'>[capitalize(src.name)] malfunctions, spewing harmless gas.</span>")
			throwSmoke(loc)
		else if(prob(EFFECT_PROB_MEDIUM-badThingCoeff))
			visible_message("<span class='warning'>[capitalize(src.name)] melts [exp_on], ionizing the air around it!</span>")
			empulse(loc, 4, 6)
			investigate_log("Experimentor has generated an Electromagnetic Pulse.", INVESTIGATE_EXPERIMENTOR)
			ejectItem(TRUE)
	////////////////////////////////////////////////////////////////////////////////////////////////
	if(exp == SCANTYPE_HEAT)
		visible_message("<span class='notice'>[capitalize(src.name)] raises [exp_on]'s temperature.</span>")
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			visible_message("<span class='warning'>[capitalize(src.name)]'s emergency coolant system gives off a small ding!</span>")
			...
			if(MT)
				visible_message("<span class='danger'>[capitalize(src.name)] dangerously overheats, launching a flaming fuel orb!</span>")
			...
			visible_message("<span class='danger'>[capitalize(src.name)] malfunctions, melting [exp_on] and releasing a burst of flame!</span>")
			explosion(loc, -1, 0, 0, 0, 0, flame_range = 2)
			investigate_log("Experimentor started a fire.", INVESTIGATE_EXPERIMENTOR)
			ejectItem(TRUE)
		else if(prob(EFFECT_PROB_MEDIUM-badThingCoeff))
			visible_message("<span class='warning'>[capitalize(src.name)] malfunctions, melting [exp_on] and leaking hot air!</span>")
			...
			visible_message("<span class='warning'>[capitalize(src.name)] malfunctions, activating its emergency coolant systems!</span>")
			throwSmoke(loc)
			for(var/mob/living/m in oview(1, src))
	...
	if(exp == SCANTYPE_COLD)
		visible_message("<span class='notice'>[capitalize(src.name)] lowers [exp_on]'s temperature.</span>")
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			visible_message("<span class='warning'>[capitalize(src.name)]'s emergency coolant system gives off a small ding!</span>")
			var/obj/item/reagent_containers/food/drinks/coffee/C = new /obj/item/reagent_containers/food/drinks/coffee(get_turf(pick(oview(1,src))))
			...
			visible_message("<span class='danger'>[capitalize(src.name)] malfunctions, shattering [exp_on] and releasing a dangerous cloud of coolant!</span>")
			var/datum/reagents/R = new/datum/reagents(50)
			R.my_atom = src
			...
			visible_message("<span class='warning'>[capitalize(src.name)] malfunctions, shattering [exp_on] and leaking cold air!</span>")
			var/datum/gas_mixture/env = loc.return_air()
			var/transfer_moles = 0.25 * env.total_moles()
			var/datum/gas_mixture/removed = env.remove(transfer_moles)
			if(removed)
			...
			visible_message("<span class='warning'>[capitalize(src.name)] malfunctions, releasing a flurry of chilly air as [exp_on] pops out!</span>")
			var/datum/effect_system/smoke_spread/smoke = new
			smoke.set_up(0, loc)
			smoke.start()
			ejectItem()
	...
	if(exp == SCANTYPE_OBLITERATE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			visible_message("<span class='warning'>[capitalize(src.name)]'s crushing mechanism slowly and smoothly descends, flattening the [exp_on]!</span>")
			new /obj/item/stack/sheet/plasteel(get_turf(pick(oview(1,src))))
		else if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
			visible_message("<span class='danger'>[capitalize(src.name)]'s crusher goes way too many levels too high, crushing right through space-time!</span>")
			...
			visible_message("<span class='danger'>[capitalize(src.name)]'s crusher goes one level too high, crushing right into space-time!</span>")
			playsound(src, 'sound/effects/supermatter.ogg', 50, TRUE, -3)
	...
	if(exp == SCANTYPE_DISCOVER)
		visible_message("<span class='notice'>[capitalize(src.name)] scans the [exp_on], revealing its true nature!</span>")
	...
	if(prob(EFFECT_PROB_VERYLOW-badThingCoeff) && loaded_item)
		...
		if(globalMalf < 15)
			visible_message("<span class='warning'>[capitalize(src.name)]'s onboard detection system has malfunctioned!</span>")
		...
		if(globalMalf > 16 && globalMalf < 35)
			visible_message("<span class='warning'>[capitalize(src.name)] melts [exp_on], ian-izing the air around it!</span>")
			throwSmoke(loc)
			if(trackedIan)
		...
		if(globalMalf > 51 && globalMalf < 75)
			visible_message("<span class='warning'>[capitalize(src.name)] encounters a run-time error!</span>")
			throwSmoke(loc)
			if(trackedRuntime)
		...
		if(globalMalf > 76 && globalMalf < 98)
			visible_message("<span class='warning'>[capitalize(src.name)] begins to smoke and hiss, shaking violently!</span>")
			use_power(500000)
			investigate_log("Experimentor has drained power from its APC", INVESTIGATE_EXPERIMENTOR)
		if(globalMalf == 99)
			visible_message("<span class='warning'>[capitalize(src.name)] begins to glow and vibrate. It's going to blow!</span>")
...
/obj/item/relic/attack_self(mob/user)
	if(revealed)
		if(cooldown)
			to_chat(user, "<span class='warning'>[capitalize(src.name)] does not react!</span>")
			return
		else if(loc == user)
			cooldown = TRUE
...
/obj/item/relic/proc/petSpray(mob/user)
	var/message = "<span class='danger'>[capitalize(src.name)] begins to shake, and in the distance the sound of rampaging animals arises!</span>"
	...
	if(prob(60))
		to_chat(user, "<span class='warning'>[capitalize(src.name)] falls apart!</span>")
		qdel(src)
...
/obj/item/relic/proc/explode(mob/user)
	to_chat(user, "<span class='danger'>[capitalize(src.name)] begins to heat up!</span>")
...
/obj/item/relic/proc/teleport(mob/user)
	to_chat(user, "<span class='notice'>[capitalize(src.name)] begins to vibrate!</span>")
...
/obj/item/relic/proc/do_the_teleport(mob/user)
	...
	if(loc == user && !is_centcom_level(userturf.z)) //Because Nuke Ops bringing this back on their shuttle, then looting the ERT area is 2fun4you!
		visible_message("<span class='notice'>[capitalize(src.name)] twists and bends, relocating itself!</span>")
		throwSmoke(userturf)
