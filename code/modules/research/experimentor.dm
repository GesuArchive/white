...
/obj/machinery/rnd/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_GAS)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			visible_message("<span class='danger'>[capitalize(src.name)] chemical chamber has sprung a leak!</span>")
	...
	if(exp == SCANTYPE_HEAT)
		visible_message("<span class='notice'>[capitalize(src.name)] raises [exp_on] temperature.</span>")
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			visible_message("<span class='warning'>[capitalize(src.name)] emergency coolant system gives off a small ding!</span>")
	...
	if(exp == SCANTYPE_COLD)
		visible_message("<span class='notice'>[capitalize(src.name)] lowers [exp_on] temperature.</span>")
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			visible_message("<span class='warning'>[capitalize(src.name)] emergency coolant system gives off a small ding!</span>")
			var/obj/item/reagent_containers/food/drinks/coffee/C = new /obj/item/reagent_containers/food/drinks/coffee(get_turf(pick(oview(1,src))))
	...
	if(exp == SCANTYPE_OBLITERATE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			visible_message("<span class='warning'>[capitalize(src.name)] crushing mechanism slowly and smoothly descends, flattening the [exp_on]!</span>")
			new /obj/item/stack/sheet/plasteel(get_turf(pick(oview(1,src))))
		else if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
			visible_message("<span class='danger'>[capitalize(src.name)] crusher goes way too many levels too high, crushing right through space-time!</span>")
			...
			visible_message("<span class='danger'>[capitalize(src.name)] crusher goes one level too high, crushing right into space-time!</span>")
			playsound(src, 'sound/effects/supermatter.ogg', 50, TRUE, -3)
	...
	if(prob(EFFECT_PROB_VERYLOW-badThingCoeff) && loaded_item)
		...
		if(globalMalf < 15)
			visible_message("<span class='warning'>[capitalize(src.name)] onboard detection system has malfunctioned!</span>")
