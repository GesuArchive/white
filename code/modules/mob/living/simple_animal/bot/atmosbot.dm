...
/mob/living/simple_animal/bot/atmosbot
	name = "Атмосбот"
	desc = "Маленький робот, который рад видеть тебя живым!"
...
/mob/living/simple_animal/bot/atmosbot/set_custom_texts()
	...
	text_dehack = "Замечаю ошибки в прошивке [name] и исправляю их."
	text_dehack_fail = "[name] не отвечает на сброс команд!"
...
/mob/living/simple_animal/bot/atmosbot/get_controls(mob/user)
	...
	dat += hack(user)
	dat += showpai(user)
	dat += "<tt><b>Управление Атмосботом v1.1</b></tt><br><br>"
	dat += "Состояние: <a href='?src=[REF(src)];power=1'>[on ? "Вкл" : "Выкл"]</a><br>"
	dat += "Техническая панель [open ? "открыта" : "закрыта"]<br>"
	if(!locked || issilicon(user) || isAdminObserver(user))
		dat += "Давление в пробоинах: <a href='?src=[REF(src)];set_breach_pressure=1'>[breached_pressure]</a><br>"
		dat += "Управление температурой: <a href='?src=[REF(src)];toggle_temp_control=1'>[temperature_control?"Вкл":"Выкл"]</a><br>"
		dat += "Целевая температура: <a href='?src=[REF(src)];set_ideal_temperature=[ideal_temperature]'>[ideal_temperature]C</a><br>"
		dat += "Фильтрация газов<br>"
		for(var/gas_typepath in gasses)
			var/gas_enabled = gasses[gas_typepath]
			var/datum/gas/gas_type = gas_typepath
			dat += "[initial(gas_type.name)]: <a href='?src=[REF(src)];toggle_gas=[gas_typepath]'>[gas_enabled?"Фильтрация":"Нет"]</a><br>"
		dat += "Патруль: <A href='?src=[REF(src)];operation=patrol'>[auto_patrol ? "Да" : "Нет"]</A><BR>"
	return dat
...
/mob/living/simple_animal/bot/atmosbot/Topic(href, href_list)
	...
	if(href_list["set_breach_pressure"])
		var/new_breach_pressure = input(usr, "Сканировать давление в диапазоне? (0 до 100)", "Пробоины") as num
		...
		var/new_temp = input(usr, "Выставить целевую температуру ([T0C] до [T20C + 20])", "Целевая температура") as num
...
/mob/living/simple_animal/bot/atmosbot/explode()
	...
	if(tank && GM)
		for(var/datum/gas/G in tank.air_contents.get_gases())
			GM.adjust_moles(G.id, tank.air_contents.get_moles(G))
			tank.air_contents.adjust_moles(G.id, -tank.air_contents.get_moles())
