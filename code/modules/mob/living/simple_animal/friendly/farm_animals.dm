...
/mob/living/simple_animal/hostile/retaliate/goat/Life()
	...
	if(.)
		...
		if(enemies.len && prob(10))
			enemies = list()
			LoseTarget()
			src.visible_message("<span class='notice'>[capitalize(src.name)] calms down.</span>")
	if(stat == CONSCIOUS)
		udder.generateMilk()
		eat_plants()
		if(!pulledby)
...
/mob/living/simple_animal/hostile/retaliate/goat/AttackingTarget()
	...
	if(. && ishuman(target))
		...
		if(istype(H.dna.species, /datum/species/pod))
			...
			H.visible_message("<span class='warning'>[capitalize(src.name)] takes a big chomp out of [H]!</span>", \
								  "<span class='userdanger'>[capitalize(src.name)] takes a big chomp out of your [NB]!</span>")
			NB.dismember()
//cow
...
/mob/living/simple_animal/cow/wisdom/attack_hand(mob/living/carbon/M)
	if(!stat && M.a_intent == INTENT_HELP)
		to_chat(M, "<span class='nicegreen'>[capitalize(src.name)] whispers you some intense wisdoms and then disappears!</span>")
...
/mob/living/simple_animal/chicken/Life()
	...
	if((!stat && prob(3) && eggsleft > 0) && egg_type)
		visible_message("<span class='alertalien'>[capitalize(src.name)] [pick(layMessage)]</span>")
		eggsleft--
...
/obj/item/food/egg/process(delta_time)
	if(isturf(loc))
		...
		if(amount_grown >= 200)
			visible_message("<span class='notice'>[capitalize(src.name)] hatches with a quiet cracking sound.</span>")
