				...
				if("Cannibalize")
					if(D.health < D.maxHealth)
						...
						if(do_after(D, 60, 0, target = src))
							D.visible_message(span_notice("[D] repairs itself using [src] remains!") , span_notice("You repair yourself using [src] remains."))
...
/mob/living/simple_animal/drone/attack_hand(mob/user)
	if(ishuman(user))
		...
		if(buckled)
			to_chat(user, span_warning("[capitalize(src.name)] is buckled to [buckled] and cannot be picked up!"))
			return
	...
	if(I.tool_behaviour == TOOL_SCREWDRIVER && stat != DEAD)
		if(health < maxHealth)
			...
			if(I.use_tool(src, user, 80))
				...
				visible_message(span_notice("[user] tightens [src == user ? "[user.ru_ego()]" : "[src]"] loose screws!") , span_notice("You tighten [src == user ? "your" : "[src]"] loose screws."))
			else
				to_chat(user, span_warning("You need to remain still to tighten [src] screws!"))
		else
			to_chat(user, span_warning("[capitalize(src.name)] screws can't get any tighter!"))
		...
		user.visible_message(span_notice("[user] starts resetting [src]...") , \
			span_notice("You press down on [src] factory reset control..."))
		if(I.use_tool(src, user, 50, volume=50))
			user.visible_message(span_notice("[user] resets [src]!") , \
				span_notice("You reset [src] directives to factory defaults!"))
...
/mob/living/simple_animal/drone/proc/update_drone_hack(hack)
	...
	if(hack)
		if(hacked)
			return
		Stun(40)
		visible_message(span_warning("[capitalize(src.name)] display glows a vicious red!") , \
		...
		ADD_TRAIT(src, TRAIT_VENTCRAWLER_ALWAYS, "drone_hack")
		...
		if(!hacked)
			return
		Stun(40)
		visible_message(span_info("[capitalize(src.name)] display glows a content blue!") , \
		...
		REMOVE_TRAIT(src, TRAIT_VENTCRAWLER_ALWAYS, "drone_hack")
		speed = initial(speed)
