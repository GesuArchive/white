...
/datum/action/innate/elite_attack/apply_button_icon(atom/movable/screen/movable/action_button/button, force = FALSE)
	. = ..()
	if(!.)
		return
	...
	if(ishuman(user))
		switch(activity)
			if(TUMOR_PASSIVE)
				...
				visible_message(span_boldwarning("[capitalize(src.name)] convulses as your arm enters its radius. Your instincts tell you to step back."))
				activator = user
				if(boosted)
			...
			if(TUMOR_INACTIVE)
				...
				visible_message(span_boldwarning("[capitalize(src.name)] begins to convulse. Your instincts tell you to step back."))
				activator = user
				if(!boosted)
					addtimer(CALLBACK(src, PROC_REF(spawn_elite)), 30)
					return
				visible_message(span_boldwarning("Something within [src] stirs..."))
				var/list/candidates = poll_candidates_for_mob("Do you want to play as a lavaland elite?", ROLE_SENTIENCE, null, 50, src, POLL_IGNORE_SENTIENCE_POTION)
				if(candidates.len)
...
/obj/structure/elite_tumor/proc/spawn_elite(mob/dead/observer/elitemind)
	...
	if(boosted)
		...
		notify_ghosts("\A [mychild] has been awakened in [get_area(src)]!", source = mychild, action = NOTIFY_ORBIT, flashwindow = FALSE, header = "Lavaland Elite awakened")
...
/obj/structure/elite_tumor/proc/return_elite()
	...
	if(boosted)
		...
		notify_ghosts("\A [mychild] has been challenged in [get_area(src)]!", source = mychild, action = NOTIFY_ORBIT, flashwindow = FALSE, header = "Lavaland Elite challenged")
...
/obj/structure/elite_tumor/proc/onEliteLoss()
	...
	visible_message(span_boldwarning("[capitalize(src.name)] begins to convulse violently before beginning to dissipate."))
...
/obj/item/tumor_shard/afterattack(atom/target, mob/user, proximity_flag)
	...
	if(istype(target, /mob/living/simple_animal/hostile/asteroid/elite) && proximity_flag)
		...
		to_chat(E, "<span class='userdanger'>You have been revived by [user]. While you can't speak to them, you owe [user] a great debt. Assist [user.ru_na()] in achieving [user.ru_ego()] goals, regardless of risk.</span")
		...
		to_chat(user, span_info("[capitalize(src.name)] only works on the corpse of a sentient lavaland elite."))
