	...
	if(ishuman(user))
		switch(activity)
			...
			if(TUMOR_INACTIVE)
				...
				var/list/candidates = poll_candidates_for_mob("Do you want to play as a lavaland elite?", ROLE_SENTIENCE, null, ROLE_SENTIENCE, 50, src, POLL_IGNORE_SENTIENCE_POTION)
				if(candidates.len)
