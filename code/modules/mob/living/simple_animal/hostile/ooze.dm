...
/datum/action/cooldown/globules/set_click_ability(mob/on_who)
	...
	to_chat(on_who, "<span class='notice'>You prepare to launch a mending globule. <B>Left-click to fire at a target!</B></span>")
...
/datum/action/cooldown/globules/unset_click_ability(mob/on_who, refund_cooldown = TRUE)
	...
	if(refund_cooldown)
		to_chat(on_who, "<span class='notice'>You stop preparing your mending globules.</span>")
...
/datum/action/cooldown/globules/Activate(atom/target)
	...
	if(istype(oozy_owner))
		if(oozy_owner.ooze_nutrition < 5)
			to_chat(oozy_owner, "<span class='warning'>You need at least 5 nutrition to launch a mending globule.</span>")
			return FALSE
...
/datum/action/cooldown/globules/InterceptClickOn(mob/living/caller, params, atom/target)
	...
	caller.visible_message(
		"<span class='nicegreen'>[caller] launches a mending globule!</span>",
		"<span class='notice'>You launch a mending globule.</span>",
	)
