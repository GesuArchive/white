...
/obj/structure/carp_rift/proc/update_check()
	...
	if(time_charged == (max_charge - 120))
		...
		priority_announce("A rift is causing an unnaturally large energy flux in [A.map_name].  Stop it at all costs!", "Central Command Spatial Corps", 'sound/ai/announcer/anomaly.ogg')
	if(time_charged == max_charge)
		var/area/A = get_area(src)
