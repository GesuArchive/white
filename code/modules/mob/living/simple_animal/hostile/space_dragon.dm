...
/mob/living/simple_animal/hostile/space_dragon/Life(delta_time = SSMOBS_DT, times_fired)
	...
	for(var/mob/living/consumed_mob in src)
		...
		visible_message(span_danger("[capitalize(src.name)] vomits up [consumed_mob]!"))
...
/mob/living/simple_animal/hostile/space_dragon/proc/dragon_fire_line(turf/T)
	...
	for(var/mob/living/L in T.contents)
		...
		hit_list += L
		L.adjustFireLoss(30)
		to_chat(L, span_userdanger("You're hit by [src] fire breath!"))
...
/obj/structure/carp_rift/proc/summon_carp(mob/user)
	...
	newcarp.AddElement(/datum/element/prevent_attacking_of_types, GLOB.typecache_general_bad_hostile_attack_targets, "Мерзость! Оно плохо пахнет! Нехочу это есть или трогать!")
