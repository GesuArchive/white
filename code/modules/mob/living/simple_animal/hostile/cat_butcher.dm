...
/mob/living/simple_animal/hostile/cat_butcherer/AttackingTarget()
	...
	if(. && prob(35) && iscarbon(target))
		...
		if(!QDELETED(tail))
			visible_message("<span class='notice'>[capitalize(src.name)] severs [L]'s tail in one swift swipe!</span>", "<span class='notice'>You sever [L]'s tail in one swift swipe.</span>")
			tail.Remove(L)
