...
/mob/living/simple_animal/attack_hand(mob/living/carbon/human/M)
	...
	switch(M.a_intent)
		if(INTENT_HELP)
			if (stat == DEAD)
				return
			visible_message("<span class='notice'>[M] [response_help_continuous] [skloname(name, VINITELNI, gender)].</span>" , \
							"<span class='notice'>[M] [response_help_continuous] меня.</span>" , null, null, M)
			to_chat(M, "<span class='notice'>[M] [response_help_simple] [skloname(name, VINITELNI, gender)].</span>")
		...
		if(INTENT_DISARM)
			...
			if(!Move(get_step(src, shove_dir), shove_dir))
				...
				M.visible_message("<span class='danger'>[M.name] толкает [skloname(name, VINITELNI, gender)]!</span>" ,
					"<span class='danger'>Толкаю [skloname(name, VINITELNI, gender)]!</span>" , "<span class='hear'>Слышу агрессивную потасовку!</span>" , COMBAT_MESSAGE_RANGE, list(src))
				to_chat(src, "<span class='userdanger'>Меня толкает [M.name]!</span>")
				return TRUE
			log_combat(M, src, "shoved", "pushing it")
			M.visible_message("<span class='danger'>[M.name] толкает [skloname(name, VINITELNI, gender)] достаточно сильно!</span>" ,
				"<span class='danger'>Толкаю [skloname(name, VINITELNI, gender)] достаточно сильно!</span>" , "<span class='hear'>Слышу агрессивную потасовку!</span>" , COMBAT_MESSAGE_RANGE, list(src))
			to_chat(src, "<span class='userdanger'>Меня толкает [name]!</span>")
			return TRUE

		if(INTENT_HARM)
			if(HAS_TRAIT(M, TRAIT_PACIFISM))
				to_chat(M, "<span class='warning'>Не хочу вредить [skloname(name, VINITELNI, gender)]!</span>")
				return
			M.do_attack_animation(src, ATTACK_EFFECT_PUNCH)
			visible_message("<span class='danger'>[M] [response_harm_continuous] [skloname(name, VINITELNI, gender)]!</span>" ,\
							"<span class='userdanger'>[M] [response_harm_continuous] меня!</span>" , null, COMBAT_MESSAGE_RANGE, M)
			to_chat(M, "<span class='danger'>Моя атака [response_harm_simple] [skloname(name, VINITELNI, gender)]!</span>")
...
/mob/living/simple_animal/attack_hulk(mob/living/carbon/human/user)
	...
	visible_message("<span class='danger'>[user] бьёт [skloname(name, VINITELNI, gender)]!</span>" , \
					"<span class='userdanger'>Меня бьёт [user]!</span>" , null, COMBAT_MESSAGE_RANGE, user)
	to_chat(user, "<span class='danger'>Бью [skloname(name, VINITELNI, gender)]!</span>")
	adjustBruteLoss(15)
...
/mob/living/simple_animal/attack_paw(mob/living/carbon/human/M)
	...
	if (M.a_intent == INTENT_HELP)
		if (health > 0)
			visible_message("<span class='notice'>[M.name] [response_help_continuous] [skloname(name, VINITELNI, gender)].</span>" , \
							"<span class='notice'>[M.name] [response_help_continuous] меня.</span>" , null, COMBAT_MESSAGE_RANGE, M)
			to_chat(M, "<span class='notice'>Моя атака [response_help_simple] [skloname(name, VINITELNI, gender)].</span>")
			playsound(loc, 'sound/weapons/thudswoosh.ogg', 50, TRUE, -1)
...
/mob/living/simple_animal/attack_alien(mob/living/carbon/alien/humanoid/M)
	if(..()) //if harm or disarm intent.
		if(M.a_intent == INTENT_DISARM)
			...
			visible_message("<span class='danger'>[M] [response_disarm_continuous] [skloname(name, VINITELNI, gender)]!</span>" , \
							"<span class='userdanger'>[M] [response_disarm_continuous] меня!</span>" , null, COMBAT_MESSAGE_RANGE, M)
			to_chat(M, "<span class='danger'>Моя атака [response_disarm_simple] [skloname(name, VINITELNI, gender)]!</span>")
			log_combat(M, src, "disarmed")
		else
			var/damage = rand(15, 30)
			visible_message("<span class='danger'>[M] рвёт когтями [skloname(name, VINITELNI, gender)]!</span>" , \
							"<span class='userdanger'>Меня рвёт когтями [M]!</span>" , null, COMBAT_MESSAGE_RANGE, M)
			to_chat(M, "<span class='danger'>Рву когтями [skloname(name, VINITELNI, gender)]!</span>")
