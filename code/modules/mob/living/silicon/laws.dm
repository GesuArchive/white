...
/mob/living/silicon/proc/deadchat_lawchange()
	...
	deadchat_broadcast(" <b>теперь следует новым законам.</b> <a href='?src=[REF(src)]&dead=1&printlawtext=[url_encode(lawtext)]'>Показать</a>", "<span class='name'>[capitalize(src.name)]</span>", follow_target=src, message_type=DEADCHAT_LAWCHANGE)
