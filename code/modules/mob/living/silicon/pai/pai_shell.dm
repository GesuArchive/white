...
/mob/living/silicon/pai/proc/fold_out(force = FALSE)
	...
	set_density(TRUE)
	if(istype(card.loc, /obj/item/modular_computer/tablet/pda))
		var/obj/item/modular_computer/tablet/pda/P = card.loc
		P.pai = null
		P.visible_message(span_notice("[capitalize(src.name)] ejects itself from [P]!"))
	...
	icon_state = "[chassis]"
	held_state = "[chassis]"
	visible_message(span_boldnotice("[capitalize(src.name)] folds out its holochassis emitter and forms a holoshell around itself!"))
	holoform = TRUE
...
/mob/living/silicon/pai/proc/fold_in(force = FALSE)
	...
	if(!holoform)
		. = fold_out(force)
		return
	visible_message(span_notice("[capitalize(src.name)] deactivates its holochassis emitter and folds back into a compact card!"))
	stop_pulling()
...
/mob/living/silicon/pai/update_resting()
	...
	if(resting)
		...
		icon_state = "[chassis]"
	if(loc != card)
		visible_message(span_notice("[capitalize(src.name)] [resting? "lays down for a moment..." : "perks up from the ground"]"))
...
/mob/living/silicon/pai/mob_try_pickup(mob/living/user)
	if(!possible_chassis[chassis])
		to_chat(user, span_warning("[capitalize(src.name)] current form isn't able to be carried!"))
		return FALSE
	return ..()
