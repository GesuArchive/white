...
/mob/living/hitby(atom/movable/AM, skipcatch, hitpush = TRUE, blocked = FALSE, datum/thrownthing/throwingdatum)
	if(isitem(AM))
		...
		if(nosell_hit)
			return ..()
		visible_message("<span class='danger'>В <b>[skloname(name, VINITELNI, gender)]</b> попадает <b>[thrown_item.name]</b>!</span>" , \
...
/mob/living/proc/grabbedby(mob/living/carbon/user, supress_message = FALSE)
	...
	if(!(status_flags & CANPUSH) || HAS_TRAIT(src, TRAIT_PUSHIMMUNE))
		to_chat(user, "<span class='warning'>Не могу схватить <b>[skloname(name, VINITELNI, gender)]</b> сильнее!</span>")
		return FALSE

	if(user.grab_state >= GRAB_AGGRESSIVE && HAS_TRAIT(user, TRAIT_PACIFISM))
		to_chat(user, "<span class='warning'>Не хочу случайно навредить <b>[skloname(name, VINITELNI, gender)]</b>!</span>")
		return FALSE
	grippedby(user)
...
/mob/living/proc/grippedby(mob/living/carbon/user, instant = FALSE)
	if(user.grab_state < GRAB_KILL)
		...
		if(user.grab_state) //only the first upgrade is instantaneous
			...
			visible_message("<span class='danger'><b>[user]</b> начинает брать <b>[skloname(name, VINITELNI, gender)]</b> в более крепкий захват!</span>" , \
				"<span class='userdanger'><b>[user]</b> начинает брать меня <b>[skloname(name, VINITELNI, gender)]</b> в более крепкий захват!</span>" , "<span class='hear'>Слышу агрессивную потасовку!</span>" , null, user)
			to_chat(user, "<span class='danger'>Начинаю брать [skloname(name, VINITELNI, gender)] в более крепкий захват!</span>")
		...
		switch(user.grab_state)
			if(GRAB_AGGRESSIVE)
				...
				if(HAS_TRAIT(user, TRAIT_PACIFISM))
					visible_message("<span class='danger'><b>[user]</b> крепко хватает <b>[skloname(name, VINITELNI, gender)]</b>!</span>" ,
									"<span class='danger'><b>[user]</b> крепко держит меня!</span>" , "<span class='hear'>Слышу агрессивную потасовку!</span>" , null, user)
					to_chat(user, "<span class='danger'>Крепко хватаю [skloname(name, VINITELNI, gender)]!</span>")
					add_log = " (pacifist)"
				else
					visible_message("<span class='danger'><b>[user]</b> хватает <b>[skloname(name, VINITELNI, gender)]</b> крепче!</span>" , \
									"<span class='userdanger'><b>[user]</b> хватает меня крепче!</span>" , "<span class='hear'>Слышу агрессивную потасовку!</span>" , null, user)
					to_chat(user, "<span class='danger'>Хватаю [skloname(name, VINITELNI, gender)] крепче!</span>")
				drop_all_held_items()
				stop_pulling()
				log_combat(user, src, "grabbed", addition="aggressive grab[add_log]")
			if(GRAB_NECK)
				log_combat(user, src, "grabbed", addition="neck grab")
				visible_message("<span class='danger'><b>[user]</b> хватает <b>[skloname(name, VINITELNI, gender)]</b> за шею!</span>" ,\
								"<span class='userdanger'><b>[user]</b> хватает меня за шею!</span>" , "<span class='hear'>Слышу агрессивную потасовку!</span>" , null, user)
				to_chat(user, "<span class='danger'>Хватаю [skloname(name, VINITELNI, gender)] за шею!</span>")
				if(!buckled && !density)
					Move(user.loc)
			if(GRAB_KILL)
				log_combat(user, src, "strangled", addition="kill grab")
				visible_message("<span class='danger'><b>[user]</b> душит <b>[skloname(name, VINITELNI, gender)]</b>!</span>" , \
								"<span class='userdanger'><b>[user]</b> душит меня!</span>" , "<span class='hear'>Слышу агрессивную потасовку!</span>" , null, user)
				to_chat(user, "<span class='danger'>Душу [skloname(name, VINITELNI, gender)]!</span>")
				if(!buckled && !density)
					Move(user.loc)
...
/mob/living/attack_slime(mob/living/simple_animal/slime/M)
	...
	if (stat != DEAD)
		...
		visible_message("<span class='danger'><b>[M.name]</b> кушает <b>[skloname(name, VINITELNI, gender)]</b>!</span>" , \
		...
		to_chat(M, "<span class='danger'>Кушаю [skloname(name, VINITELNI, gender)]!</span>")
		return TRUE

/mob/living/attack_animal(mob/living/simple_animal/M)
	M.face_atom(src)
	if(M.melee_damage_upper == 0)
		visible_message("<span class='notice'><b>[M]</b> [M.friendly_verb_continuous] <b>[skloname(name, VINITELNI, gender)]</b>!</span>" , \
						"<span class='notice'><b>[M]</b> [M.friendly_verb_continuous] меня!</span>" , null, COMBAT_MESSAGE_RANGE, M)
		to_chat(M, "<span class='notice'>[M.friendly_verb_simple] <b>[skloname(name, VINITELNI, gender)]</b>!</span>")
		return FALSE
	if(HAS_TRAIT(M, TRAIT_PACIFISM))
		to_chat(M, "<span class='warning'>Не хочу вредить!</span>")
		return FALSE
	...
	M.do_attack_animation(src)
	visible_message("<span class='danger'><b>[M]</b> [M.attack_verb_continuous] <b>[skloname(name, VINITELNI, gender)]</b>!</span>" , \
					"<span class='userdanger'><b>[M]</b> [M.attack_verb_continuous] меня!</span>" , null, COMBAT_MESSAGE_RANGE, M)
	to_chat(M, "<span class='danger'>[M.attack_verb_simple] <b>[skloname(name, VINITELNI, gender)]</b>!</span>")
	...
	switch (M.a_intent)
		if (INTENT_HARM)
			...
			if (prob(75))
				...
				visible_message("<span class='danger'><b>[M.name]</b> кусает <b>[skloname(name, VINITELNI, gender)]</b>!</span>" , \
								"<span class='userdanger'><b>[M.name]</b> кусает меня!</span>" , "<span class='hear'>Слышу кусь!</span>" , COMBAT_MESSAGE_RANGE, M)
				to_chat(M, "<span class='danger'>Кусаю [skloname(name, VINITELNI, gender)]!</span>")
				return TRUE
			else
				visible_message("<span class='danger'><b>[M.name]</b> пытается укусить <b>[skloname(name, VINITELNI, gender)]</b>!</span>" , \
				...
				to_chat(M, "<span class='warning'>Пытаюсь укусить [skloname(name, VINITELNI, gender)]!</span>")
		if (INTENT_GRAB)
			grabbedby(M)
			return FALSE
		if (INTENT_DISARM)
...
/mob/living/attack_larva(mob/living/carbon/alien/larva/L)
	switch(L.a_intent)
		if(INTENT_HELP)
			visible_message("<span class='notice'><b>[L.name]</b> трётся своей головой о <b>[skloname(name, VINITELNI, gender)]</b>.</span>" , \
							"<span class='notice'><b>[L.name]</b> трётся своей головой о меня.</span>" , null, null, L)
			to_chat(L, "<span class='notice'>Тру свою голову о [skloname(name, VINITELNI, gender)].</span>")
			return FALSE
			...
			if(prob(90))
				...
				visible_message("<span class='danger'><b>[L.name]</b> кусает [skloname(name, VINITELNI, gender)]!</span>" , \
								"<span class='userdanger'><b>[L.name]</b> кусает меня!</span>" , "<span class='hear'>Слышу кусь!</span>" , COMBAT_MESSAGE_RANGE, L)
				to_chat(L, "<span class='danger'>Кусаю [skloname(name, VINITELNI, gender)]!</span>")
				playsound(loc, 'sound/weapons/bite.ogg', 50, TRUE, -1)
				return TRUE
			else
				visible_message("<span class='danger'><b>[L.name]</b> пытается укусить <b>[skloname(name, VINITELNI, gender)]</b>!</span>" , \
				...
				to_chat(L, "<span class='danger'>Пытаюсь укусить [skloname(name, VINITELNI, gender)]!</span>")
	return FALSE

/mob/living/attack_alien(mob/living/carbon/alien/humanoid/M)
	switch(M.a_intent)
		if (INTENT_HELP)
			visible_message("<span class='notice'><b>[M]</b> обнимает <b>[skloname(name, VINITELNI, gender)]</b> своей клешнёй.</span>" , \
				"<span class='notice'><b>[M]</b> обнимает меня своей клешнёй.</span>" , null, null, M)
			to_chat(M, "<span class='notice'>Обнимаю [skloname(name, VINITELNI, gender)] своей клешнёй.</span>")
			return FALSE
		if (INTENT_GRAB)
			grabbedby(M)
			return FALSE
...
/mob/living/attack_hulk(mob/living/carbon/human/user)
	..()
	if(HAS_TRAIT(user, TRAIT_PACIFISM))
		to_chat(user, "<span class='warning'>Не хочу вредить <b>[skloname(name, VINITELNI, gender)]</b>!</span>")
		return FALSE
	return TRUE
