...
/mob/living/attack_slime(mob/living/simple_animal/slime/M)
	...
	if (stat != DEAD)
		...
		visible_message(span_danger("<b>[M.name]</b> кушает <b>[skloname(name, VINITELNI, gender)]</b>!") , \
				span_userdanger("<b>[M.name]</b> кушает меня!") , span_hear("Слышу как что-то сильно бьёт по плоти!") , COMBAT_MESSAGE_RANGE, M)
		to_chat(M, span_danger("Кушаю [skloname(name, VINITELNI, gender)]!"))
		return TRUE
	...
	switch(L.a_intent)
		if(INTENT_HELP)
			...
			if(prob(90))
				...
				visible_message(span_danger("<b>[L.name]</b> кусает [skloname(name, VINITELNI, gender)]!") , \
								span_userdanger("<b>[L.name]</b> кусает меня!") , span_hear("Слышу кусь!") , COMBAT_MESSAGE_RANGE, L)
				to_chat(L, span_danger("Кусаю [skloname(name, VINITELNI, gender)]!"))
				playsound(loc, 'sound/weapons/bite.ogg', 50, TRUE, -1)
				return TRUE
			else
				visible_message(span_danger("<b>[L.name]</b> пытается укусить <b>[skloname(name, VINITELNI, gender)]</b>!") , \
								span_userdanger("<b>[L.name]</b> пытается укусить меня!") , span_hear("Слышу как защелкиваются челюсти!") , COMBAT_MESSAGE_RANGE, L)
				to_chat(L, span_danger("Пытаюсь укусить [skloname(name, VINITELNI, gender)]!"))
	return FALSE
...
/mob/living/attack_hulk(mob/living/carbon/human/user)
	..()
	if(HAS_TRAIT(user, TRAIT_PACIFISM))
		to_chat(user, span_warning("Не хочу вредить <b>[skloname(name, VINITELNI, gender)]</b>!"))
		return FALSE
	return TRUE
