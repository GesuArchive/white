		...
		if(isliving(hit_atom))
			...
			if(ishuman(hit_atom))
				...
				if(H.check_shields(src, 0, "the [name]", attack_type = LEAP_ATTACK))
					blocked = TRUE
			if(!blocked)
				L.visible_message("<span class='danger'>[capitalize(src.name)] pounces on [L]!</span>", "<span class='userdanger'>[capitalize(src.name)] pounces on you!</span>")
				L.Paralyze(100)
			...
			visible_message("<span class='danger'>[capitalize(src.name)] smashes into [hit_atom]!</span>", "<span class='alertalien'>[capitalize(src.name)] smashes into [hit_atom]!</span>")
			Paralyze(40, ignore_canstun = TRUE)
