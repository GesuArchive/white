...
/mob/living/carbon/proc/offer_high_five(obj/item/slap)
	...
	if(!(locate(/mob/living/carbon) in orange(1, src)))
		visible_message("<span class='danger'>[capitalize(src.name)] raises [p_their()] arm, looking around for a high-five, but there's no one around! How embarassing...</span>", \
