...
/datum/species/jelly/stargazer/proc/unlink_mob(mob/living/M)
	...
	action.Remove(M)
	to_chat(M, span_notice("You are no longer connected to [slimelink_owner.real_name] Slime Link."))
...
/datum/action/innate/link_minds/Activate()
	...
	to_chat(H, span_notice("You begin linking [target] mind to yours..."))
	...
	if(do_after(H, 60, target = target))
		...
		if(species.link_mob(target))
			to_chat(H, span_notice("You connect [target] mind to your slime link!"))
		else
			to_chat(H, span_warning("You can't seem to link [target] mind..."))
