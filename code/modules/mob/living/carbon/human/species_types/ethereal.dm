...
/datum/species/ethereal/proc/on_emp_act(mob/living/carbon/human/H, severity)
	EMPeffect = TRUE
	spec_updatehealth(H)
	to_chat(H, "<span class='notice'>Чувствую, как свет покидает моё тело.</span>")
	switch(severity)
		if(EMP_LIGHT)
...
/datum/species/ethereal/proc/on_emag_act(mob/living/carbon/human/H, mob/user)
	...
	emageffect = TRUE
	if(user)
		to_chat(user, "<span class='notice'>Касаюсь картой по [H].</span>")
	H.visible_message("<span class='danger'>[H] начинает мерцать множеством цветов!</span>")
	handle_emag(H)
...
/datum/species/ethereal/proc/stop_emp(mob/living/carbon/human/H)
	EMPeffect = FALSE
	spec_updatehealth(H)
	to_chat(H, "<span class='notice'>Чувствую себя более наэлектризованным, как только моё сияние вернулось.</span>")
...
/datum/species/ethereal/proc/stop_emag(mob/living/carbon/human/H)
	emageffect = FALSE
	spec_updatehealth(H)
	H.visible_message("<span class='danger'>[H] перестаёт мигать и возвращается в нормальное состояние.</span>")
...
/datum/species/ethereal/proc/discharge_process(mob/living/carbon/human/H)
	to_chat(H, "<span class='warning'>Чувствую, что теряю контроль над своим зарядом!</span>")
	H.visible_message("<span class='danger'>[H] начинает неистово искрить!</span>")
	...
	if(do_after(H, 5 SECONDS, timed_action_flags = (IGNORE_USER_LOC_CHANGE|IGNORE_HELD_ITEM|IGNORE_INCAPACITATED)))
		...
		to_chat(H, "<span class='warning'>Неистово выпускаю энергию!</span>")
		H.visible_message("<span class='danger'>[H] неистово выпускает энергию!</span>")
		if(prob(10)) //chance of developing heart disease to dissuade overcharging oneself
			var/datum/disease/D = new /datum/disease/heart_failure
			H.ForceContractDisease(D)
			to_chat(H, "<span class='userdanger'>Сердце остановилось на секунду...</span>")
