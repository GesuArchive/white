...
/mob/living/carbon/human/handle_blood(delta_time, times_fired)
	...
	if(bodytemperature >= TCRYO && !(HAS_TRAIT(src, TRAIT_HUSK))) //cryosleep or husked people do not pump the blood.
		...
		if(blood_volume < BLOOD_VOLUME_NORMAL && !HAS_TRAIT(src, TRAIT_NOHUNGER))
			...
			adjust_pooition(nutrition_ratio * HUNGER_FACTOR * delta_time)
			blood_volume = min(blood_volume + (BLOOD_REGEN_FACTOR * nutrition_ratio * delta_time), BLOOD_VOLUME_NORMAL)

		//Effects of bloodloss
		var/word = pick("головокружение","тошноту","слабость")
		switch(blood_volume)
			if(BLOOD_VOLUME_EXCESS to BLOOD_VOLUME_MAX_LETHAL)
				if(DT_PROB(7.5, delta_time))
					to_chat(src, "<span class='userdanger'>Кровь начинает разрывать мою кожу. Я ЛОПАЮСЬ!</span>")
					inflate_gib()
			if(BLOOD_VOLUME_MAXIMUM to BLOOD_VOLUME_EXCESS)
				if(DT_PROB(5, delta_time))
					to_chat(src, "<span class='warning'>Ощущаю себя раздуто.</span>")
			if(BLOOD_VOLUME_OKAY to BLOOD_VOLUME_SAFE)
				if(DT_PROB(2.5, delta_time))
					to_chat(src, "<span class='warning'>Ощущаю [word].</span>")
			...
			if(BLOOD_VOLUME_BAD to BLOOD_VOLUME_OKAY)
				...
				if(DT_PROB(2.5, delta_time))
					blur_eyes(6)
					to_chat(src, "<span class='warning'>Чувствую всем своим телом [word].</span>")
			if(BLOOD_VOLUME_SURVIVE to BLOOD_VOLUME_BAD)
				adjustOxyLoss(2.5 * delta_time)
				if(DT_PROB(7.5, delta_time))
					Unconscious(rand(20,60))
					to_chat(src, "<span class='warning'>Панически ощущаю [word].</span>")
