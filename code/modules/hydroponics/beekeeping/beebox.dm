...
/obj/structure/beebox/examine(mob/user)
	...
	. += "<span class='notice'>\n[bee_resources]/100 resource supply.</span>"
	. += "<span class='notice'>\n[bee_resources]% towards a new honeycomb.</span>"
	. += "<span class='notice'>\n[bee_resources*2]% towards a new bee.</span>"

	if(honeycombs.len)
		var/plural = honeycombs.len > 1
		. += "<span class='notice'>\nThere [plural? "are" : "is"] [honeycombs.len] uncollected honeycomb[plural ? "s":""] in the apiary.</span>"

	if(honeycombs.len >= get_max_honeycomb())
		. += "<span class='warning'>\nThere's no room for more honeycomb!</span>"
