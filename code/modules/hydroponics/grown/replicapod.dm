...
/obj/item/seeds/replicapod/harvest(mob/user) //now that one is fun -- Urist
	...
	if(!make_podman)
		...
		if(mind || ckey)
			var/choice = tgui_alert(usr, "The pod is currently devoid of soul. There is a possibility that a soul could claim this creature, or you could harvest it for seeds.", "Harvest Seeds?", "Harvest Seeds", "Cancel")
			if(choice == "Cancel")
				return result
