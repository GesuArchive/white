...
/datum/smite/dock_pay/effect(client/user, mob/living/target)
	...
	if (!card)
		to_chat(user, "<span class='warning'>[dude] does not have an ID card on!</span>")
		return
	if (!card.registered_account)
		to_chat(user, "<span class='warning'>[dude] does not have an ID card with an account!</span>")
		return
