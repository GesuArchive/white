...
/obj/effect/fun_balloon/attack_ghost(mob/user)
	if(!user.client || !user.client.holder || popped)
		return
	var/confirmation = tgui_alert(usr, "Pop [src]?","Fun Balloon","Yes","No")
...
/obj/effect/fun_balloon/sentience/effect()
	...
	var/list/candidates = poll_candidates_for_mobs(question, ROLE_PAI, null, 100, bodies)
	while(LAZYLEN(candidates) && LAZYLEN(bodies))
		var/mob/dead/observer/C = pick_n_take(candidates)
		var/mob/living/body = pick_n_take(bodies)

		to_chat(body, span_warning("Your mob has been taken over by a ghost!"))
...
/obj/effect/forcefield/arena_shuttle/Bumped(atom/movable/AM)
	...
	if(L.pulling && istype(L.pulling, /obj/item/bodypart/head))
		to_chat(L, span_notice("Your offering is accepted. You may pass."))
		qdel(L.pulling)
		...
		to_chat(L, span_warning("You are not yet worthy of passing. Drag a severed head to the barrier to be allowed entry to the hall of champions."))
