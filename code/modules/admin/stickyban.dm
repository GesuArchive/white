/datum/admins/proc/stickyban(action,data)
	...
	switch (action)
		...
		if ("remove_alt")
			...
			if (!key)
				to_chat(usr, "<span class='adminnotice'>Error: [alt] is not linked to [ckey] sticky ban!</span>")
				return
			...
			if (!key)
				to_chat(usr, "<span class='adminnotice'>Error: [alt] link to [ckey] sticky ban disappeared.</span>")
				return
		...
		if ("unexempt")
			...
			if (!key)
				to_chat(usr, "<span class='adminnotice'>Error: [alt] is not exempt from [ckey] sticky ban!</span>")
				return
			...
			if (!key)
				to_chat(usr, "<span class='adminnotice'>Error: [alt] exemption from [ckey] sticky ban disappeared.</span>")
				return
