...
/client/proc/callproc_blocking(list/get_retval)
	...
	switch(tgui_alert(usr,"Proc owned by something?",,list("Yes","No")))
		if("Yes")
			targetselected = TRUE
			var/list/value = vv_get_value(default_class = VV_ATOM_REFERENCE, classes = list(VV_ATOM_REFERENCE, VV_DATUM_REFERENCE, VV_MOB_REFERENCE, VV_CLIENT, VV_MARKED_DATUM, VV_TEXT_LOCATE, VV_PROCCALL_RETVAL, VV_CALLERMOB))
...
/client/proc/callproc_datum(datum/A as null|area|mob|obj|turf)
	...
	if(!hascall(A,procname))
		to_chat(usr, "<span class='red'>Error: callproc_datum(): type [A.type] has no proc named [procname].</span>")
		return
	var/list/lst = get_callproc_args()
	if(!lst)
		return
...
/client/proc/get_callproc_returnval(returnval,procname)
	...
	if(islist(returnval))
		...
		. = "<span class='blue'>[procname] returned: [!isnull(returnval) ? html_encode(returnval) : "null"]</span>"
