...
/mob/verb/pray(msg as text)
	...
	if(GLOB.say_disabled)	//This is here to try to identify lag problems
		to_chat(usr, "<span class='danger'>Не могу молиться.</span>")
		return
	...
	if(usr.client)
		if(usr.client.prefs.muted & MUTE_PRAY)
			to_chat(usr, "<span class='danger'>Не хочу молиться.</span>")
			return
	...
	to_chat(usr, "<span class='info'>Молитва: \"[msg_tmp]\"</span>")
