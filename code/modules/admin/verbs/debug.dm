...
/client/proc/generate_ruin()
	...
	var/is_city = alert(src, "Город генерируем?", "Да", "Нет")
	if (is_city == "Да")
		generate_space_ruin(mob.x, mob.y, mob.z, border_size, border_size, forced_decoration = /datum/generator_settings/city, allowed_flags = list(RUIN_PART_CITY))
