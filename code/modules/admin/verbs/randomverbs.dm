...
/client/proc/admin_headset_message(mob/M in GLOB.mob_list, sender = null)
	...
	var/input = tgui_input_text(usr, "Please enter a message to reply to [key_name(H)] via their headset.","Outgoing message from [sender]", "")
	if(!input)
	...
	to_chat(H, "<span class='hear'>Наушники начинают шуршать, затем из них раздаётся голос, который говорит:  \"Пожалуйста, прослушайте сообщение от [sender == "Syndicate" ? "вашего покровителя" : "Центрального Командования"]. Сообщение гласит[sender == "Syndicate" ? ", агент" : ""]: <b>[input].</b> Конец сообщения.\"</span>")
...
//shitty pun
/proc/pieing_squad(mob/living/carbon/target, turf/source_turf)
...
/client/proc/trigger_centcom_recall()
	...
	message = tgui_input_text(usr, "Enter message from the on-call admiral to be put in the recall report.", "Admiral Message", message)
