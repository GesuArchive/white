	...
	switch(action)
		...
		if("togglearrivals")
			...
			if(A)
				...
				to_chat(holder, "<span class='admin'>There is no arrivals shuttle.</span>")
		if("movelaborshuttle")
		...
		if("customportal")
			...
			if (prefreturn["button"] == 1)
				...
				if (prefs["amount"]["value"] < 1 || prefs["portalnum"]["value"] < 1)
					to_chat(holder, "<span class='warning'>Number of portals and mobs to spawn must be at least 1.</span>")
					return
				...
				if (!ispath(pathToSpawn))
					to_chat(holder, "<span class='notice'>Invalid path [pathToSpawn].</span>")
					return
