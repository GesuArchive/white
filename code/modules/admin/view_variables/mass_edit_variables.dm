...
/client/proc/massmodify_variables(datum/O, var_name = "", method = 0)
	...
	if(variable in GLOB.VVpixelmovement)
		if(!check_rights(R_DEBUG))
			return
		var/prompt = tgui_alert(usr, "Editing this var may irreparably break tile gliding for the rest of the round. THIS CAN'T BE UNDONE", "DANGER", "ABORT ", "Continue", " ABORT")
		if (prompt != "Continue")
			return
	...
	switch(class)
		...
		if(VV_TEXT)
			...
			var/unique
			if (varsvars?.len)
				unique = tgui_alert(usr, "Process vars unique to each instance, or same for all?", "Variable Association", "Unique", "Same")
				if(unique == "Unique")
					unique = TRUE
				else
					unique = FALSE
		...
		if (VV_NEW_TYPE)
			var/many = tgui_alert(usr, "Create only one [value["type"]] and assign each or a new one for each thing", "How Many", "One", "Many", "Cancel")
			if (many == "Cancel")
				return
			if (many == "Many")
				many = TRUE
