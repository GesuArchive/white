/datum/round_event_control/fake_virus
	name = "Страдание: Фальшивый вирус"
	...
	if(defacto_min)
		for(var/i=1; i<=rand(1,defacto_min); i++)
			...
			if(prob(25))//1/4 odds to get a spooky message instead of coughing out loud
				addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(to_chat), onecoughman, span_warning("[pick("Твоя голова болит.", "Твоя голова тяжелеет.")]")), rand(30,150))
			else
				addtimer(CALLBACK(onecoughman, .mob/proc/emote, pick("кашляет", "чихает", "сопит")), rand(30,150))//deliver the message with a slightly randomized time interval so there arent multiple people coughing at the exact same time
