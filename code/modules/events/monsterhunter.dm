...
/datum/round_event_control/bloodsucker_hunters
	name = "Спавн: Охотник на вампиров"
...
/datum/round_event/bloodsucker_hunters/start()
	...
	for(var/mob/living/carbon/human/all_players in shuffle(GLOB.player_list))
		...
		if(!SSjob.GetJob(all_players.mind.assigned_role) || (all_players.mind.assigned_role in GLOB.security_positions))
			continue
		if(IS_BLOODSUCKER(all_players) || IS_VASSAL(all_players) || IS_HERETIC(all_players) || iscultist(all_players) || IS_WIZARD(all_players) || is_servant_of_ratvar(all_players) || all_players.mind.has_antag_datum(/datum/antagonist/changeling))
			continue
...
/datum/round_event_control/monster_hunters
	name = "Спавн: Охотник на монстров"
...
/datum/round_event/monster_hunters/start()
	for(var/mob/living/carbon/human/all_players in GLOB.player_list)
		if(iscultist(all_players) || IS_HERETIC(all_players) || IS_WIZARD(all_players) || is_servant_of_ratvar(all_players) || all_players.mind.has_antag_datum(/datum/antagonist/changeling))
	...
	for(var/mob/living/carbon/human/all_players in shuffle(GLOB.player_list))
		...
		if(IS_HERETIC(all_players) || iscultist(all_players) || IS_WIZARD(all_players) || is_servant_of_ratvar(all_players) || all_players.mind.has_antag_datum(/datum/antagonist/changeling))
			continue
		if(!all_players.getorgan(/obj/item/organ/brain))
			continue
		all_players.mind.add_antag_datum(/datum/antagonist/monsterhunter)
