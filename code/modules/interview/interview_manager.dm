...
/datum/interview_manager/proc/enqueue(datum/interview/to_queue)
	...
	var/admins_present = send2discord_adminless_only("panic-bunker-interview", "Interview for [ckey] enqueued for review. Current position in queue: [to_queue.pos_in_queue]")
	...
	for(var/client/X in GLOB.admins)
		...
		to_chat(X, "<span class='adminhelp'>Interview for [ckey] enqueued for review. Current position in queue: [to_queue.pos_in_queue]</span>")
