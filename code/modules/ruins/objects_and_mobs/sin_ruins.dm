...
/obj/structure/cursed_slot_machine/interact(mob/living/carbon/human/user)
	...
	if(user.stat)
		...
		user.visible_message("<span class='warning'>[user] дергает [src] рычаг с блеском в [user.ru_ego()] глахах!</span>" , "<span class='warning'>Чувствую истощение и слабость во всем теле как только дергаете рычаг, но вы \
