...
/obj/machinery/power/apc/proc/ethereal_interact(mob/living/user, list/modifiers)
	...
	if(ethereal.a_intent == INTENT_HARM)
		if(cell.charge <= (cell.maxcharge / 2)) // ethereals can't drain APCs under half charge, this is so that they are forced to look to alternative power sources if the station is running low
			addtimer(CALLBACK(src, /atom.proc/balloon_alert, ethereal, "мало заряда!"), alert_timer_duration)
			return
		if(stomach.crystal_charge > charge_limit)
			addtimer(CALLBACK(src, /atom.proc/balloon_alert, ethereal, "заряд полный!"), alert_timer_duration)
			return
		stomach.drain_time = world.time + APC_DRAIN_TIME
		addtimer(CALLBACK(src, /atom.proc/balloon_alert, ethereal, "выкачиваем энергию"), alert_timer_duration)
	...
	if(cell.charge >= cell.maxcharge - APC_POWER_GAIN)
		addtimer(CALLBACK(src, /atom.proc/balloon_alert, ethereal, "энергощиток не хочет больше принимать!"), alert_timer_duration)
		return
	if(stomach.crystal_charge < APC_POWER_GAIN)
		addtimer(CALLBACK(src, /atom.proc/balloon_alert, ethereal, "заряд слишком мал!"), alert_timer_duration)
		return
	stomach.drain_time = world.time + APC_DRAIN_TIME
	addtimer(CALLBACK(src, /atom.proc/balloon_alert, ethereal, "передаём энергию"), alert_timer_duration)
	if(!do_after(user, APC_DRAIN_TIME, target = src))
		return
