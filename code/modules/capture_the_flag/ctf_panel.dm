...
/datum/ctf_panel/ui_data(mob/user)
	...
	for(var/obj/machinery/capture_the_flag/team in GLOB.machines)
		...
		if(!data["enabled"])
			if(team.ctf_enabled)
				data["enabled"] = "Захват флага уже включен!"
			else
				data["enabled"] = "Захват флага требует [CTF_REQUIRED_PLAYERS] активных игроков для начала, пока что [team.people_who_want_to_play.len]/[CTF_REQUIRED_PLAYERS] готово!"
	return data
