...
/datum/surgery_step/filter_blood/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("Начинаю фильтрацию крови [sklonenie(target.name, RODITELNI, target.gender)]...") ,
		span_notice("[user] использует [tool] для фильтрации моей крови.") ,
		span_notice("[user] использует [tool] на груди [sklonenie(target.name, RODITELNI, target.gender)]."))
...
/datum/surgery_step/filter_blood/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	...
	display_results(user, target, span_notice("Закончив фильтрацию крови [sklonenie(target.name, RODITELNI, target.gender)] [tool] издает короткий звон.") ,
...
/datum/surgery_step/filter_blood/failure(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_warning("[gvorno(TRUE)], но [gvorno(TRUE)], но я облажался, оставив синяк на груди [sklonenie(target.name, RODITELNI, target.gender)]!") ,
		span_warning("[user] облажался, оставив синяк на груди [sklonenie(target.name, RODITELNI, target.gender)]!") ,
