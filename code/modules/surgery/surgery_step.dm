	...
	if(success)
		if(target_zone == surgery.location)
			if(get_location_accessible(target, target_zone) || surgery.ignore_clothes)
				...
				to_chat(user, span_warning("Мне нужно иметь доступ к [parse_zone(target_zone)] <b>[sklonenie(target.name, DATELNI, target.gender)]</b> для проведения хирургической операции!"))
...
/datum/surgery_step/proc/preop(mob/user, mob/living/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("Начинаю выполнять операцию на <b>[sklonenie(target.name, PREDLOZHNI, target.gender)]</b>...") ,
		span_notice("<b>[user]</b> начинает выполнять операцию на <b>[sklonenie(target.name, PREDLOZHNI, target.gender)]</b>.") ,
		span_notice("<b>[user]</b> начинает выполнять операцию на <b>[sklonenie(target.name, PREDLOZHNI, target.gender)]</b>."))
