...
/datum/surgery_step/fix_eyes/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("Начинаю исправлять глаза [sklonenie(target.name, RODITELNI, target.gender)]...") ,
...
/datum/surgery_step/fix_eyes/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	var/obj/item/organ/eyes/E = target.getorganslot(ORGAN_SLOT_EYES)
	user.visible_message(span_notice("[user] успешно исправил [target.ru_who()] глаза!") , span_notice("Успешно исправил глаз [sklonenie(target.name, RODITELNI, target.gender)]."))
	display_results(user, target, span_notice("Успешно исправил глаза [sklonenie(target.name, RODITELNI, target.gender)].") ,
...
/datum/surgery_step/fix_eyes/failure(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(target.getorgan(/obj/item/organ/brain))
		display_results(user, target, span_warning("Случайно уколол [sklonenie(target.name, RODITELNI, target.gender)] прямо в мозг!") ,
			span_warning("[user] случайно уколол [target.ru_who()] прямо в мозг!") ,
			span_warning("[user] случайно уколол [target.ru_who()] прямо в мозг!"))
		target.adjustOrganLoss(ORGAN_SLOT_BRAIN, 70)
	else
		display_results(user, target, span_warning("Случайно уколол [sklonenie(target.name, RODITELNI, target.gender)] прямо в мозг! Ну, точнее уколол бы, если бы у [target.ru_who()] был мозг.") ,
