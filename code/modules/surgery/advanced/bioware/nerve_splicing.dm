...
/datum/surgery_step/splice_nerves/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, "<span class='notice'>Начинаю соединять между собой нервы [skloname(target.name, RODITELNI, target.gender)].</span>" ,
		"<span class='notice'>[user] начал соединять между собой нервы [skloname(target.name, RODITELNI, target.gender)].</span>" ,
		"<span class='notice'>[user] начал работать с нервной системой [skloname(target.name, RODITELNI, target.gender)].</span>")
...
/datum/surgery_step/splice_nerves/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(user, target, "<span class='notice'>Успешно срастил нервную систему [skloname(target.name, RODITELNI, target.gender)]!</span>" ,
		"<span class='notice'>[user] успешно переплел нервную систему [skloname(target.name, RODITELNI, target.gender)]!</span>" ,
		"<span class='notice'>[user] закончил работать с нервной системой [skloname(target.name, RODITELNI, target.gender)].</span>")
