...
/datum/surgery_step/imprint_cortex/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("Начинаю вырезать на коре большого мозга [sklonenie(target.name, RODITELNI, target.gender)] само-импринтирующий паттерн.") ,
		span_notice("[user] начинает вырезать на коре большого мозга [sklonenie(target.name, RODITELNI, target.gender)] само-импринтирующий паттерн.") ,
		span_notice("[user] начинает проводить операцию на мозге [sklonenie(target.name, RODITELNI, target.gender)]."))
...
/datum/surgery_step/imprint_cortex/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(user, target, span_notice("Изменил форму коры большого мозга [sklonenie(target.name, RODITELNI, target.gender)] на само-импринтирующий паттерн!") ,
		span_notice("[user] изменил форму коры большого мозга [sklonenie(target.name, RODITELNI, target.gender)] на само-импринтирующий паттерн!") ,
		span_notice("[user] завершил операцию на мозге [sklonenie(target.name, RODITELNI, target.gender)]."))
