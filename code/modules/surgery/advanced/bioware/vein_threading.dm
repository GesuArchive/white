...
/datum/surgery_step/thread_veins/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, "<span class='notice'>Начинаю переплетать кровеносную систему [skloname(target.name, RODITELNI, target.gender)].</span>" ,
		"<span class='notice'>[user] начал переплетать кровеносную систему [skloname(target.name, RODITELNI, target.gender)].</span>" ,
		"<span class='notice'>[user] начал работать над кровеносной системой [skloname(target.name, RODITELNI, target.gender)].</span>")
...
/datum/surgery_step/thread_veins/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(user, target, "<span class='notice'>Сплел кровеносную систему [skloname(target.name, RODITELNI, target.gender)] в прочную сеть!</span>" ,
		"<span class='notice'>[user] сплел кровеносную систему [skloname(target.name, RODITELNI, target.gender)] в прочную сеть!</span>" ,
		"<span class='notice'>[user] закончил работать над кровеносной системой [skloname(target.name, RODITELNI, target.gender)].</span>")
