...
/datum/surgery_step/muscled_veins/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("Начинаю накручивать мышцы вокруг кровеносной системы [sklonenie(target.name, RODITELNI, target.gender)].") ,
		span_notice("[user] начал накручивать мышцы вокруг кровеносной системы [sklonenie(target.name, RODITELNI, target.gender)].") ,
		span_notice("[user] начал работать с кровеносной системой [sklonenie(target.name, RODITELNI, target.gender)]."))
...
/datum/surgery_step/muscled_veins/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(user, target, span_notice("Изменил кровеносную систему [sklonenie(target.name, RODITELNI, target.gender)], добавив мышечные мембраны!") ,
		span_notice("[user] изменил кровеносную систему [sklonenie(target.name, RODITELNI, target.gender)], добавив мышечные мембраны!") ,
		span_notice("[user] закончил работать с кровеносной системой [sklonenie(target.name, RODITELNI, target.gender)]."))
