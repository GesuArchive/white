...
/datum/surgery_step/muscled_veins/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, "<span class='notice'>Начинаю накручивать мышцы вокруг кровеносной системы [skloname(target.name, RODITELNI, target.gender)].</span>" ,
		"<span class='notice'>[user] начал накручивать мышцы вокруг кровеносной системы [skloname(target.name, RODITELNI, target.gender)].</span>" ,
		"<span class='notice'>[user] начал работать с кровеносной системой [skloname(target.name, RODITELNI, target.gender)].</span>")
...
/datum/surgery_step/muscled_veins/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(user, target, "<span class='notice'>Изменил кровеносную систему [skloname(target.name, RODITELNI, target.gender)], добавив мышечные мембраны!</span>" ,
		"<span class='notice'>[user] изменил кровеносную систему [skloname(target.name, RODITELNI, target.gender)], добавив мышечные мембраны!</span>" ,
		"<span class='notice'>[user] закончил работать с кровеносной системой [skloname(target.name, RODITELNI, target.gender)].</span>")
