...
/datum/surgery_step/reinforce_ligaments/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("Начинаю укреплять связки [sklonenie(target.name, RODITELNI, target.gender)].") ,
		span_notice("[user] начал укреплять связки [sklonenie(target.name, RODITELNI, target.gender)].") ,
		span_notice("[user] начал работу со связками [sklonenie(target.name, RODITELNI, target.gender)]."))
...
/datum/surgery_step/reinforce_ligaments/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(user, target, span_notice("Укрепил связки [sklonenie(target.name, RODITELNI, target.gender)]!") ,
		span_notice("[user] укрепил связки [sklonenie(target.name, RODITELNI, target.gender)]!") ,
		span_notice("[user] закончил работу со связками [sklonenie(target.name, RODITELNI, target.gender)]."))
