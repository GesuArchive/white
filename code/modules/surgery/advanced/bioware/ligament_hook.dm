...
/datum/surgery_step/reshape_ligaments/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("Начинаю менять форму связок [sklonenie(target.name, RODITELNI, target.gender)] на крючкообразную.") ,
		span_notice("[user] начал менять форму связок [sklonenie(target.name, RODITELNI, target.gender)] на крючкообразную.") ,
		span_notice("[user] начал работать над связками [sklonenie(target.name, RODITELNI, target.gender)]."))
...
/datum/surgery_step/reshape_ligaments/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(user, target, span_notice("Изменил форму связок [sklonenie(target.name, RODITELNI, target.gender)] на соединяющий крючок!") ,
		span_notice("[user] сформировал из связок [sklonenie(target.name, RODITELNI, target.gender)] соединяющий крючок!") ,
		span_notice("[user] закончил работу над связками [sklonenie(target.name, RODITELNI, target.gender)]."))
