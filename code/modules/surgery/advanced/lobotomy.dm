...
/datum/surgery_step/lobotomize/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("Начинаю проведение лоботомии на мозге [sklonenie(target.name, RODITELNI, target.gender)]...") ,
		span_notice("[user] начинает проведение лоботомии на мозге [sklonenie(target.name, RODITELNI, target.gender)].") ,
...
/datum/surgery_step/lobotomize/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(user, target, span_notice("Успешно выполнил лоботомию [sklonenie(target.name, RODITELNI, target.gender)].") ,
			span_notice("[user] успешно выполнил лоботомию [sklonenie(target.name, RODITELNI, target.gender)]!") ,
			span_notice("[user] завершает операцию [sklonenie(target.name, RODITELNI, target.gender)]."))
