...
/datum/surgery_step/lobotomize
	...
	implements = list(TOOL_SCALPEL = 85, /obj/item/melee/energy/sword = 55, /obj/item/kitchen/knife = 35,
...
/datum/surgery_step/lobotomize/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, "<span class='notice'>Начинаю проведение лоботомии на мозге [skloname(target.name, RODITELNI, target.gender)]...</span>" ,
		"<span class='notice'>[user] начинает проведение лоботомии на мозге [skloname(target.name, RODITELNI, target.gender)].</span>" ,
...
/datum/surgery_step/lobotomize/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(user, target, "<span class='notice'>Успешно выполнил лоботомию [skloname(target.name, RODITELNI, target.gender)].</span>" ,
			"<span class='notice'>[user] успешно выполнил лоботомию [skloname(target.name, RODITELNI, target.gender)]!</span>" ,
			"<span class='notice'>[user] завершает операцию [skloname(target.name, RODITELNI, target.gender)].</span>")
...
/datum/surgery_step/lobotomize/failure(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(B)
		display_results(user, target, "<span class='warning'>Извлек неверную часть, что привело к большим повреждениям!</span>" ,
			"<span class='notice'>[user] успешно выполнил лоботомию [skloname(target.name, RODITELNI, target.gender)]!</span>" ,
			"<span class='notice'>[user] завершает операцию на мозге [skloname(target.name, RODITELNI, target.gender)].</span>")
