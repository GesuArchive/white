...
/datum/surgery_step/insert_pill/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, "<span class='notice'>Начинаю помещать [tool] в [parse_zone(target_zone)] [skloname(target.name, RODITELNI, target.gender)]...</span>" ,
		"<span class='notice'>[user] начинет вводить [tool] в [parse_zone(target_zone)] [skloname(target.name, RODITELNI, target.gender)].</span>" ,
		"<span class='notice'>[user] начинат вводить что-то в [parse_zone(target_zone)] [skloname(target.name, RODITELNI, target.gender)] .</span>")
...
/datum/surgery_step/insert_pill/success(mob/user, mob/living/carbon/target, target_zone, obj/item/reagent_containers/pill/tool, datum/surgery/surgery, default_display_results = FALSE)
	...
	display_results(user, target, "<span class='notice'>Ввёл [tool] в [parse_zone(target_zone)] [skloname(target.name, RODITELNI, target.gender)] .</span>" ,
			"<span class='notice'>[user] ввёл [tool] в [parse_zone(target_zone)] [skloname(target.name, RODITELNI, target.gender)]!</span>" ,
			"<span class='notice'>[user] ввёл что-то в [parse_zone(target_zone)] [skloname(target.name, RODITELNI, target.gender)] !</span>")
	return ..()
...
/datum/action/item_action/hands_free/activate_pill/Trigger(trigger_flags)
	...
	to_chat(owner, "<span class='notice'>Сжимаю зубы и разжёвываю имплант [item_target.name]!</span>")
