...
/datum/surgery_step/sever_limb/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("Начинаю отрезать [ru_parse_zone(parse_zone(target_zone))] [sklonenie(target.name, RODITELNI, target.gender)]...") ,
		span_notice("[user] начинает отрезать [ru_parse_zone(parse_zone(target_zone))] [sklonenie(target.name, RODITELNI, target.gender)]!") ,
		span_notice("[user] начинает отрезать [ru_parse_zone(parse_zone(target_zone))] [sklonenie(target.name, RODITELNI, target.gender)]!"))
...
//	var/mob/living/carbon/human/L = target
	display_results(user, target, span_notice("Отрезал [ru_parse_zone(parse_zone(target_zone))] [sklonenie(target.name, RODITELNI, target.gender)].") ,
		span_notice("[user] отрезал [ru_parse_zone(parse_zone(target_zone))] [sklonenie(target.name, RODITELNI, target.gender)]!") ,
		span_notice("[user] отрезал [ru_parse_zone(parse_zone(target_zone))] [sklonenie(target.name, RODITELNI, target.gender)]!"))
