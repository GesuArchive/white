...
/datum/surgery_step/fix_brain/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(user, target, span_notice("Начинаю исправлять мозг [sklonenie(target.name, RODITELNI, target.gender)]...") ,
...
/datum/surgery_step/fix_brain/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(user, target, span_notice("Успешно исправил мозг [sklonenie(target.name, RODITELNI, target.gender)].") ,
	...
	if(target.getOrganLoss(ORGAN_SLOT_BRAIN) > 0)
		to_chat(user, "Похоже, что в мозгу [sklonenie(target.name, RODITELNI, target.gender)] всё еще можно что-то исправить.")
	return ..()
