...
/datum/surgery_step/repair_innards/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(pierce_wound.blood_flow <= 0)
		to_chat(user, span_notice("[target] [parse_zone(user.zone_selected)] has no puncture to repair!"))
		surgery.status++
		return

	display_results(user, target, span_notice("You begin to realign the torn blood vessels in [target] [parse_zone(user.zone_selected)]...") ,
		span_notice("[user] begins to realign the torn blood vessels in [target] [parse_zone(user.zone_selected)] with [tool].") ,
		span_notice("[user] begins to realign the torn blood vessels in [target] [parse_zone(user.zone_selected)]."))
...
/datum/surgery_step/repair_innards/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	...
	display_results(user, target, span_notice("You successfully realign some of the blood vessels in [target] [parse_zone(target_zone)].") ,
		span_notice("[user] successfully realigns some of the blood vessels in [target] [parse_zone(target_zone)] with [tool]!") ,
		span_notice("[user] successfully realigns some of the blood vessels in [target] [parse_zone(target_zone)]!"))
...
/datum/surgery_step/seal_veins/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(!pierce_wound)
		user.visible_message(span_notice("[user] looks for [target] [parse_zone(user.zone_selected)].") , span_notice("You look for [target] [parse_zone(user.zone_selected)]..."))
		return
	display_results(user, target, span_notice("You begin to meld some of the split blood vessels in [target] [parse_zone(user.zone_selected)]...") ,
		span_notice("[user] begins to meld some of the split blood vessels in [target] [parse_zone(user.zone_selected)] with [tool].") ,
		span_notice("[user] begins to meld some of the split blood vessels in [target] [parse_zone(user.zone_selected)]."))
...
/datum/surgery_step/seal_veins/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	...
	display_results(user, target, span_notice("You successfully meld some of the split blood vessels in [target] [parse_zone(target_zone)] with [tool].") ,
		span_notice("[user] successfully melds some of the split blood vessels in [target] [parse_zone(target_zone)] with [tool]!") ,
		span_notice("[user] successfully melds some of the split blood vessels in [target] [parse_zone(target_zone)]!"))
	...
	if(pierce_wound.blood_flow > 0)
		...
		to_chat(user, span_green("You've repaired all the internal damage in [target] [parse_zone(target_zone)]!"))
	return ..()
