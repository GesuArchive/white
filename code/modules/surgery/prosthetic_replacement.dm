...
/datum/surgery_step/add_prosthetic/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(istype(tool, /obj/item/bodypart))
		...
		if(target_zone == BP.body_zone) //so we can't replace a leg with an arm, or a human arm with a monkey arm.
			display_results(user, target, span_notice("Вы начинаете заменять [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)] на [tool]...") ,
				span_notice("[user] начинает заменять [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)] на [tool].") ,
				span_notice("[user] начинает заменять [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)]."))
		else
		...
		display_results(user, target, span_notice("Вы начинаете присоединять [tool] к [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)]...") ,
			span_notice("[user] начинает присоединять [tool] к [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)].") ,
			span_notice("[user] начинает присоединять [tool] к [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)]."))
	else
...
/datum/surgery_step/add_prosthetic/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	...
	if(istype(tool, /obj/item/bodypart) && user.temporarilyRemoveItemFromInventory(tool))
		...
		if(!L.attach_limb(target))
			display_results(user, target, span_warning("Вам не удалось заменить [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)]! Тело отвергает [L]!") ,
				span_warning("[user] не удалось заменить [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)]!") ,
				span_warning("[user] не удалось заменить [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)]!"))
			return
		if(organ_rejection_dam)
			target.adjustToxLoss(organ_rejection_dam)
		display_results(user, target, span_notice("Вы успешно заменили [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)] на [tool].") ,
			span_notice("[user] успешно заменил [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)] на [tool]!") ,
			span_notice("[user] успешно заменил [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)]!"))
		return
	else
		var/obj/item/bodypart/L = target.newBodyPart(target_zone, FALSE, FALSE)
		L.is_pseudopart = TRUE
		if(!L.attach_limb(target))
			display_results(user, target, span_warning("Вам не удалось присоединить [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)]! Тело отвергает [L]!") ,
				span_warning("[user] не удалось присоединить [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)]!") ,
				span_warning("[user] не удалось присоединить [parse_zone(target_zone)] [sklonenie(target.name, RODITELNI, target.gender)]!"))
