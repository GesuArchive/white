...
/obj/item/paper_bin/attack_hand(mob/user, list/modifiers)
	...
	if(bin_pen)
		...
		to_chat(user, span_notice("Беру [pen] из <b>[src.name]</b>."))
		bin_pen = null
		update_appearance()
		...
		to_chat(user, span_notice("Беру [top_paper] из <b>[src.name]</b>."))
		update_appearance()
	else
...
/obj/item/paper_bin/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/paper))
		...
		if(!user.transferItemToLoc(paper, src))
			return
		to_chat(user, span_notice("Кладу [paper] в [src]."))
		paper_stack += paper
		total_paper += 1
		update_appearance()
		...
		if(!user.transferItemToLoc(pen, src))
			return
		to_chat(user, span_notice("Кладу [pen] в [src]."))
		bin_pen = pen
		update_appearance()
	else
		return ..()
