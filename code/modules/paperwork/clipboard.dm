...
/obj/item/clipboard/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/paper))
		if(!user.transferItemToLoc(W, src))
			return
		toppaper = W
		to_chat(user, "<span class='notice'>Зажимаю бумагу на <b>[src.name]</b>.</span>")
		update_icon()
	else if(toppaper)
...
/obj/item/clipboard/Topic(href, href_list)
	...
	if(usr.contents.Find(src))
			...
			if(!haspen)
				...
				if(istype(held, /obj/item/pen))
					...
					if(!usr.transferItemToLoc(W, src))
						return
					haspen = W
					to_chat(usr, "<span class='notice'>Вставляю [W] в [src].</span>")
		...
		if(href_list["top"])
			...
			if(istype(P))
				toppaper = P
				to_chat(usr, "<span class='notice'>Перемещаю [P.name] наверх.</span>")
