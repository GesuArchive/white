...
/obj/machinery/photocopier/ui_act(action, list/params)
	...
	switch(action)
		...
		if("make_copy")
			if(busy)
				to_chat(usr, span_warning("[capitalize(src.name)] is currently busy copying something. Please wait until it is finished."))
				return FALSE
			if(paper_copy)
				if(!length(paper_copy.info))
					to_chat(usr, span_warning("An error message flashes across [src] screen: \"The supplied paper is blank. Aborting.\""))
					return FALSE
				// Basic paper
...
/obj/machinery/photocopier/proc/make_ass_copy()
	...
	if(ishuman(ass) && (ass.get_item_by_slot(ITEM_SLOT_ICLOTHING) || ass.get_item_by_slot(ITEM_SLOT_OCLOTHING)))
		to_chat(usr, span_notice("You feel kind of silly, copying [ass == usr ? "your" : ass][ass == usr ? "" : "\'s"] ass with [ass == usr ? "your" : "[ass.ru_ego()]"] clothes on.")  )
		return
...
/obj/machinery/photocopier/attackby(obj/item/O, mob/user, params)
	if(default_unfasten_wrench(user, O))
		...
		if(toner_cartridge)
			to_chat(user, span_warning("[capitalize(src.name)] already has a toner cartridge inserted. Remove that one first."))
			return
		O.forceMove(src)
		toner_cartridge = O
