...
/obj/item/paper/contract/employment/update_text()
	name = "бумага- [target] трудовой договор"
...
/obj/item/paper/contract/infernal/proc/attempt_signature(mob/living/carbon/human/user, blood = 0)
	if((!ISADVANCEDTOOLUSER(user)) || !user.is_literate())
		to_chat(user, span_warning("You don't know how to read or write!"))
		return FALSE
	if(user.mind != target)
		to_chat(user, span_warning("Your signature simply slides off the sheet, it seems this contract is not meant for you to sign!"))
		return FALSE
	if(user.mind.soulOwner == owner)
		to_chat(user, span_warning("This devil already owns your soul, you may not sell it to [owner.p_them()] again!"))
		return FALSE
	if(signed)
		to_chat(user, span_warning("This contract has already been signed! It may not be signed again."))
		return FALSE
	...
	if (contractType == CONTRACT_REVIVE)
		to_chat(user, span_warning("You are already alive, this contract would do nothing."))
		return FALSE
	to_chat(user, span_notice("You quickly scrawl your name on the contract."))
...
/obj/item/paper/contract/infernal/revive/attack(mob/M, mob/living/user)
	if (target == M.mind && M.stat == DEAD && M.mind.soulOwner == M.mind)
		if (cooldown)
			to_chat(user, span_warning("Give [M] a chance to think through the contract, don't rush [M.p_them()]!"))
			return
		cooldown = TRUE
		...
		if(ghost)
			...
			response = tgalert(ghost, "A devil is offering you another chance at life, at the price of your soul, do you accept?", "Infernal Resurrection", list("Yes", "No"), "Never for this round", 0, 200)
			if(!ghost)
				return		//handle logouts that happen whilst the alert is waiting for a response.
		else
			response = tgalert(target.current, "A devil is offering you another chance at life, at the price of your soul, do you accept?", "Infernal Resurrection", list("Yes", "No"), "Never for this round", 0, 200)
		if(response == "Yes")
...
/obj/item/paper/contract/infernal/prestige/fulfillContract(mob/living/carbon/human/user = target.current, blood = 0)
	...
	id = new /obj/item/card/id/advanced/gold/captains_spare(drop_location())
