...
/obj/item/papercutter/suicide_act(mob/user)
	if(storedcutter)
		user.visible_message("<span class='suicide'>[user] is beheading [user.ru_na()]self with [src.name]! It looks like [user.p_theyre()] trying to commit suicide!</span>")
		if(iscarbon(user))
		...
		user.visible_message("<span class='suicide'>[user] repeatedly bashes [src.name] against [user.ru_ego()] head! It looks like [user.p_theyre()] trying to commit suicide!</span>")
...
/obj/item/papercutter/attackby(obj/item/P, mob/user, params)
	...
	if(istype(P, /obj/item/hatchet/cutterblade) && !storedcutter)
		if(!user.transferItemToLoc(P, src))
			return
		to_chat(user, "<span class='notice'>You replace [src] [P].</span>")
		P.forceMove(src)
		storedcutter = P
		update_icon()
		return
...
/obj/item/papercutter/attack_hand(mob/user)
	...
	if(!cuttersecured)
		to_chat(user, "<span class='notice'>You remove [src] [storedcutter].</span>")
