...
/obj/machinery/fax/Initialize(mapload)
	...
	if (!fax_name)
		if(mapload)
			fax_name = get_area_name(src, TRUE)
		else
...
/obj/machinery/fax/multitool_act(mob/living/user, obj/item/I)
	var/new_fax_name = tgui_input_text(user, "Как назовём наш факс?", "Новое имя факса", , 128)
	if(!new_fax_name)
		return TOOL_ACT_TOOLTYPE_SUCCESS
...
/obj/machinery/fax/proc/send(obj/item/paper/paper, id)
	for(var/obj/machinery/fax/FAX in GLOB.machines)
		if (FAX.fax_id == id)
			...
			history_add("Отправка", FAX.fax_name)
			flick("fax_send", src)
...
/obj/machinery/fax/proc/receive(obj/item/paper/paper, sender_name)
	...
	say("Принимаем корреспонденцию от [sender_name].")
	history_add("Приём", sender_name)
	paper.forceMove(drop_location())
 ...
 */
/obj/machinery/fax/proc/history_add(history_type = "Отправка", history_fax_name)
