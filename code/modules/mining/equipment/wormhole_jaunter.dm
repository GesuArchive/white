...
/obj/item/wormhole_jaunter/emp_act(power)
	...
	if(istype(M))
		...
		if(triggered)
			M.visible_message(span_warning("[capitalize(src.name)] overloads and activates!"))
...
/obj/item/wormhole_jaunter/proc/chasm_react(mob/user)
	if(user.get_item_by_slot(ITEM_SLOT_BELT) == src)
		...
		to_chat(user, span_userdanger("[capitalize(src.name)] is not attached to your belt, preventing it from saving you from the chasm. RIP."))
