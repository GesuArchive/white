...
/area/survivalpod
	...
	area_flags = BLOBS_ALLOWED | UNIQUE_AREA | HIDDEN_AREA
...
/obj/item/survivalcapsule/attack_self()
	...
	get_template()
	if(!used)
		loc.visible_message(span_warning("<b>[capitalize(src)]</b> begins to shake. Stand back!"))
		used = TRUE
		sleep(50)
		...
		switch(status)
			if(SHELTER_DEPLOY_BAD_AREA)
				src.loc.visible_message(span_warning("<b>[capitalize(src)]</b> will not function in this area."))
			if(SHELTER_DEPLOY_BAD_TURFS, SHELTER_DEPLOY_ANCHORED_OBJECTS)
				var/width = template.width
				var/height = template.height
				src.loc.visible_message(span_warning("<b>[capitalize(src)]</b> doesn't have room to deploy! You need to clear a [width]x[height] area!"))
