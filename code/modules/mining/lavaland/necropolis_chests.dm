...
/datum/design/unique_modkit
	category = list("Шахтёрское оборудование", "Улучшения киборгов") //can't be normally obtained
...
/obj/item/melee/ghost_sword
	...
	attack_verb_continuous = list("атакует", "рубит", "втыкает", "режет", "кромсает", "разрывает", "нарезает", "душманит")
	attack_verb_simple = list("атакует", "рубит", "втыкает", "режет", "кромсает", "разрывает", "нарезает", "душманит")
		...
		if(!istype(T, turf_type))
			...
			if(do_after(user, create_delay, target = T))
				...
				if(T.TerraformTurf(turf_type, flags = CHANGETURF_INHERIT_AIR))
					user.visible_message("<span class='danger'>[user] turns [old_name] into [transform_string]!</span>")
			...
			if(T.TerraformTurf(reset_turf_type, flags = CHANGETURF_INHERIT_AIR))
				user.visible_message("<span class='danger'>[user] turns [old_name] into [reset_string]!</span>")
...
/datum/action/innate/dash/hierophant/charge()
	...
	to_chat(owner, "<span class='notice'>[capitalize(src.name)] now has [current_charges]/[max_charges] charges.</span>")
...
/obj/item/hierophant_club
	...
	force = 15
	attack_verb_continuous = list("бьёт", "лупит", "пульверизирует")
	attack_verb_simple = list("бьёт", "лупит", "пульверизирует")
