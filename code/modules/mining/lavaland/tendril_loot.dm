...
/obj/item/rod_of_asclepius/proc/activated()
	...
	desc = "Небольшой деревянный жезл с мистической змеей, обвившей жезл и мою руку. Он источает целительную энергию, которая окутывает меня и всех окружающих. Я чувствую, что способен одним только касанием поднять мертвеца!"
...
/obj/item/rod_of_asclepius/attack(mob/living/M, mob/living/user)
	if(activated)
		if(M.stat == DEAD)
			...
			addtimer(CALLBACK(M, TYPE_PROC_REF(/mob/living, revive), FALSE, FALSE, 10), 79)
			..()
		else
...
/obj/item/cursed_katana/proc/strike(mob/living/target, mob/user)
	...
	playsound(src, 'sound/weapons/genhit3.wav', 50, TRUE)
