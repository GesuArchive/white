/obj/structure/world_anvil
	...
	desc = "Наковальня, которая связана через лавовые водохранилища к ядру Лаваленда. Тот, кто использовал это последним, создавал что-то мощное."
...
/obj/structure/world_anvil/attack_hand(mob/user)
	if(!LAZYLEN(placed_objects))
		to_chat(user,"Нужно разместить кусок плазменного макмита и кинетический ускоритель или передовой плазменный резак на наковальне!")
		return ..()
	if(forge_charges <= 0)
	...
	for(var/obj/item/magmite/placed_magmite in placed_objects)
		magmite_amount++
	if(magmite_amount <= 0)
		to_chat(user,"На наковальне нет ни одного плазменного макмита!")
		return ..()
