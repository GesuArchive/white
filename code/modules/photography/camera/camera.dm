...
/obj/item/camera/examine(mob/user)
	. = ..()
	. += "<hr><span class='notice'>ПКМ to change its focusing, allowing you to set how big of an area it will capture.</span>"
...
/obj/item/camera/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/camera_film))
		if(pictures_left)
			to_chat(user, span_notice("[capitalize(src.name)] still has some film in it!") )
			return
...
/obj/item/camera/proc/printpicture(mob/user, datum/picture/picture) //Normal camera proc for creating photos
	...
	if(in_range(src, user)) //needed because of TK
		...
		var/customise = "No"
		if(can_customise)
			customise = tgui_alert(user, "Do you want to customize the photo?", "Customization", "Yes", "No")
		if(customise == "Yes")
