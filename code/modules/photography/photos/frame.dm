...
/obj/item/wallframe/picture/attack_hand(mob/user)
	...
	if(contents.len)
		...
		user.put_in_hands(I)
		to_chat(user, "<span class='notice'>Аккуратно достаю фотографию из <b>[src.name]</b>.</span>")
		displayed = null
		update_icon()
	return ..()
...
/obj/structure/sign/picture_frame/attackby(obj/item/I, mob/user, params)
	if(can_decon && (I.tool_behaviour == TOOL_SCREWDRIVER || I.tool_behaviour == TOOL_WRENCH))
		to_chat(user, "<span class='notice'>Начинаю разбирать [name]...</span>")
		if(I.use_tool(src, user, 30, volume=50))
			playsound(loc, 'sound/items/deconstruct.ogg', 50, TRUE)
			to_chat(user, "<span class='notice'>Разбираю [name].</span>")
			deconstruct()

	else if(I.tool_behaviour == TOOL_WIRECUTTER && framed)
		framed.forceMove(drop_location())
		framed = null
		user.visible_message("<span class='warning'>[user] снимает [framed] с [src]!</span>")
		return
