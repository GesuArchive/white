...
/obj/machinery/conveyor/attackby(obj/item/I, mob/user, params)
	if(I.tool_behaviour == TOOL_CROWBAR)
		user.visible_message(span_notice("[user] struggles to pry up <b>[src.name]</b> with [I].") , \
		span_notice("You struggle to pry up <b>[src.name]</b> with [I]."))
		...
		if(!(machine_stat & BROKEN))
			verted = verted * -1
			update_move_direction()
			to_chat(user, span_notice("You reverse [src] direction."))
...
/obj/machinery/conveyor_switch/examine(mob/user)
	. = ..()
	. += "<hr><span class='notice'>[capitalize(src.name)] is set to [oneway ? "one way" : "default"] configuration. It can be changed with <b>screwdriver</b>.</span>"
	. += "\n<span class='notice'>[capitalize(src.name)] is set to [invert_icon ? "inverted": "normal"] position. It can be rotated with <b>wrench</b>.</span>"
