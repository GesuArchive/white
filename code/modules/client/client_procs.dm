...
/client/AllowUpload(filename, filelength)
	if (holder)
		if(filelength > UPLOAD_LIMIT_ADMIN)
			to_chat(src, span_red("Error: AllowUpload(): File Upload too large. Upload Limit: [UPLOAD_LIMIT_ADMIN/1024]KiB."))
			return FALSE
	...
	if (oldcid)
		...
		if (oldcid != computer_id && computer_id != lastcid) //IT CHANGED!!!
			...
			if (!cidcheck_failedckeys[ckey])
				...
				send2discord_adminless_only("CidRandomizer", "[key_name(src)] has been detected as using a cid randomizer. Connection rejected.")
			...
			if (cidcheck_failedckeys[ckey])
				...
				send2discord_adminless_only("CidRandomizer", "[key_name(src)] has been allowed to connect after showing they removed their cid randomizer.")
...
/client/proc/cid_check_reconnect()
	...
	to_chat(src, {"<a href="byond://[url]?token=[token]"><font size=+8>Нажми на это сообщение.</font></a>"})
...
/client/proc/award_heart(heart_reason)
	to_chat(src, span_nicegreen("Someone awarded you a heart![heart_reason ? " They said: [heart_reason]!" : ""]"))
	if(!src)
		return
