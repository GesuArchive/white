...
/client/AllowUpload(filename, filelength)
	if (holder)
		if(filelength > UPLOAD_LIMIT_ADMIN)
			to_chat(src, "<span class='red'>Error: AllowUpload(): File Upload too large. Upload Limit: [UPLOAD_LIMIT_ADMIN/1024]KiB.</span>")
			return FALSE
...
/client/New(TopicData)
	...
	if(prefs?.lastchangelog != GLOB.changelog_hash)
		to_chat(src, "<span class='info'>Тут всякие обновления были, прочитай список изменений обязательно.</span>")
		mob.view_changelog()
