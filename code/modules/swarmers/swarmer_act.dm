...
/obj/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	...
	for(var/mob/living/living_content in contents)
		if(issilicon(living_content) || isbrain(living_content))
			continue
		to_chat(actor, "<span class='warning'>Внутри объекта обнаружен организм. Прерывание.</span>")
		return FALSE
	return ..()
...
/obj/structure/swarmer_beacon/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Эта машина нужна для дальнейшего размножения роевиков. Прерывание.</span>")
	return FALSE
...
/obj/machinery/door/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	...
	for(var/turf/turf_in_range in range(1, src))
		...
		if (turf_in_range.return_turf_delta_p() > DANGEROUS_DELTA_P)
			to_chat(actor, "<span class='warning'>Уничтожение этого объекта может вызвать взрывной сброс давления. Прерывание.</span>")
			actor.target = null
			return TRUE
		//Check if breaking this door will expose the station to space/planetary atmos
			...
			to_chat(actor, "<span class='warning'>Уничтожение этого объекта может привести к повреждению корпуса. Прерывание.</span>")
			actor.target = null
			return FALSE
		//Check if this door is important in supermatter containment
		else if(istype(turf_area, /area/engineering/supermatter))
			to_chat(actor, "<span class='warning'>Нарушение условий содержания кристалла суперматериалы не пойдет нам на пользу. Прерывание.</span>")
			actor.target = null
			return FALSE
	actor.dis_integrate(src)
	return TRUE
...
/obj/machinery/chem_dispenser/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Летучие химические вещества в этой машине уничтожат нас. Прерывание.</span>")
	return FALSE

/obj/machinery/nuclearbomb/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Уничтожение этого устройства приведет к уничтожению всего в этом районе. Прерывание.</span>")
	return FALSE

/obj/effect/rune/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Поиск... неисправность датчика! Цель потеряна. Прерывание.</span>")
	return FALSE

/obj/structure/reagent_dispensers/fueltank/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Уничтожение этого объекта могло вызвать цепную реакцию. Прерывание.</span>")
	return FALSE

/obj/structure/cable/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Нарушение энергосистемы не принесет нам пользы. Прерывание.</span>")
	return FALSE

/obj/machinery/portable_atmospherics/canister/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>В результате разрушения этого объекта может образоваться негостеприимный район. Прерывание.</span>")
	return FALSE

/obj/machinery/telecomms/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Этот коммуникационный ретранслятор следует сохранить, он будет полезным ресурсом для наших мастеров в будущем. Прерывание.</span>")
	return FALSE

/obj/machinery/deepfryer/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Этот кухонный прибор стоит консервировать, он в будущем станет для наших мастеров вкусными нездоровыми закусками. Прерывание.</span>")
	return FALSE
...
/obj/machinery/gateway/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Этот источник bluespace будет важен для нас позже. Прерывание.</span>")
	return FALSE
...
/obj/item/stack/cable_coil/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)//Wiring would be too effective as a resource
	to_chat(actor, "<span class='warning'>Этот объект не содержит достаточно материалов для работы.</span>")
	return FALSE

/obj/machinery/porta_turret/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Попытка демонтировать эту машину приведет к немедленной контратаке. Прерывание.</span>")
	return FALSE
...
/obj/machinery/field/generator/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Уничтожение этого объекта вызовет катастрофическую цепную реакцию. Прерывание.</span>")
	return FALSE

/obj/machinery/field/containment/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Этот объект не содержит твердого вещества. Прерывание.</span>")
	return FALSE

/obj/machinery/power/shieldwallgen/swarmer_act(mob/living/simple_animal/hostile/swarmer/actor)
	to_chat(actor, "<span class='warning'>Уничтожение этого объекта может непредсказуемо повлиять на целостность конструкции. Прерывание.</span>")
	return FALSE
