...
/obj/item/grenade/c4/ninja/afterattack(atom/movable/AM, mob/user, flag)
	...
	if(!ninja_antag)
		to_chat(user, "<span class='notice'>Несмотря на то, что вроде как всё в порядке, я не могу взорвать заряд.</span>")
		return
	var/datum/objective/plant_explosive/objective = locate() in ninja_antag.objectives
	if(!objective)
		to_chat(user, "<span class='notice'>Кажется, я не могу активировать заряд. Он должен детонировать в какой-то конкретной локации, но я не знаю, где его взорвать.</span>")
		return
	if(objective.detonation_location != get_area(user))
		to_chat(user, "<span class='notice'>Это не то место, где я должен это использовать!</span>")
		return
	detonator = user
	return ..()
