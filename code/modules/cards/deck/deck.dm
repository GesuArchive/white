...
/obj/item/toy/cards/deck
	...
	hitsound = null
	attack_verb_continuous = list("атакует")
	attack_verb_simple = list("атакует")
...
/obj/item/toy/cards/deck/suicide_act(mob/living/carbon/user)
	user.visible_message(span_suicide("[user] is slitting [user.p_their()] wrists with <b>[src]</b>! It looks like their luck ran out!"))
...
/obj/item/toy/cards/deck/add_context(atom/source, list/context, obj/item/held_item, mob/living/user)
	if(src == held_item)
		...
		context[SCREENTIP_CONTEXT_LMB] = dealer_deck.wielded ? "Убирать" : "Дилер"
		context[SCREENTIP_CONTEXT_ALT_LMB] = "Перетасовать"
		return CONTEXTUAL_SCREENTIP_SET

	if(isnull(held_item))
		context[SCREENTIP_CONTEXT_LMB] = "Тянуть"
		context[SCREENTIP_CONTEXT_RMB] = "Тянуть перевернув"
...
/obj/item/toy/cards/deck/attack_hand(mob/living/user, list/modifiers, flip_card = FALSE)
	...
	user.balloon_alert_to_viewers("тянет карту")
...
/obj/item/toy/cards/deck/attackby(obj/item/item, mob/living/user, params)
	if(istype(item, /obj/item/toy/singlecard) || istype(item, /obj/item/toy/cards/cardhand))
		insert(item)
		var/card_grammar = istype(item, /obj/item/toy/singlecard) ? "карту" : "карты"
...
/obj/item/toy/cards/deck/syndicate
	...
	attack_verb_simple = list("атакует", "режет", "нарезает", "рубит", "режет")
