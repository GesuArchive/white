...
/obj/machinery/computer/bookmanagement/proc/print_forbidden_lore(mob/user)
	...
	user.visible_message("<span class='warning'>[user] stares at the blank screen for a few moments, [user.ru_ego()] expression frozen in fear. When [user.ru_who()] finally awaken[user.p_s()] from it, [user.ru_who()] look[user.p_s()] a lot older.</span>", 2)
