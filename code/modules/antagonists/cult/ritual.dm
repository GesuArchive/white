...
/obj/item/melee/cultblade/dagger/examine(mob/user)
	...
	if(iscultist(user) || isobserver(user))
		. += {"<hr><span class='cult'>The scriptures of the Geometer. Allows the scribing of runes and access to the knowledge archives of the cult of Nar'Sie.\n
...
/obj/item/melee/cultblade/dagger/attack_self(mob/user)
	if(!iscultist(user))
		to_chat(user, span_warning("[capitalize(src.name)] is covered in unintelligible shapes and markings.") )
		return
	scribe_rune(user)
...
/obj/item/melee/cultblade/dagger/proc/scribe_rune_attempt(mob/living/user)
	...
	if(ispath(rune_to_scribe, /obj/effect/rune/narsie))
		...
		if(!(A in summon_objective.summon_spots))
			...
			return
		var/confirm_final = tgui_alert(user, "This is the FINAL step to summon Nar'Sie; it is a long, painful ritual and the crew will be alerted to your presence", "Are you prepared for the final battle?", "My life for Nar'Sie!", "No")
		if(confirm_final == "No")
		...
		for(var/B in spiral_range_turfs(1, user, 1))
			...
			shields += N
	user.visible_message(span_warning("[user] [user.blood_volume ? "cuts open [user.ru_ego()] arm and begins writing in [user.ru_ego()] own blood":"begins sketching out a strange design"]!") , \
	...
	if(!check_rune_turf(Turf, user))
		return
	user.visible_message(span_warning("[user] creates a strange circle[user.blood_volume ? " in [user.ru_ego()] own blood":""].") , \
