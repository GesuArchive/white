...
/proc/pollCultists(mob/living/Nominee,datum/team/cult/team) //Cult Master Poll
	...
	var/list/yes_voters = poll_candidates("[Nominee] seeks to lead your cult, do you support [Nominee.ru_na()]?", poll_time = 300, group = asked_cultists)
	...
	for(var/datum/mind/B in team.members)
		if(B.current)
			...
			if(!B.current.incapacitated())
				to_chat(B.current,span_cultlarge("[Nominee] has won the cult's support and is now their master. Follow [Nominee.ru_ego()] orders to the best of your ability!"))
	return TRUE
				...
				if(B.current && B.current.stat != DEAD)
					...
					switch(i)
						if(1)
							...
							playsound(mobloc, "zap", 50, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
						if(2)
							new /obj/effect/temp_visual/dir_setting/cult/phase/out(mobloc, B.current.dir)
							playsound(mobloc, "zap", 75, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
						if(3)
							new /obj/effect/temp_visual/dir_setting/cult/phase(mobloc, B.current.dir)
							playsound(mobloc, "zap", 100, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
						if(4)
