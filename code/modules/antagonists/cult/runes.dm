...
/obj/effect/rune/attackby(obj/I, mob/user, params)
	if(istype(I, /obj/item/melee/cultblade/dagger) && iscultist(user))
		if(log_when_erased)
			var/confirm = tgui_alert(user, "Erasing this [cultist_name] rune might be against your goal to summon Nar'Sie.", "Begin to erase the [cultist_name] rune?", "Proceed", "Abort")
			if(confirm != "Proceed")
				return
...
/obj/effect/rune/proc/conceal() //for talisman of revealing/hiding
	visible_message(span_danger("[capitalize(src.name)] fades away.") )
...
/obj/effect/rune/convert/invoke(list/invokers)
	...
	if(!myriad_targets.len)
		...
		return
	rune_in_use = TRUE
	visible_message(span_warning("[capitalize(src.name)] pulses blood red!") )
	var/oldcolor = color
...
/obj/effect/rune/convert/proc/do_convert(mob/living/convertee, list/invokers)
	...
	if(convertee.anti_magic_check(TRUE, TRUE, FALSE, 0)) //Not chargecost because it can be spammed
		for(var/M in invokers)
			to_chat(M, span_warning("Something is shielding [convertee] mind!") )
...
/obj/effect/rune/raise_dead/invoke(list/invokers)
	...
	mob_to_revive.visible_message(span_warning("[mob_to_revive] draws in a huge breath, red light shining from [mob_to_revive.ru_ego()] eyes.") , \
...
/obj/effect/rune/blood_boil/invoke(list/invokers)
	...
	rune_in_use = TRUE
	var/turf/T = get_turf(src)
	visible_message(span_warning("[capitalize(src.name)] turns a bright, glowing orange!") )
	color = "#FC9B54"
	set_light(6, 1, color)
...
/obj/effect/rune/manifest/invoke(list/invokers)
	...
	var/mob/living/user = invokers[1]
	var/turf/T = get_turf(src)
	var/choice = tgui_alert(user,"You tear open a connection to the spirit realm...",,"Summon a Cult Ghost","Ascend as a Dark Spirit","Cancel")
	if(choice == "Summon a Cult Ghost")
		...
		while(!QDELETED(affecting))
			...
			if(affecting.key)
				affecting.visible_message(span_warning("[affecting] slowly relaxes, the glow around [affecting.ru_na()] dimming.") , \
...
/obj/effect/rune/apocalypse/invoke(list/invokers)
	...
	for(var/mob/living/M in GLOB.alive_mob_list)
		...
		if(!iscultist(M))
			...
			SEND_SOUND(M, sound('sound/effects/pope_entry.ogg'))
