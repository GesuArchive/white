...
/datum/action/innate/cult/blood_spell/emp/Activate()
	...
	owner.visible_message(span_warning("[owner] hand flashes a bright blue!") , \
...
/datum/action/innate/cult/blood_spell/dagger/Activate()
	...
	owner.whisper(invocation, language = /datum/language/common)
	owner.visible_message(span_warning("[owner] hand glows red for a moment.") , \
	...
	if(owner.put_in_hands(summoned_blade))
		...
		owner.visible_message(span_warning("A ritual dagger appears at [owner] feet!") , \
...
/datum/action/innate/cult/blood_spell/veiling/Activate()
	if(!revealing)
		owner.visible_message(span_warning("Thin grey dust falls from [owner] hand!") , \
		...
		owner.visible_message(span_warning("A flash of light shines from [owner] hand!") , \
...
/obj/item/melee/blood_magic/stun/afterattack(atom/target, mob/living/carbon/user, proximity)
	...
	if(iscultist(target))
		return
	if(iscultist(user))
		user.visible_message(span_warning("[user] holds up [user.ru_ego()] hand, which explodes in a flash of red light!") , \
...
/obj/item/melee/blood_magic/teleport/afterattack(atom/target, mob/living/carbon/user, proximity)
	...
	if(iscultist(user))
		...
		if(do_teleport(L, dest, channel = TELEPORT_CHANNEL_CULT))
			origin.visible_message(span_warning("Dust flows from [user] hand, and [user.ru_who()] disappear[user.p_s()] with a sharp crack!") , \
...
/obj/item/restraints/handcuffs/energy/cult/used/dropped(mob/user)
	user.visible_message(span_danger("[user] shackles shatter in a discharge of dark magic!") , \
...
/obj/item/melee/blood_magic/construction/afterattack(atom/target, mob/user, proximity_flag, click_parameters)
	if(proximity_flag && iscultist(user))
		...
		if(istype(target, /obj/item/stack/sheet/iron))
			...
			if(candidate.mmi)
				channeling = TRUE
				user.visible_message(span_danger("A dark cloud emanates from [user] hand and swirls around [candidate]!"))
			...
			if(do_after(user, 50, target = user))
				...
				target.narsie_act()
				uses--
				user.visible_message(span_warning("Black ribbons suddenly emanate from [user] hand and cling to the airlock - twisting and corrupting it!"))
				SEND_SOUND(user, sound('sound/effects/magic.ogg',0,1,25))
				channeling = FALSE
			else
				channeling = FALSE
...
/obj/item/melee/blood_magic/manipulator/examine(mob/user)
	. = ..()
	. += "<hr>Bloody halberd, blood bolt barrage, and blood beam cost [BLOOD_HALBERD_COST], [BLOOD_BARRAGE_COST], and [BLOOD_BEAM_COST] charges respectively."
			...
			if(iscultist(H))
				...
				if(H.blood_volume < BLOOD_VOLUME_SAFE)
					...
					if(uses*2 < restore_blood)
						...
						to_chat(user,span_warning("Your blood rites have restored [H == user ? "your" : "[H.ru_ego()]"] blood to safe levels!"))
				...
				if(overall_damage == 0)
					...
					if(ratio>1)
						...
						H.visible_message(span_warning("[H] is fully healed by [H==user ? "[H.ru_ego()]":"[H]"] blood magic!"))
					else
						H.visible_message(span_warning("[H] is partially healed by [H==user ? "[H.ru_ego()]":"[H]"] blood magic."))
						uses = 0
					ratio *= -1
				...
				if(H.stat == DEAD)
					to_chat(user,span_warning("[H.ru_ego(TRUE)] blood has stopped flowing, you'll have to find another way to extract it."))
					return
				if(H.cultslurring)
					to_chat(user,span_danger("[H.ru_ego(TRUE)] blood has been tainted by an even stronger form of blood magic, it's no use to us like this!"))
					return
				if(H.blood_volume > BLOOD_VOLUME_SAFE)
					H.blood_volume -= 100
					uses += 50
					...
					H.visible_message(span_danger("[user] drains some of [H] blood!"))
					to_chat(user,span_cultitalic("Your blood rite gains 50 charges from draining [H] blood."))
					new /obj/effect/temp_visual/cult/sparks(get_turf(H))
				else
					to_chat(user,span_warning("[H.p_theyre(TRUE)] missing too much blood - you cannot drain [H.ru_na()] further!"))
					return
		if(isconstruct(target))
			...
			if(missing)
				if(uses > missing)
					...
					M.visible_message(span_warning("[M] is fully healed by [user] blood magic!"))
					uses -= missing
				else
					M.adjustHealth(-uses)
					M.visible_message(span_warning("[M] is partially healed by [user] blood magic!"))
					uses = 0
			...
			if("Bloody Halberd (150)")
				if(uses < BLOOD_HALBERD_COST)
					...
					if(user.put_in_hands(rite))
						...
						user.visible_message(span_warning("A [rite.name] appears at [user] feet!") , \
