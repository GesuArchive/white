...
/mob/camera/blob/proc/create_blobbernaut()
	...
	var/list/mob/dead/observer/candidates = poll_ghost_candidates("Хочешь быть массанаутом [blobstrain.name]?", ROLE_BLOB, null, ROLE_BLOB, 50) //players must answer rapidly
	if(LAZYLEN(candidates)) //if we got at least one candidate, they're a blobbernaut now.
		...
		to_chat(blobber, "<b><font color=\"[blobstrain.color]\">[blobstrain.name]</b></font> структура [blobstrain.shortdesc ? "[blobstrain.shortdesc]" : "[blobstrain.description]"]")
	else
...
/mob/camera/blob/proc/blob_help()
	...
	to_chat(src, "<i>Обычная масса</i> расширит мой охват и может быть преобразована в специальные структуры, которые выполняют определенные функции.")
	to_chat(src, "<b>Можно улучшить обычную массу до следующих типов структур:</b>")
	to_chat(src, "<i>Крепкая масса</i> сильная и дорогая, которая поглощает больше урона. Кроме того, она пожаробезопасна и может блокировать воздух. Стоит использовать её, чтобы защитить себя от пожаров на станции. Её повторное улучшение приведет к появлению отражающей массы, способной отражать большинство снарядов за счет дополнительного здоровья сильной массы.")
	to_chat(src, "<i>Ресурсная масса</i>, которая производит для меня больше ресурсов, нужно построить её как можно больше, чтобы поглотить станцию. Этот тип массы должен быть размещен рядом с родительской массой или рядом с ядром для работы.")
	to_chat(src, "<i>Производящая масса</i>, которая порождает споры, которые атакуют ближайших врагов. Этот тип массы должен быть размещен рядом с родительской массой или рядом с ядром для работы.")
	to_chat(src, "<i>Массанауты</i> могут быть произведены на производящей массе за определенную плату, их трудно убить, они мощные и умеренно умные. Производящая масса, использованная для ее создания, станет хрупкой и не сможет производить споры.")
	to_chat(src, "<i>Родительская масса</i>, которая растёт, как ядро. Как и ядро, он может активировать ресурсы и прозводящую массу.")
