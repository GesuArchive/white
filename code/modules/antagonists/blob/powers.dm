...
/mob/camera/blob/proc/create_blobbernaut()
	...
	var/list/mob/dead/observer/candidates = poll_ghost_candidates("Do you want to play as a [blobstrain.name] blobbernaut?", ROLE_BLOB, null, ROLE_BLOB, 50) //players must answer rapidly
...
/mob/camera/blob/proc/remove_blob(turf/T)
	...
	if(B.point_return)
		...
		to_chat(src, span_notice("Gained [B.point_return] resources from removing [B]."))
	qdel(B)
