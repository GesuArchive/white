...
/obj/item/soulstone/examine(mob/user)
	...
	if(role_check(user) || isobserver(user))
		if(!grab_sleeping)
			...
			. += "<hr><span class='cult'>A soulstone, used to capture souls, either from unconscious or sleeping humans or from freed shades.</span>"
...
/obj/structure/constructshell/examine(mob/user)
	...
	if(iscultist(user) || iswizard(user) || user.stat == DEAD)
		. += {"<hr><span class='cult'>A construct shell, used to house bound souls from a soulstone.\n
...
/obj/item/soulstone/proc/getCultGhost(mob/living/carbon/human/T, mob/user)
	...
	if(!chosen_ghost || !chosen_ghost.client)	//Failing that, we grab a ghosts
		var/list/consenting_candidates = poll_ghost_candidates("Would you like to play as a Shade?", "Cultist", null, ROLE_CULTIST, 50, POLL_IGNORE_SHADE)
