...
/obj/item/clockwork/integration_cog/attack_obj(obj/O, mob/living/user)
	...
	if(A.integration_cog)
		to_chat(user, "<span class='brass'>There is already \an [src] in [A].</span>")
		return
	if(!A.panel_open)
		//Cut open the panel
		to_chat(user, "<span class='notice'>You begin cutting open [A].</span>")
		if(do_after(user, 50, target=A))
			to_chat(user, "<span class='brass'>You cut open [A] with [src].</span>")
			A.panel_open = TRUE
			A.update_icon()
			return
		return
	//Insert the cog
	to_chat(user, "<span class='notice'>You begin inserting [src] into [A].</span>")
	if(do_after(user, 40, target=A))
		A.integration_cog = src
		forceMove(A)
		A.panel_open = FALSE
		A.update_icon()
		to_chat(user, "<span class='notice'>You insert [src] into [A].</span>")
