...
/mob/living/simple_animal/revenant/ClickOn(atom/A, params) //revenants can't interact with the world directly.
	...
	if(ishuman(A))
		if(A in drained_mobs)
			to_chat(src, span_revenwarning("[A] soul is dead and empty.")  )
...
/mob/living/simple_animal/revenant/proc/Harvest(mob/living/carbon/human/target)
	...
	if(draining)
		...
		return
	if(!target.stat)
		to_chat(src, span_revennotice("[target.ru_ego(TRUE)] soul is too strong to harvest."))
		if(prob(10))
	...
	if(do_after(src, rand(10, 20), target, timed_action_flags = IGNORE_HELD_ITEM)) //did they get deleted in that second?
		if(target.ckey)
			to_chat(src, span_revennotice("[target.ru_ego(TRUE)] soul burns with intelligence."))
			essence_drained += rand(20, 30)
		if(target.stat != DEAD)
			to_chat(src, span_revennotice("[target.ru_ego(TRUE)] soul blazes with life!"))
			essence_drained += rand(40, 50)
		else
			to_chat(src, span_revennotice("[target.ru_ego(TRUE)] soul is weak and faltering."))
		if(do_after(src, rand(15, 20), target, timed_action_flags = IGNORE_HELD_ITEM)) //did they get deleted NOW?
			...
			if(do_after(src, rand(15, 25), target, timed_action_flags = IGNORE_HELD_ITEM)) //how about now
				...
				to_chat(src, span_revenminor("You begin siphoning essence from [target] soul."))
				if(target.stat != DEAD)
				...
				reveal(46)
				stun(46)
				target.visible_message(span_warning("[target] suddenly rises slightly into the air, [target.ru_ego()] skin turning an ashy gray."))
				...
				if(do_after(src, 46, target, timed_action_flags = IGNORE_HELD_ITEM)) //As one cannot prove the existance of ghosts, ghosts cannot prove the existance of the target they were draining.
					...
					if(essence_drained <= 90 && target.stat != DEAD)
						essence_regen_cap += 5
						to_chat(src, span_revenboldnotice("The absorption of [target] living soul has increased your maximum essence level. Your new maximum essence is [essence_regen_cap]."))
					if(essence_drained > 90)
						essence_regen_cap += 15
						perfectsouls++
						to_chat(src, span_revenboldnotice("The perfection of [target] soul has increased your maximum essence level. Your new maximum essence is [essence_regen_cap]."))
					to_chat(src, span_revennotice("[target] soul has been considerably weakened and will yield no more essence for the time being."))
					target.visible_message(span_warning("[target] slumps onto the ground.") , \
				...
				to_chat(src, span_revenwarning("You are not close enough to siphon [target ? "[target]":"[target.ru_ego()]"] soul. The link has been broken."))
	draining = FALSE
	essence_drained = 0
