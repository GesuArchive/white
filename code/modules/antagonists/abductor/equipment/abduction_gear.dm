...
/obj/item/abductor/gizmo/proc/scan(atom/target, mob/living/user)
	if(ishuman(target))
		...
		to_chat(user, "<span class='notice'>You scan [target] and add [target.ru_na()] to the database.</span>")
...
/obj/item/melee/baton/abductor/proc/SleepAttack(mob/living/L,mob/living/user)
	...
	if(L.incapacitated(TRUE, TRUE))
		if(L.anti_magic_check(FALSE, FALSE, TRUE))
			...
			L.visible_message("<span class='danger'>[user] tried to induced sleep in [L] with [src], but [L.ru_ego()] tinfoil protection [L.ru_na()]!</span>", \
		...
		if(L.anti_magic_check(FALSE, FALSE, TRUE, 0))
			...
			L.visible_message("<span class='danger'>[user] tried to induce sleep in [L] with [src], but [L.ru_ego()] tinfoil protection completely protected [L.ru_na()]!</span>", \
