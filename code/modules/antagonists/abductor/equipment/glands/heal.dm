...
/obj/item/organ/heart/gland/heal/proc/replace_eyes(obj/item/organ/eyes/eyes)
	if(eyes)
		owner.visible_message(span_warning("[owner] [eyes.name] fall out of their sockets!") , span_userdanger("Your [eyes.name] fall out of their sockets!"))
...
/obj/item/organ/heart/gland/heal/proc/finish_replace_eyes()
	...
	owner.visible_message(span_warning("A pair of new eyes suddenly inflates into [owner] eye sockets!") , span_userdanger("A pair of new eyes suddenly inflates into your eye sockets!"))

/obj/item/organ/heart/gland/heal/proc/replace_limb(body_zone, obj/item/bodypart/limb)
	if(limb)
		owner.visible_message(span_warning("[owner] [limb.name] suddenly detaches from [owner.ru_ego()] body!") , span_userdanger("Your [limb.name] suddenly detaches from your body!"))
...
/obj/item/organ/heart/gland/heal/proc/finish_replace_limb(body_zone)
	owner.visible_message(span_warning("With a loud snap, [owner] [parse_zone(body_zone)] rapidly grows back from [owner.ru_ego()] body!") ,
...
/obj/item/organ/heart/gland/heal/proc/replace_chest(obj/item/bodypart/chest/chest)
	if(chest.status == BODYPART_ROBOTIC)
		owner.visible_message(span_warning("[owner] [chest.name] rapidly expels its mechanical components, replacing them with flesh!") , span_userdanger("Your [chest.name] rapidly expels its mechanical components, replacing them with flesh!"))
		...
		owner.visible_message(span_warning("[owner] [chest.name] sheds off its damaged flesh, rapidly replacing it!") , span_warning("Your [chest.name] sheds off its damaged flesh, rapidly replacing it!"))
		playsound(owner, 'sound/effects/splat.ogg', 50, TRUE)
		var/list/dirs = GLOB.alldirs.Copy()
		for(var/i in 1 to 3)
