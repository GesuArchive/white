...
/obj/item/antag_spawner/nuke_ops/attack_self(mob/user)
	...
	var/list/nuke_candidates = poll_ghost_candidates("Do you want to play as a syndicate [borg_to_spawn ? "[lowertext(borg_to_spawn)] cyborg":"operative"]?", ROLE_OPERATIVE, null, ROLE_OPERATIVE, 150, POLL_IGNORE_SYNDICATE)
...
/obj/item/antag_spawner/slaughter_demon/attack_self(mob/user)
	...
	if(used)
		return
	var/list/candidates = poll_candidates_for_mob("Do you want to play as a [initial(demon_type.name)]?", ROLE_ALIEN, null, ROLE_ALIEN, 50, src)
	if(LAZYLEN(candidates))
