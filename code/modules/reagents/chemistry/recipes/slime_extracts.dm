...
/datum/chemical_reaction/slime/slimespawn/on_reaction(datum/reagents/holder)
	...
	S.visible_message("<span class='danger'>Наполненное плазмой ядро начало дрожать и увеличиваться в размерах, после чего из него выливается новый маленький Слайм!</span>")
	..()
...
/datum/chemical_reaction/slime/slimemobspawn/proc/summon_mobs(datum/reagents/holder, turf/T)
	T.visible_message("<span class='danger'>Экстракт Слайма начинает сильно вибрировать!</span>")
...
/datum/chemical_reaction/slime/slimemobspawn/friendly/summon_mobs(datum/reagents/holder, turf/T)
	T.visible_message("<span class='danger'>Экстракт Слайма начинает умилительно вибрировать!</span>")
...
/datum/chemical_reaction/slime/slimefoam/on_reaction(datum/reagents/holder)
	holder.create_foam(/datum/effect_system/foam_spread,80, "<span class='danger'>[capitalize(src)] изрыгает пену!</span>")
...
/datum/chemical_reaction/slime/slimefreeze/on_reaction(datum/reagents/holder)
	var/turf/T = get_turf(holder.my_atom)
	T.visible_message("<span class='danger'>Экстракт слайма становится очень холодным!</span>")
...
/datum/chemical_reaction/slime/slimebloodlust/on_reaction(datum/reagents/holder)
	for(var/mob/living/simple_animal/slime/slime in viewers(get_turf(holder.my_atom), null))
		if(slime.docile) //Undoes docility, but doesn't make rabid.
			slime.visible_message("<span class='danger'>[slime] забывает все тренировки, вновь становясь диким!</span>")
			slime.docile = FALSE
			slime.update_name()
			continue
		slime.rabid = 1
		slime.visible_message("<span class='danger'>[slime] доведен до бешенства!</span>")
	..()
...
/datum/chemical_reaction/slime/slimecrystal/on_reaction(datum/reagents/holder, created_volume)
	...
	BC.visible_message("<span class='notice'>[BC.name] появляется из ниоткуда!</span>")
	..()
...
/datum/chemical_reaction/slime/slime_rng/on_reaction(datum/reagents/holder, created_volume)
	if(created_volume >= 5)
		...
		S.visible_message("<span class='danger'>Наполненное плазмой ядро начинает неконтролируемое расширение!</span>")
...
/datum/chemical_reaction/slime/slimebomb/on_reaction(datum/reagents/holder, created_volume)
	...
	S.visible_message("<span class='danger'>Наполненное слаймовым желе ядро начинает неконтролируемое расширение!</span>")
	S.icon_state = "[S.base_state]_active"
	S.active = TRUE
	addtimer(CALLBACK(S, /obj/item/grenade.proc/detonate), rand(15,60))
	var/lastkey = holder.my_atom.fingerprintslast
