...
/obj/machinery/chem_master/attackby_secondary(obj/item/weapon, mob/living/user, params)
	. = ..()
...
/obj/machinery/chem_master/ui_data(mob/user)
	...
	if(beaker)
		for(var/datum/reagent/R in beaker.reagents.reagent_list)
			beaker_contents.Add(list(list("name" = R.name, "id" = lowertext(R.name), "volume" = round(R.volume, 0.01)))) // list in a list because Byond merges the first list...
	data["beakerContents"] = beaker_contents

	var/buffer_contents[0]
	if(reagents.total_volume)
		for(var/datum/reagent/N in reagents.reagent_list)
			buffer_contents.Add(list(list("name" = N.name, "id" = lowertext(N.name), "volume" = round(N.volume, 0.01)))) // ^
	...
	for(var/e in holder.reaction_list)
		...
		for(var/result in E.reaction.required_reagents)
			...
			if(R == reagent.type)
				canMove = FALSE
	if(!canMove)
		say("Невозможно переместить реагенты, задержанные в химической реакции!")
	return canMove
