...
/datum/reagent/medicine/oculine/on_mob_life(mob/living/carbon/owner, delta_time, times_fired)
	...
	if(HAS_TRAIT_FROM(owner, TRAIT_BLIND, EYE_DAMAGE))
		if(DT_PROB(fix_prob, delta_time))
			to_chat(owner, "<span class='warning'>Ко мне постепенно возвращается зрение...</span>")
			owner.cure_blind(EYE_DAMAGE)
			owner.cure_nearsighted(EYE_DAMAGE)
			owner.blur_eyes(35)
	else if(HAS_TRAIT_FROM(owner, TRAIT_NEARSIGHT, EYE_DAMAGE))
		to_chat(owner, "<span class='warning'>В моем периферийном зрении рассеивается темнота.</span>")
		owner.cure_nearsighted(EYE_DAMAGE)
		owner.blur_eyes(10)
	..()

/datum/reagent/medicine/oculine/on_mob_delete(mob/living/owner, delta_time)
	var/obj/item/organ/eyes/eyes = owner.get_organ_slot(ORGAN_SLOT_EYES)
	if(!eyes)
		return
...
/datum/reagent/medicine/mannitol/overdose_start(mob/living/owner)
	to_chat(owner, "<span class='notice'>Ощущаю </span><span class='purple'>П Р О С В Я Щ Е Н И Е!</span>")
...
/datum/reagent/medicine/mannitol/overdose_process(mob/living/owner, delta_time, times_fired)
	...
	to_chat(owner, "<span class='purple'><b>Заметка: </b>[html_encode(message)]</span>")
	..()
...
/datum/reagent/medicine/coagulant/banana_peel
	...
	glass_name = "стакан banana peel pulp"
