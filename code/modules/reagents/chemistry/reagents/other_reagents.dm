...
/datum/reagent/water/holywater/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume)
	...
	if(iscultist(exposed_mob))
		to_chat(exposed_mob, "<span class='userdanger'>Омерзительная святость начинает распространять свои сияющие щупальца по вашему разуму, очищая влияние Геометра Крови!</span>")
...
/datum/reagent/water/holywater/on_mob_life(mob/living/carbon/M)
	...
	if(iscultist(M))
		for(var/datum/action/innate/cult/blood_magic/BM in M.actions)
			to_chat(M, "<span class='cultlarge'>Мои кровавые ритуалы сорваны, так как святая вода омыла моё тело!</span>")
	...
	if(data["misc"] >= 25)		// 10 units, 45 seconds @ metabolism 0.4 units & tick rate 1.8 sec
		...
		if(iscultist(M) && prob(20))
			...
			if(prob(10))
				M.visible_message("<span class='danger'>У [M] начался припадок!</span>", "<span class='userdanger'>У меня припадок!</span>")
				M.Unconscious(120)
...
/datum/reagent/spraytan/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message = TRUE)
	...
	if(ishuman(exposed_mob))
		...
		if((methods & INGEST) && show_message)
			to_chat(exposed_mob, "<span class='notice'>Вкус говнище.</span>")
...
/datum/reagent/mutationtoxin/jelly/on_mob_life(mob/living/carbon/human/H)
	if(isjellyperson(H))
		to_chat(H, "<span class='warning'>Моя масса трансформируется и смещается, превращая меня в другой подвид!</span>")
...
/datum/reagent/mulligan/on_mob_life(mob/living/carbon/human/H)
	...
	if (!istype(H))
		return
	to_chat(H, "<span class='warning'><b>Стискиваю зубы от боли, появившейся во время быстрой мутации моего тела!</b></span>")
...
/datum/reagent/bluespace/on_mob_life(mob/living/carbon/M)
	if(current_cycle > 10 && prob(15))
		to_chat(M, "<span class='warning'>Чувствую себя нестабильно...</span>")
		M.Jitter(2)
		current_cycle = 1
...
/datum/reagent/barbers_aid/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message=TRUE, touch_protection=FALSE)
	...
	to_chat(exposed_human, "<span class='notice'>На моем скальпе начали расти волосы.</span>")
...
/datum/reagent/concentrated_barbers_aid/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message=TRUE, touch_protection=FALSE)
	...
	var/mob/living/carbon/human/exposed_human = exposed_mob
	to_chat(exposed_human, "<span class='notice'>Мои волосы начали очень быстро расти!</span>")
...
/datum/reagent/baldium/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message=TRUE, touch_protection=FALSE)
	...
	var/mob/living/carbon/human/exposed_human = exposed_mob
	to_chat(exposed_human, "<span class='danger'>Мои волосы начали выпадать целыми клочьями!</span>")
