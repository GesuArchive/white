...
/obj/vehicle/sealed/car/clowncar/take_damage(damage_amount, damage_type = BRUTE, damage_flag = 0, sound_effect = 1, attack_dir)
	. = ..()
	if(prob(33))
		visible_message(span_danger("[capitalize(src.name)] spews out a ton of space lube!"))
...
/obj/vehicle/sealed/car/clowncar/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		return
	obj_flags |= EMAGGED
	to_chat(user, span_danger("You scramble <b>[src.name]</b>'s child safety lock, and a panel with six colorful buttons appears!"))
...
/obj/vehicle/sealed/car/clowncar/proc/roll_the_dice(mob/user)
	...
	switch(rand(1,6))
		...
		if(2)
			...
			var/datum/effect_system/fluid_spread/foam/foam = new
			foam.set_up(200, location = loc, carry = randomchems)
			foam.start()
		if(3)
