...
/obj/item/key/atv
	name = "ключ квадроцикла"
	desc = "ключ зажигания от квадроцикла."

/obj/item/key/security
	name = "ключ гироскутера"
	desc = "Брелок для ключей с маленьким стальным ключом и резиновой электрошоковой дубинкой."
	icon_state = "keysec"
...
/obj/item/key/security/suicide_act(mob/living/carbon/user)
	if(!user.emote("spin")) //In the off chance that someone attempts this suicide while under the effects of mime's bane they deserve the silliness.
		user.visible_message(span_suicide("[user] is putting <b>[src.name]</b> in [user.ru_ego()] ear and starts [user.ru_ego()] motor! It looks like [user.p_theyre()] trying to commit suicide... But [user.ru_who()] sputters and stalls out! "))
		playsound(src, 'sound/misc/sadtrombone.ogg', 50, TRUE, -1)
		return SHAME
	user.visible_message(span_suicide("[user] is putting <b>[src.name]</b> in [user.ru_ego()] ear and starts [user.ru_ego()] motor! It looks like [user.p_theyre()] trying to commit suicide!"))
...
/obj/item/key/janitor
	name = "ключ джаникара"
	desc = "Брелок с маленьким стальным ключом и розовым брелоком с надписью \"Мокрая киска\"."
...
/obj/item/key/janitor/suicide_act(mob/living/carbon/user)
	switch(user.mind?.get_skill_level(/datum/skill/cleaning))
		if(SKILL_LEVEL_NONE to SKILL_LEVEL_NOVICE) //Their mind is too weak to ascend as a janny
			user.visible_message(span_suicide("[user] is putting <b>[src.name]</b> in [user.ru_ego()] mouth and пытается become one with the janicart, but has no idea where to start! It looks like [user.p_theyre()] trying to commit suicide!"))
			user.gib()
			return MANUAL_SUICIDE
		if(SKILL_LEVEL_APPRENTICE to SKILL_LEVEL_JOURNEYMAN) //At least they tried
			user.visible_message(span_suicide("[user] is putting <b>[src.name]</b> in [user.ru_ego()] mouth and has inefficiently become one with the janicart! It looks like [user.p_theyre()] trying to commit suicide!"))
		...
		if(SKILL_LEVEL_EXPERT to SKILL_LEVEL_MASTER) //They are worthy enough, but can it go even further beyond?
			user.visible_message(span_suicide("[user] is putting <b>[src.name]</b> in [user.ru_ego()] mouth and has skillfully become one with the janicart! It looks like [user.p_theyre()] trying to commit suicide!"))
			user.AddElement(/datum/element/cleaning)
		...
		if(SKILL_LEVEL_LEGENDARY to INFINITY) //Holy shit, look at that janny go!
			user.visible_message(span_suicide("[user] is putting <b>[src.name]</b> in [user.ru_ego()] mouth and has epically become one with the janicart, and they're even in overdrive mode! It looks like [user.p_theyre()] trying to commit suicide!"))
			user.AddElement(/datum/element/cleaning)
...
/obj/item/key/proc/manual_suicide(mob/living/user)
	if(user)
		...
		user.visible_message(span_suicide("[user] forgot [user.ru_who()] isn't actually a janicart! That's a paddlin'!"))
