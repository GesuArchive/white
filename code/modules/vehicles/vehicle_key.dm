...
/obj/item/key/security/suicide_act(mob/living/carbon/user)
	if(!user.emote("spin")) //In the off chance that someone attempts this suicide while under the effects of mime's bane they deserve the silliness.
		user.visible_message("<span class='suicide'>[user] is putting \the [src] in [user.ru_ego()] ear and starts [user.ru_ego()] motor! It looks like [user.p_theyre()] trying to commit suicide... But [user.ru_who()] sputters and stalls out! </span>")
		playsound(src, 'sound/misc/sadtrombone.ogg', 50, TRUE, -1)
		return SHAME
	user.visible_message("<span class='suicide'>[user] is putting \the [src] in [user.ru_ego()] ear and starts [user.ru_ego()] motor! It looks like [user.p_theyre()] trying to commit suicide!</span>")
...
/obj/item/key/janitor/suicide_act(mob/living/carbon/user)
	switch(user.mind?.get_skill_level(/datum/skill/cleaning))
		if(SKILL_LEVEL_NONE to SKILL_LEVEL_NOVICE) //Their mind is too weak to ascend as a janny
			user.visible_message("<span class='suicide'>[user] is putting \the [src] in [user.ru_ego()] mouth and is trying to become one with the janicart, but has no idea where to start! It looks like [user.p_theyre()] trying to commit suicide!</span>")
			user.gib()
			return MANUAL_SUICIDE
		if(SKILL_LEVEL_APPRENTICE to SKILL_LEVEL_JOURNEYMAN) //At least they tried
			user.visible_message("<span class='suicide'>[user] is putting \the [src] in [user.ru_ego()] mouth and has inefficiently become one with the janicart! It looks like [user.p_theyre()] trying to commit suicide!</span>")
		...
		if(SKILL_LEVEL_EXPERT to SKILL_LEVEL_MASTER) //They are worthy enough, but can it go even further beyond?
			user.visible_message("<span class='suicide'>[user] is putting \the [src] in [user.ru_ego()] mouth and has skillfully become one with the janicart! It looks like [user.p_theyre()] trying to commit suicide!</span>")
			user.AddElement(/datum/element/cleaning)
		...
		if(SKILL_LEVEL_LEGENDARY to INFINITY) //Holy shit, look at that janny go!
			user.visible_message("<span class='suicide'>[user] is putting \the [src] in [user.ru_ego()] mouth and has epically become one with the janicart, and they're even in overdrive mode! It looks like [user.p_theyre()] trying to commit suicide!</span>")
			user.AddElement(/datum/element/cleaning)
...
/obj/item/key/proc/manual_suicide(mob/living/user)
	if(user)
		...
		user.visible_message("<span class='suicide'>[user] forgot [user.ru_who()] isn't actually a janicart! That's a paddlin'!</span>")
