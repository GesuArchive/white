...
/datum/action/cooldown/spell/pointed/mind_transfer/is_valid_target(atom/cast_on)
	...
	if(!living_target.key && target_requires_key)
		to_chat(owner, "<span class='warning'>[living_target.p_theyve(TRUE)] appear[living_target.p_s()] to be catatonic! Not even magic can affect [living_target.p_their()] vacant mind.</span>")
		return FALSE
