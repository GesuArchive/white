...
/datum/action/cooldown/spell/furion/cast(atom/cast_on)
	...
	for(var/B in SSeconomy.bank_accounts_by_id)
		...
		for(var/obj/BC in A.bank_cards)
			...
			if(ismob(card_holder))
				...
				lastloc = card_holder
				to_chat(card_holder, "<span class='warning'><b>Федерация волшебников:</b> С аккаунта было списано [credits_drawed] кредит[get_num_string(credits_drawed)]. Приятной смены!</span>")
	...
	to_chat(cast_on, "<span class='notice'>Удалось собрать с проклятых гоев целых [total_cash_looted] кредит[get_num_string(total_cash_looted)]!</span>")
