...
/obj/item/phylactery/proc/rise()
	...
	if(old_body?.loc)
		...
		if(wheres_wizdo)
			old_body.visible_message(span_warning("Suddenly [old_body.name] corpse falls to pieces! You see a strange energy rise from the remains, and speed off towards the [wheres_wizdo]!"))
