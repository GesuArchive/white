...
/proc/curse_of_madness(mob/user, message)
	...
	for(var/mob/living/carbon/human/to_curse in GLOB.player_list)
		...
		if(to_curse.can_block_magic(MAGIC_RESISTANCE|MAGIC_RESISTANCE_MIND))
			to_chat(to_curse, "<span class='notice'>You have a strange feeling for a moment, but then it passes.</span>")
			continue
...
/proc/give_madness(mob/living/carbon/human/to_curse, message)
	...
	to_chat(to_curse, "<span class='reallybig'><span class='hypnophrase'>[message]</span></span>")
	to_chat(to_curse, "<span class='warning'>Your mind shatters!</span>")
	switch(rand(1, 10))
		if(1 to 3)
