...
/obj/effect/immovablerod/wizard/penetrate(mob/living/L)
	if(L.anti_magic_check())
		L.visible_message(span_danger("[capitalize(src.name)] hits [L], but it bounces back, then vanishes!")  , span_userdanger("[capitalize(src.name)] hits you... but it bounces back, then vanishes!")  , span_danger("You hear a weak, sad, CLANG."))
		qdel(src)
		return
