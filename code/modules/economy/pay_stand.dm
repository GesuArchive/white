	...
	if(istype(W, /obj/item/card/id))
		...
		if(!my_card)
			...
			if(!msg)
				return
			name = msg
			desc = "Owned by [new_card.registered_account.account_holder], pays directly into [user.ru_ego()] account."
			my_card = new_card
...
/obj/machinery/paystand/examine(mob/user)
	...
	if(user.get_active_held_item() == my_card)
		. += "<span class='notice'>\nPaystands can be edited through swiping your card with different intents. <b>Disarm</b> allows editing the name while <b>Grab</b> changes payment functionality.</span>"
