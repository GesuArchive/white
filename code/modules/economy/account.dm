...
/datum/bank_account/proc/payday(amt_of_paychecks, free = FALSE)
	if(!account_job)
		return
	var/money_to_transfer = round(account_job.paycheck * payday_modifier * amt_of_paychecks) + SSeconomy.bonus_money
	if(free)
		...
		if(ismob(card_holder)) //If on a mob
			...
			for(var/mob/M in hearers(1,T))
				...
				if(M.can_hear())
					...
					to_chat(M, "[icon2html(icon_source, card_holder)] <span class='notice'>[message]</span>")
		else
			var/atom/sound_atom
