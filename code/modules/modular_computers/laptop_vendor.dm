...
/obj/machinery/lapvend
	name = "Торговый автомат компьютеров"
	desc = "Торговый автомат с микрофабрикатором, способным продавать различные компьютеры марки NT."
...
/obj/machinery/lapvend/proc/fabricate_and_recalc_price(fabricate = FALSE)
	...
	if(devtype == 1) 		// Laptop, generally cheaper to make it accessible for most station roles
		...
		total_price = CARGO_CRATE_VALUE - 1
		switch(dev_cpu)
			if(1)
				if(fabricate)
			...
			if(2)
				...
				total_price += 3 * CARGO_CRATE_VALUE - 1
		switch(dev_battery)
			if(1) // Basic(750C)
				if(fabricate)
		...
		if(dev_apc_recharger)
			total_price += 4 * CARGO_CRATE_VALUE - 1
			if(fabricate)
...
/obj/machinery/lapvend/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/stack/spacecash))
		...
		if(!user.temporarilyRemoveItemFromInventory(c))
			return
		credits += c.value
		visible_message("<span class='info'><span class='name'>[user]</span> вставляет [c.value] cr в [src].</span>")
		qdel(c)
		return
	else if(istype(I, /obj/item/holochip))
		var/obj/item/holochip/HC = I
		credits += HC.credits
		visible_message("<span class='info'>[user] вставляет [HC.credits] cr чип голокредитов в [src].</span>")
		qdel(HC)
		return
