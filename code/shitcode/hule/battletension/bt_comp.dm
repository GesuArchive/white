...
/datum/component/battletension/proc/fuck_screen(mob/living/user, howmuch)
	...
	if (prob(howmuch) && howmuch > 5)
		...
		for (var/obj/screen/plane_master/f1 in OT)
			var/matrix/M = matrix()
			M.Translate(0, rand(-2, 2))
			animate(f1, transform = M, time = rand(10, 25), loop = 1, easing = SINE_EASING)
			animate(f1 = null, time = rand(10, 15), easing = SINE_EASING)
		for (var/obj/screen/plane_master/f2 in GW)
			var/matrix/M = matrix()
			M.Translate(0, rand(-2, 2))
			animate(f2, transform = M, time = rand(10, 25), loop = 1, easing = SINE_EASING)
			animate(f2 = null, time = rand(10, 15), easing = SINE_EASING)
