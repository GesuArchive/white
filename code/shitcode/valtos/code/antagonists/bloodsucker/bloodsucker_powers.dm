...
/datum/action/bloodsucker
	...
	button_icon = 'code/shitcode/valtos/icons/bloodsucker/fulpicons.dmi'	//This is the file for the BACKGROUND icon
	...
	icon_icon = 'code/shitcode/valtos/icons/bloodsucker/fulpicons.dmi'		//This is the file for the ACTION icon
...
/datum/action/bloodsucker/proc/CheckCanUse(display_error)	// These checks can be scanned every frame while a ranged power is on.
	...
	if (must_be_capacitated)
		var/mob/living/L = owner
		if (L.incapacitated(ignore_restraints=TRUE,ignore_grab=TRUE,check_immobilized=!can_be_immobilized) || !(L.mobility_flags & MOBILITY_STAND) && !can_be_immobilized)
			if (display_error)
