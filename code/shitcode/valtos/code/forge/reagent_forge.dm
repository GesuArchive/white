/obj/machinery/reagent_forge
	...
	icon = 'code/shitcode/valtos/icons/forge/3x3.dmi'
...
/obj/machinery/reagent_forge/proc/check_cost(materials, using)
	...
	if(materials*efficiency > ourmaterials.get_material_amount(/datum/material/reagent))
		...
		if(using)
			ourmaterials.use_amount_mat(materials*efficiency, /datum/material/reagent)
		return TRUE
...
/obj/machinery/reagent_forge/ui_interact(mob/user, ui_key = "main", datum/tgui/ui = null, force_open = 0, datum/tgui/master_ui = null, datum/ui_state/state = GLOB.default_state)
	ui = SStgui.try_update_ui(user, src, ui_key, ui, force_open)
	if(!ui)
		ui = new(user, src, ui_key, "chem_reagent_forge", name, ui_x, ui_y, master_ui, state)
		ui.open()
