...
/mob/living/carbon/human/proc/recalculate_stats()
	...
	dna.species.brutemod 		   = bmods["brutemod"]			  		 / ((bstats[MOB_STR] + bstats[MOB_STM]) / 30)
	dna.species.burnmod 		   = bmods["burnmod"]			  		 / ((bstats[MOB_STR] + bstats[MOB_STM]) / 30)
	dna.species.coldmod 		   = bmods["coldmod"]			  		 / ((bstats[MOB_STR] + bstats[MOB_STM]) / 30)
	dna.species.heatmod 		   = bmods["heatmod"]			  		 / ((bstats[MOB_STR] + bstats[MOB_STM]) / 30)
	dna.species.stunmod 		   = bmods["stunmod"]   		  		 / ((bstats[MOB_STR] + bstats[MOB_STM] + bstats[MOB_DEX]) / 30)
	dna.species.speedmod 		   = bmods["speedmod"]  		  		 / (bstats[MOB_DEX] / 15)
	next_move_modifier			   = bmods["next_move_modifier"] 		 / (bstats[MOB_DEX] / 15)
