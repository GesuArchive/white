...
/datum/controller/subsystem/title/proc/set_load_state(state)
	if(enabled_shit)
		switch(state)
			if("init1")
				set_load_pos(25, "инициализация")
			if("init2")
				set_load_pos(30, "инициализация?")
			if("atoms1")
				set_load_pos(60, "атомы")
			if("atoms2")
				set_load_pos(65, "атомы обработаны")
			if("diseases")
				set_load_pos(70, "система болезней")
			if("air")
				set_load_pos(75, "атмосфера")
			if("assets")
				set_load_pos(80, "ассеты")
			if("smoothing")
				set_load_pos(80, "сглаживание")
			if("overlays")
				set_load_pos(85, "оверлеи")
			if("light")
				set_load_pos(90, "свет")
			if("shuttle")
				set_load_pos(100, "шаттлы")
			if("end")
