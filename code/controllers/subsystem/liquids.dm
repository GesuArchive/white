...
/obj/effect/abstract/liquid_turf
	name = "liquid"
	icon = 'white/valtos/icons/liquid/liquid.dmi'
	icon_state = "water-0"
...
/obj/effect/abstract/liquid_turf/proc/set_new_liquid_state(new_state)
	...
	switch(liquid_state)
		if(LIQUID_STATE_ANKLES)
			var/mutable_appearance/overlay = mutable_appearance('white/valtos/icons/liquid/liquid_overlays.dmi', "stage1_bottom")
			var/mutable_appearance/underlay = mutable_appearance('white/valtos/icons/liquid/liquid_overlays.dmi', "stage1_top")
		...
		if(LIQUID_STATE_WAIST)
			var/mutable_appearance/overlay = mutable_appearance('white/valtos/icons/liquid/liquid_overlays.dmi', "stage2_bottom")
			var/mutable_appearance/underlay = mutable_appearance('white/valtos/icons/liquid/liquid_overlays.dmi', "stage2_top")
			overlay.plane = GAME_PLANE
			overlay.layer = ABOVE_MOB_LAYER
			underlay.plane = GAME_PLANE
			underlay.layer = GATEWAY_UNDERLAY_LAYER
		...
		if(LIQUID_STATE_SHOULDERS)
			var/mutable_appearance/overlay = mutable_appearance('white/valtos/icons/liquid/liquid_overlays.dmi', "stage3_bottom")
			var/mutable_appearance/underlay = mutable_appearance('white/valtos/icons/liquid/liquid_overlays.dmi', "stage3_top")
			overlay.plane = GAME_PLANE
			overlay.layer = ABOVE_MOB_LAYER
			underlay.plane = GAME_PLANE
			underlay.layer = GATEWAY_UNDERLAY_LAYER
		...
		if(LIQUID_STATE_FULLTILE)
			var/mutable_appearance/overlay = mutable_appearance('white/valtos/icons/liquid/liquid_overlays.dmi', "stage4_bottom")
			overlay.plane = GAME_PLANE
			overlay.layer = ABOVE_MOB_LAYER
			add_overlay(overlay)
...
/obj/effect/abstract/liquid_turf/proc/mob_fall(datum/source, mob/M)
	...
	if(liquid_state >= LIQUID_STATE_ANKLES && T.has_gravity(T))
		playsound(T, 'white/valtos/sounds/liquid/splash.ogg', 50, 0)
		if(iscarbon(M))
...
/obj/effect/temp_visual/liquid_splash
	icon = 'white/valtos/icons/liquid/splash.dmi'
	icon_state = "splash"
	layer = FLY_LAYER
	randomdir = FALSE
