//unsuccessful goon mechcomp port, i want to die btw
//оставляю в билде для РЕФЕРЕНСА ЕБАТЬ чтобы было куда смотреть при попытке портировать компоненты с goovna на богоугодный ТГ
//был немножко потроган мною в не очень успешной попытке деконвертировать говно в код, что стоит иметь в виду.
//Не включать, сука, в билд, или вам буёнд руки переломает.
#define isscrewingtool(w) (W.tool_behaviour == TOOL_SCREWDRIVER)
#define iswrenchingtool(w) (W.tool_behaviour == TOOL_WRENCH)
#define ispulsingtool(w) (W.tool_behaviour == TOOL_MULTITOOL)
#define ispryingtool(w) (W.tool_behaviour == TOOL_CROWBAR)
...
#define LIGHT_UP_HOUSING //src.noop() //spawn(0) src.light_up_housing()


//плейсхолдер потому что я ебал гуны
/obj/item/mechanics/noop()
	return
...
/obj/item/storage/mechanics // generic
	...
	icon='white/RedFoxIV/icons/mechanics.dmi'
	...
	suicide(var/mob/user as mob) // lel
		...
		spawn(3 SECONDS)
			...
			user.adjustOrganLoss(ORGAN_SLOT_BRAIN, BRAIN_DAMAGE_DEATH, BRAIN_DAMAGE_DEATH)
		spawn(20 SECONDS)
			if (user && !isdead(user))
				user.suiciding = 0
		return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (isscrewingtool(W))
			if(src.welded)
				to_chat(user,"<span class='alert'>The [src] is welded shut.</span>")
				return
			...
			if(!src.can_be_anchored)
				to_chat(user,"<span class='alert'>[src] cannot be anchored to the ground.</span>")
				return
			if(!src.open || src.welded)
				to_chat(user,"<span class='alert'>You are unable to access the [src]'s bolts as they are on the inside.</span>")
				return
			if(!isturf(src.loc) && !src.anchored)
				to_chat(user,"<span class='alert'>You cannot anchor a component housing inside something else.</span>")
				return
			src.anchored=!src.anchored
			playsound(src.loc,'sound/items/Ratchet.ogg',50)
			to_chat(user,"<span class='notice'>You [src.anchored ? "anchor the [src] to" : "unsecure the [src] from"] the ground</span>")
			if (!src.anchored)
				src.destroy_outside_connections() //burn those bridges
			return 1
		else if (isweldingtool(W))
			if (!src.can_be_welded)
				to_chat(user,"<span class='alert'>[src]'s cover cannot be welded shut.</span>")
				return
			if (src.open)
				to_chat(user,"Why would you want to weld something <i>open?</i>")
				return
			if(W:try_weld(user, 1))
				src.welded=!src.welded
				to_chat(user,"<span class='notice'>You [src.welded ? "" : "un"]weld the [src]'s cover</span>")
				src.updateIcon()
				return 1
	...
	housing_large // chonker
		...
		w_class = WEIGHT_CLASS_BULKY //all the weight
		num_f_icons=3
		density=1
		anchored = FALSE
	...
	housing_handheld
		...
		w_class = WEIGHT_CLASS_NORMAL // fits in backpacks but not pockets. no quickdraw honk boxess
		density=0
		anchored=0
		num_f_icons=1
...
/obj/item/mechanics/trigger/trigger/New()
	..()
	SEND_SIGNAL(src,COMSIG_MECHCOMP_ALLOW_MANUAL_SIGNAL)
/obj/item/mechanics/trigger/trigger/attackby(obj/item/W as obj, mob/user as mob)
...
/obj/item/mechanics/trigger/trigger/attack_hand(mob/user as mob)
	..()
	...
	return
/obj/item/mechanics/trigger/trigger/afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
	qdel(src)// never should be outside of the gun (in someone's hands), so kill it
	return
/obj/item/mechanics/trigger/trigger/updateIcon()
	icon_state = icon_up
	return
...
/obj/item/mechanics
		...
		if (ispryingtool(W))
			if (can_rotate)
				if (!anchored)
					...
					to_chat(user, "You must unsecure the [src] in order to rotate it.")
			return 1
		else if(iswrenchingtool(W))
			switch(level)
				if(1) //Level 1 = wrenched into place
					to_chat(user, "You detach the [src] from the [istype(src.loc,/obj/item/storage/mechanics) ? "housing" : "underfloor"] and deactivate it.")
				...
				if(2) //Level 2 = loose
					if(!isturf(src.loc) && !(IN_CABINET)) // allow items to be deployed inside housings, but not in other stuff like toolboxes
						to_chat(user, "<span class='alert'>[src] needs to be on the ground  [src.cabinet_banned ? "" : "or in a component housing"] for that to work.</span>")
						return 0
					if(IN_CABINET && src.cabinet_banned)
						to_chat(user,"<span class='alert'>[src] is not allowed in component housings.</span>")
						return
					if(src.one_per_tile)
						for(var/obj/item/mechanics/Z in src.loc)
							if (Z.type == src.type && Z.level == 1)
								to_chat(user,"<span class='alert'>No matter how hard you try, you are not able to think of a way to fit more than one [src] on a single tile.</span>")
								return
					to_chat(user, "You attach the [src] to the [istype(src.loc,/obj/item/storage/mechanics) ? "housing" : "underfloor"] and activate it.")
	...
	MouseDrop(obj/O, null, var/src_location, var/control_orig, var/control_new, var/params)
		if(level == 2 || (istype(O, /obj/item/mechanics) && O.level == 2))
			to_chat(usr, "<span class='alert'>Both components need to be secured into place before they can be connected.</span>")
			return ..()
...
/obj/item/mechanics/cashmoney
	...
	proc/setPrice(obj/item/W as obj, mob/user as mob)
		if (code)
			...
			if (codecheck != code)
				to_chat(user, "<span class='alert'>[bicon(src)]: Incorrect code entered.</span>")
				return 0
		...
		if(!isnull(inp))
			if (inp < 0)
				to_chat(user,"You cannot set a negative price.", "red") // Infinite credits exploit.
				return 0
			if (inp == 0)
				to_chat(user,"Please set a price higher than zero.", "red")
				return 0
			if (inp > 1000000) // ...and just to be on the safe side. Should be plenty.
				inp = 1000000
				to_chat(user,"[src] is not designed to handle such large transactions. Input has been set to the allowable limit.", "red")
			price = inp
			//tooltip_rebuild = 1
			to_chat(user, "Price set to [inp]")
			return 1
		return 0
...
/obj/item/mechanics/thprint
	...
	proc/setPrice(obj/item/W as obj, mob/user as mob)
		...
		to_chat(user, "String set to [paper_name]")
		return 1
...
/obj/item/mechanics/pscan
	...
	proc/toggleConsume(obj/item/W as obj, mob/user as mob)
		del_paper = !del_paper
		to_chat(user, "[del_paper ? "Now consuming paper":"Now NOT consuming paper"]")
		return 1
	proc/toggleThermal(obj/item/W as obj, mob/user as mob)
		thermal_only = !thermal_only
		to_chat(user, "[thermal_only ? "Now accepting only thermal paper":"Now accepting any paper"]")
		return 1
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if(..(W, user)) return 1
			...
			if(thermal_only && !istype(W, /obj/item/paper/thermal))
				to_chat(user, "<span class='alert'>This scanner only accepts thermal paper.</span>")
				return 0
			unReady()
			LIGHT_UP_HOUSING
...
/obj/item/mechanics/triplaser
	...
	proc/setRange(obj/item/W as obj, mob/user as mob)
		...
		if(!in_range(src, user) || user.stat)
			return 0
		range = clamp(rng, 1, 5)
		to_chat(user, "<span class='notice'>Range set to [range]!</span>")
		if(level == 1)
			rebeam()
		return 1
...
/obj/item/mechanics/hscan
	...
	proc/toggleSig(obj/item/W as obj, mob/user as mob)
		send_name = !send_name
		to_chat(user, "[send_name ? "Now sending user NAME":"Now sending user FINGERPRINT"]")
		return 1
	...
	attack_hand(mob/user as mob)
		if(level != 2 && isReady())
			if(ishuman(user) && user.bioHolder)
				...
				to_chat(user, "<span class='alert'>The hand scanner can only be used by humanoids.</span>")
				return
		else return ..(user)
...
/obj/item/mechanics/pausecomp
	...
	proc/setDelay(obj/item/W as obj, mob/user as mob)
		...
		if(!isnull(inp))
			delay = inp
			//tooltip_rebuild = 1
			to_chat(user, "Set delay to [inp]")
			return 1
		return 0

	proc/toggleDefault(obj/item/W as obj, mob/user as mob)
		changesig = !changesig
		to_chat(user, "Signal changing now [changesig ? "on":"off"]")
		return 1
...
/obj/item/mechanics/andcomp
	...
	proc/setTime(obj/item/W as obj, mob/user as mob)
		...
		if(!isnull(inp))
			timeframe = inp
			//tooltip_rebuild = 1
			to_chat(user, "Set Time Frame to [inp]")
			return 1
		return 0
...
/obj/item/mechanics/regreplace
	...
	proc/setPattern(obj/item/W as obj, mob/user as mob)
		...
		if(length(inp))
			...
			to_chat(user, "Pattern set to [inp]")
			//tooltip_rebuild = 1
			return 1
		return 0
	...
	proc/setReplacement(obj/item/W as obj, mob/user as mob)
		...
		if(length(inp))
			expressionrepl = inp
			inp = sanitize(html_encode(inp))
			to_chat(user, "Replacement set to [inp]")
			//tooltip_rebuild = 1
			return 1
		return 0
...
/obj/item/mechanics/regfind
	...
	proc/setRegex(obj/item/W as obj, mob/user as mob)
		...
		if(length(inp))
			...
			inp = sanitize(html_encode(inp))
			to_chat(user, "Expression Pattern set to [inp], Current Expression: [sanitize(html_encode(expression))]")
			//tooltip_rebuild = 1
			return 1
		return 0
	...
	proc/setFlags(obj/item/W as obj, mob/user as mob)
		...
		if(length(inp))
			...
			expression =("[expressionpatt]/[expressionflag]")
			inp = sanitize(html_encode(inp))
			to_chat(user, "Expression Flags set to [inp], Current Expression: [sanitize(html_encode(expression))]")
			//tooltip_rebuild = 1
			return 1
		return 0
	...
	proc/toggleReplaceing(obj/item/W as obj, mob/user as mob)
		...
		to_chat(user, "[replacesignal ? "Now forwarding own Signal":"Now forwarding found String"]")
		//tooltip_rebuild = 1
		return 1
...
/obj/item/mechanics/sigcheckcomp
	...
	proc/setTrigger(obj/item/W as obj, mob/user as mob)
		...
		if(length(inp))
			inp = adminscrub(inp)
			triggerSignal = inp
			to_chat(user, "String set to [inp]")
			//tooltip_rebuild = 1
			return 1
		return 0
	...
	proc/invertTrigger(obj/item/W as obj, mob/user as mob)
		not = !not
		to_chat(user, "[not ? "Component will now trigger when the String is NOT found.":"Component will now trigger when the String IS found."]")
		//tooltip_rebuild = 1
		return 1
...
/obj/item/mechanics/dispatchcomp
	...
	proc/toggleExactMatching(obj/item/W as obj, mob/user as mob)
		...
		to_chat(user, "Exact match mode now [exact_match ? "on" : "off"]")
		//tooltip_rebuild = 1
		return 1

	proc/toggleSingleOutput(obj/item/W as obj, mob/user as mob)
		single_output = !single_output
		to_chat(user, "Single output mode now [single_output ? "on" : "off"]")
		//tooltip_rebuild = 1
		return 1
	...
	proc/addFilter(var/comsig_target, atom/receiver, mob/user)
		...
		if (length(filter))
			...
			to_chat(user, "<span class='success'>Only passing messages that [exact_match ? "match" : "contain"] [filter] to the [receiver.name]</span>")
		else
			to_chat(user, "<span class='success'>Passing all messages to the [receiver.name]</span>")
		return
...
/obj/item/mechanics/sigbuilder
	...
	proc/setStartingStringManual(obj/item/W as obj, mob/user as mob)
		...
		to_chat(user, "String set to [bstr]")
		return 1
	...
	proc/setEndingStringManual(obj/item/W as obj, mob/user as mob)
		...
		if(!in_range(src, user) || user.stat)
			return 0
		setEndingString(inp)
		to_chat(user, "String set to [astr]")
		return 1
...
/obj/item/mechanics/wificomp
	...
	proc/setFreqManually(obj/item/W as obj, mob/user as mob)
		...
		if(!isnull(inp))
			set_frequency(inp)
			to_chat(user, "Frequency set to [inp]")
			//tooltip_rebuild = 1
			return 1
		return 0
	...
	proc/toggleAddressFiltering(obj/item/W as obj, mob/user as mob)
		...
		to_chat(user, "[only_directed ? "Now only reacting to Messages directed at this Component":"Now reacting to ALL Messages."]")
		//tooltip_rebuild = 1
		return 1

	proc/toggleForwardAll(obj/item/W as obj, mob/user as mob)
		forward_all = !forward_all
		to_chat(user, "[forward_all ? "Now forwarding all Radio Messages as they are.":"Now processing only sendmsg and normal PDA messages."]")
		//tooltip_rebuild = 1
		return 1
...
/obj/item/mechanics/selectcomp
	...
	proc/setSignalList(obj/item/W as obj, mob/user as mob)
		...
		if(numsig > 10) //Needs a limit because nerds are nerds
			to_chat(user, "<span class='alert'>This component can't handle more than 10 signals!</span>")
			return 0
		if(numsig)
			signals.Cut()
			current_index = 1
			to_chat(user, "Defining [numsig] Signals ...")
			for(var/i=0, i<numsig, i++)
				...
				if(length(signew))
					signals.Add(signew)
			to_chat(user, "Set [numsig] Signals!")
	...
	proc/setDelimetedList(obj/item/W as obj, mob/user as mob)
		...
		if(!in_range(src, user) || user.stat)
			return 0
		if(!newsigs)
			to_chat(user, "<span class='notice'>Signals remain unchanged!</span>")
			return 0
		...
		signals = built
		current_index = 1
		to_chat(user, "<span class='notice'>There are now [length(signals)] signals in the list.</span>")
		//tooltip_rebuild = 1
		return 1

	proc/toggleAnnouncements(obj/item/W as obj, mob/user as mob)
		announce = !announce
		to_chat(user, "Announcements now [announce ? "on":"off"]")
		//tooltip_rebuild = 1
		return 1

	proc/toggleRandom(obj/item/W as obj, mob/user as mob)
		random = !random
		to_chat(user, "[random ? "Now picking Items at random.":"Now using selected Items."]")
		//tooltip_rebuild = 1
		return 1

	proc/toggleAllowDuplicates(obj/item/W as obj, mob/user as mob)
		allowDuplicates = !allowDuplicates
		to_chat(user, "[allowDuplicates ? "Allowing addition of duplicate items." : "Not allowing addition of duplicate items."]")
		//tooltip_rebuild = 1
		return 1
...
/obj/item/mechanics/togglecomp
	...
	proc/setOnSignal(obj/item/W as obj, mob/user as mob)
		...
		if(length(inp))
			inp = adminscrub(inp)
			signal_on = inp
			to_chat(user, "On-Signal set to [inp]")
			//tooltip_rebuild = 1
			return 1
		return 0
	...
	proc/setOffSignal(obj/item/W as obj, mob/user as mob)
		...
		if(length(inp))
			inp = adminscrub(inp)
			signal_off = inp
			to_chat(user, "Off-Signal set to [inp]")
			//tooltip_rebuild = 1
			return 1
		return 0
...
/obj/item/mechanics/telecomp
	...
	proc/toggleSendOnly(obj/item/W as obj, mob/user as mob)
		...
		if(send_only)
			...
			src.overlays.Cut()
		to_chat(user, "Send-only Mode now [send_only ? "on":"off"]")
		//tooltip_rebuild = 1
		return 1
...
//если я правильно понял, by_type это лист, в котором они хранят ВСЕ сука объекты, которыехотят отслеживать. пиздец.
		//for_by_tcl(T, /obj/item/mechanics/telecomp)
		//лишь бы это меня не укусило в жопу...
		for(var/obj/item/mechanics/telecomp/T in world)
...
/obj/item/mechanics/miccomp
	...
	proc/toggleSender(obj/item/W as obj, mob/user as mob)
		...
		to_chat(user, "Show-Source now [add_sender ? "on":"off"]")
		return 1
...
/obj/item/mechanics/radioscanner
	...
	proc/setFreqMan(obj/item/W as obj, mob/user as mob)
		...
		if(!isnull(inp))
			set_frequency(inp)
			to_chat(user, "Frequency set to [frequency]")
			return 1
		return 0
...
/obj/item/mechanics/trigger/buttonPanel
	...
	proc/addButton(obj/item/W as obj, mob/user as mob)
		if(length(src.active_buttons) >= 10)
			to_chat(user, "<span class='alert'>There's no room to add another button - the panel is full</span>")
			return 0
		...
		if(length(new_label) && length(new_signal))
			...
			if(new_label in src.active_buttons)
				to_chat(user, "There's already a button with that label.")
				return 0
			src.active_buttons.Add(new_label)
			src.active_buttons[new_label] = new_signal
			to_chat(user, "Added button with label: [new_label] and value: [new_signal]")
			//tooltip_rebuild = 1
			return 1
		return 0
	...
	proc/removeButton(obj/item/W as obj, mob/user as mob)
		if(!length(src.active_buttons))
			to_chat(user, "<span class='alert'>[src] has no active buttons - there's nothing to remove!</span>")
		else
			...
			to_chat(user, "Removed button labeled [to_remove]")
			//tooltip_rebuild = 1
			return 1
		return 0
	...
	attack_hand(mob/user as mob)
		if (level == 1)
			if (length(src.active_buttons))
				...
				if (!selected_button || selected_button == "*CANCEL*" || !in_range(src, user)) return
				LIGHT_UP_HOUSING
				flick(icon_down, src)
				SEND_SIGNAL(src,COMSIG_MECHCOMP_TRANSMIT_SIGNAL, src.active_buttons[selected_button])
				return 1
			else
				to_chat(user, "<span class='alert'>[src] has no active buttons - there's nothing to press!</span>")
		else return ..(user)
...
/obj/item/mechanics/gunholder
	...
	proc/removeGun(obj/item/W as obj, mob/user as mob)
		if(Gun)
			...
			return 1
		to_chat(user, "<span class='alert'>There is no gun inside this component.</span>")
		return 0
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if(gun_fits)
			if(!Gun)
				to_chat(user, "You put the [W] inside the [src].")
				...
				to_chat(user, "There is already a [Gun] inside the [src]")
		else
			to_chat(user,"The [W.name] isn't compatible with this component.", "red")
		return 0
...
/obj/item/mechanics/instrumentPlayer //Grayshift's musical madness
	...
	proc/removeInstrument(obj/item/W as obj, mob/user as mob)
		if(instrument)
			...
			to_chat(user, "<span class='alert'>There is no instrument inside this component.</span>")
		return 0

	attackby(obj/item/W as obj, mob/user as mob)
		if (..(W, user)) return 1
		else if (instrument) // Already got one, chief!
			to_chat(user, "There is already \a [instrument] inside the [src].")
			return 0
			...
			to_chat(user,"\The [W] isn't compatible with this component.", "red")

		if (instrument) // You did it, boss. Now log it because someone will figure out a way to abuse it
			to_chat(user, "You put [W] inside [src].")
			//logTheThing("station", user, null, "adds [W] to [src] at [log_loc(src)].")
			user.drop_item()
...
/obj/item/mechanics/association
	...
	proc/getMapAsString(obj/item/W as obj, mob/user as mob)
		...
		to_chat(user, "[length(mapLines) ? mapLines.Join("<br>") : ""]")
	...
	proc/setMode(obj/item/W as obj, mob/user as mob)
		...
		to_chat(user, "Mode set to [input]")
		//tooltip_rebuild = 1
		return 1
	...
	proc/addItemManual(obj/item/W as obj, mob/user as mob)
		...
		if (mode == 0) // Mutable
			...
			if (!isnull(map[inputKey]))
				to_chat(user, "IMMUTABLE MODE ERROR: An association already exists for that key")
				return 0
			map.Add(inputKey)
			map[inputKey] = inputValue
		else // List
		...
		to_chat(user, "Set value of [inputKey] to [map[inputKey]]")
		//tooltip_rebuild = 1
		return 1

	proc/removeItemManual(obj/item/W as obj, mob/user as mob)
		if (!length(map))
			to_chat(user, "<span class='alert'>[src] has no associations - there's nothing to remove!</span>")
			return 0
		...
		map.Remove(input)
		to_chat(user, "Removed key [input] and value [removedValue]")
		//tooltip_rebuild = 1
		return 1

	proc/clear(obj/item/W as obj, mob/user as mob)
		map.Cut()
		to_chat(user, "Associations map cleared")
		return 1
