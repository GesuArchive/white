	...
	switch(stage)
		...
		if(3)
			...
			if(DT_PROB(10, delta_time))
				to_chat(affected_mob, "<span class='danger'>Вы чувствуете себя возбужденным.</span>")
		if(4)
			to_chat(affected_mob, "<span class='userdanger'>Вы чувствуете очень сильное напряжение в вашем половом органе!</span>")
			...
			if(DT_PROB(20, delta_time))
				to_chat(affected_mob, "<span class='danger'>Вы чувствуете себя ОЧЕНЬ возбужденным.</span>")
		if(5)
			to_chat(affected_mob, "<span class='userdanger'>Вы непроизвольно кончаете!</span>")
