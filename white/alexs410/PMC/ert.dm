...
/obj/item/gun/energy/laser/smg/selfcharge/shoot_live_shot(mob/living/user, pointblank, atom/pbtarget, message)
	if(prob(0.1))
		if(prob(50))
			user.visible_message("<span class='danger'>[src] в руках [user] разрывается снопом искр.</span>")
			to_chat(user,"<span class='warning'>[src] загорается в моих руках. Мама... </span>")
			...
			user.visible_message("<span class='danger'>[user] целиком засасывается в [src].</span>")
			to_chat(user,"<span class='warning'>Куда меня засасываеееее...</span>")
			qdel(user)
		qdel(src)
		return
	. = ..()
