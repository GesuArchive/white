...
/obj/machinery/lazylift_button/attack_hand(mob/user)
	...
	if(isliving(user) && user.mouse_opacity) //Don't display things like AIs or admin ghosts clicking the button.
		visible_message("<span class='notice'>[user] [pick("спокойно нажимает на", "давит на", "тыкает по", "шлёпает по", "ударяет")] [src] " , "<span class='notice'>Жму [src.name].</span>")
...
/obj/machinery/lazylift/Hear(message, atom/movable/speaker, message_language, raw_message, radio_freq, list/spans, message_mode)
	...
	if(!L?.has_language(message_language))
		say("[pick("Простите, не понимаю!", "Пожалуйста, говорите громко и отчётливо в микрофон.", "Ошибка обработки голосового запроса.")]")
		return
	...
	for(var/obj/machinery/lazylift/LL in master.decks)
		if(findtext(raw_message, "палуба") || findtext(raw_message, "этаж")) //Alright they're asking for a specific deck.
...
/obj/machinery/lazylift/proc/setup()
	...
	for(var/turf/T in platform)
		...
		if(door)
			...
			platform -= T
	if(!door_turf)
		message_admins("ПРОЕБАЛИ ДВЕРЬ ОТ [src.name]!")
		return
...
/obj/machinery/lazylift/master
	name = "Панель управления элеватором"
...
/obj/machinery/lazylift/master/emag_act(mob/user)
	if(obj_flags & EMAGGED)
		return
	set_music('white/jhnazar/sound/effects/lift/emagged.ogg') //Wonderful music, manifique, c'est encroyable.
	req_access = list()
	obj_flags |= EMAGGED
...
/obj/machinery/lazylift/master/proc/path_to(target_deck, startDeck=platform_location.deck)
	...
	if(!path.len)
		...
		in_use = FALSE //in use more like in PUCE am I right gamers???
		return FALSE //FUCK
	playsound(platform_location.loc, 'white/jhnazar/sound/effects/turbolift/turbolift-close.ogg', 100, FALSE)
...
/obj/machinery/lazylift/master/proc/sound_effect(start)
	if(start)
		...
		if(play_voice_lines)
			playsound(platform_location, 'white/jhnazar/sound/effects/lift/mindthegap.ogg', 100, FALSE)
			platform_location.say("ПОЖАЛУЙСТА, ОБРАТИТЕ ВНИМАНИЕ НА РАЗРЫВ!!!")
		for(var/mob/M in get_area(src))
