...
/obj/machinery/advanced_airlock_controller/update_icon(use_hash = FALSE)
	...
	if(vis_target)
		...
		var/mutable_appearance/M = mutable_appearance(icon, "hologram-line", plane = ABOVE_LIGHTING_PLANE)
		M.transform = TR
		add_overlay(M)
	...
	switch(buildstage)
		...
		if(0)
			...
			if(W.tool_behaviour == TOOL_WRENCH)
				to_chat(user, span_notice("You detach <b>[src.name]</b> from the wall."))
				W.play_tool_sound(src)
	...
	for(var/A in airlocks)
		...
		if(islist(airlock.req_access) && airlock.req_access.len)
			...
			for(var/I in 1 to airlock.req_access.len)
				if(I != 1)
					access_str += ", "
				access_str += SSid_access.get_access_desc(airlock.req_access[I])
		if(islist(airlock.req_one_access) && airlock.req_one_access.len)
			...
			for(var/I in 1 to airlock.req_one_access.len)
				if(I != 1)
					access_str += ", "
				access_str += SSid_access.get_access_desc(airlock.req_one_access[I])
