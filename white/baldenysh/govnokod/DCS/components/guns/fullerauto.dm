...
/datum/component/automatic_fire_funny
	var/client/clicker
	...
	var/autofire_shot_delay = 0.01 //FUNNI
	...
	var/trolling_execution_protocols_active = FALSE

/datum/component/automatic_fire_funny/Initialize(_autofire_shot_delay)
	. = ..()
	if(!isgun(parent))
...
/datum/component/automatic_fire_funny/Destroy()
	autofire_off()
	return ..()

/*
/datum/component/automatic_fire_funny/process(delta_time)
...
/datum/component/automatic_fire_funny/proc/wake_up(datum/source, mob/user, slot)
	SIGNAL_HANDLER
...
/datum/component/automatic_fire_funny/proc/autofire_on(client/usercli)
	SIGNAL_HANDLER
	...
	RegisterSignals(parent, list(COMSIG_PARENT_PREQDELETED, COMSIG_ITEM_DROPPED), PROC_REF(autofire_off))
	parent.RegisterSignal(src, COMSIG_AUTOFIRE_ONMOUSEDOWN, TYPE_PROC_REF(/obj/item/gun, autofire_bypass_check))

	parent.RegisterSignal(parent, COMSIG_AUTOFIRE_SHOT, TYPE_PROC_REF(/obj/item/gun, do_autofire_funny))


/datum/component/automatic_fire_funny/proc/autofire_off(datum/source)
	SIGNAL_HANDLER
	if(autofire_stat == AUTOFIRE_STAT_IDLE)
		return
	if(autofire_stat == AUTOFIRE_STAT_FIRING)
...
/datum/component/automatic_fire_funny/proc/on_client_login(mob/source)
	SIGNAL_HANDLER
	if(!source.client)
		return
...
/datum/component/automatic_fire_funny/proc/on_mouse_down(client/source, atom/_target, turf/location, control, params)
	...
	if(isnull(location)) //Clicking on a screen object.
		...
		_target = parse_caught_click_modifiers(modifiers["screen-loc"], get_turf(source.eye), source)
		if(!_target)
...
//Dakka-dakka
/datum/component/automatic_fire_funny/proc/start_autofiring()
	if(autofire_stat == AUTOFIRE_STAT_FIRING)
		return
	...
	if(isgun(parent))
		...
		if(!shoota.on_autofire_funny_start(shooter)) //This is needed because the minigun has a do_after before firing and signals are async.
			stop_autofiring()
			return
	if(autofire_stat != AUTOFIRE_STAT_FIRING)
	...
	INVOKE_ASYNC(src, PROC_REF(initiate_a_little_bit_of_trolling)) //amog

/datum/component/automatic_fire_funny/proc/on_mouse_up(datum/source, atom/object, turf/location, control, params)
	SIGNAL_HANDLER
...
/datum/component/automatic_fire_funny/proc/stop_autofiring(datum/source, atom/object, turf/location, control, params)
	SIGNAL_HANDLER
	if(autofire_stat != AUTOFIRE_STAT_FIRING)
		return
	autofire_stat = AUTOFIRE_STAT_ALERT
...
/datum/component/automatic_fire_funny/proc/on_mouse_drag(client/source, atom/src_object, atom/over_object, turf/src_location, turf/over_location, src_control, over_control, params)
	...
	if(isnull(over_location)) //This happens when the mouse is over an inventory or screen object, or on entering deep darkness, for example.
		...
		var/new_target = parse_caught_click_modifiers(modifiers["screen-loc"], get_turf(source.eye), source)
		mouse_parameters = params
		if(!new_target)
...
/datum/component/automatic_fire_funny/proc/process_shot()
	if(autofire_stat != AUTOFIRE_STAT_FIRING)
		return FALSE
...
// Gun procs.
/obj/item/gun/proc/on_autofire_funny_start(mob/living/shooter)
	if(shooter.stat || !can_trigger_gun(shooter))
		return FALSE
	if(!can_shoot())
...
/obj/item/gun/proc/do_autofire_funny(datum/source, atom/target, mob/living/shooter, params)
	SIGNAL_HANDLER
	if(shooter.stat)
		return NONE
	if(!can_shoot())
		shoot_with_empty_chamber(shooter)
		return NONE
	INVOKE_ASYNC(src, PROC_REF(do_autofire_shot_funny), source, target, shooter, params)
	return COMPONENT_AUTOFIRE_SHOT_SUCCESS //All is well, we can continue shooting.

/obj/item/gun/proc/do_autofire_shot_funny(datum/source, atom/target, mob/living/shooter, params)
...
/datum/component/automatic_fire_funny/proc/initiate_a_little_bit_of_trolling()
	trolling_execution_protocols_active = TRUE
	while(trolling_execution_protocols_active)
		process_shot()
		sleep(autofire_shot_delay)
