...
/datum/surgery_step/handle_teeth/preop(mob/user, mob/living/carbon/human/target, target_zone, obj/item/stack/teeth/T, datum/surgery/surgery)
	...
	if(istype(T))
		if(O.get_teeth() >= O.max_teeth)
			...
			return -1
		user.visible_message("<span class='notice'>[user] начинает вставлять [T] в рот [target].</span>")
	else
		user.visible_message("<span class='notice'>[user] начинает вырывать зубы [target].</span>")
	...
	if(istype(T))
		if(O.get_teeth()) //Has teeth, check if they need "refilling"
			...
			if(F)
				...
				// E.forceMove(target)
				T = E
			user.visible_message("<span class='notice'>[user] вставляет [amt] [skloname(T.name, VINITELNI)] в рот [target]!</span>")
			return 1
