...
/client/proc/cmd_mentor_pm_panel()
	...
	if(!is_mentor())
		to_chat(src, "<span class='danger'>Ошибка: Только знатоки или администраторы могут использовать эту команду.</span>")
		return
...
/client/proc/cmd_mentor_pm(whom, msg)
	...
	if(is_mentor(whom))
		to_chat(GLOB.mentors, "<font color='purple'>[src] начинает отвечать [whom].</font>")
	...
	if(!msg)
		...
		if(!msg)
			if (is_mentor(whom))
				to_chat(GLOB.mentors, "<span class='purple'>[src] прекращает отвечать [whom].</span>")
			return
	...
	if(C.is_mentor())
		if(is_mentor())//both are mentors
			to_chat(C, "<span class='purple'>Сообщение от Знатока <b>[key_name_mentor(src, C, 1, 0, 0)]</b>: [msg]</span>")
			to_chat(src, "<span class='blue'>Обращение Знатока к <b>[key_name_mentor(C, C, 1, 0, 0)]</b>: [msg]</font>")

		else		//recipient is a mentor but sender is not
			to_chat(C, "<span class='purple'>Ответ от <b>[key_name_mentor(src, C, 1, 0, show_char)]</b>: [msg]</span>")
			to_chat(src, "<span class='pink'>Ответ от <b>[key_name_mentor(C, C, 1, 0, 0)]</b>: [msg]</span>")

	else
		if(is_mentor())	//sender is a mentor but recipient is not.
			to_chat(C, "<span class='purple'>Сообщение от Знатока <b>[key_name_mentor(src, C, 1, 0, 0)]</b>: [msg]</span>")
			to_chat(src, "<span class='pink'>Обращение Знатока к <b>[key_name_mentor(C, C, 1, 0, show_char)]</b>: [msg]</span>")
	...
	for(var/it in GLOB.mentors)
		...
		if(X.key!=key && X.key!=C.key)	//check client/X is an Mentor and isn't the sender or recipient
			to_chat(X, "<B><span class='pink'>ЗНАТОК: [key_name_mentor(src, X, 0, 0, show_char_sender)]-&gt;[key_name_mentor(C, X, 0, 0, show_char_recip)]:</span></B> <span class='blue'>[msg]</span>") //inform X
