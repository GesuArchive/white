...
/client/proc/cmd_mentor_pm_panel()
	...
	var/target = tgui_input_list(src, "Кому же мы напишем?", "Сообщение знатока", sorted)
...
/client/proc/cmd_mentor_pm(whom, msg)
	...
	if(!msg)
		msg = tgui_input_text(src, "Сообщение:", "Приватное сообщение")
	...
	for(var/it in GLOB.mentors)
		...
		if(mentor && target && mentor.key != key && mentor.key != target.key)	//check client/X is an Mentor and isn't the sender or recipient
