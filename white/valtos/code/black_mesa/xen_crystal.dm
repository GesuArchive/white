...
/obj/structure/xen_crystal/attack_hand(mob/living/user, list/modifiers)
	. = ..()
	if(harvested)
		to_chat(user, "<span class='warning'>[src] has already been harvested!</span>")
		return
	to_chat(user, "<span class='notice'>You start harvesting [src]!</span>")
...
/obj/structure/xen_crystal/proc/harvest(mob/living/user)
	if(harvested)
		return
	to_chat(user, "<span class='notice'>You harvest [src]!</span>")
...
/obj/item/grenade/xen_crystal/detonate(mob/living/lanced_by)
	for(var/mob/living/mob_to_neutralize in view(src, effect_range))
		...
		mob_to_neutralize.visible_message("<span class='green'>[mob_to_neutralize] is overcome by a wave of peace and tranquility!</span>")
