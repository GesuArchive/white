		...
		if (charge > 0)
			if (istype(A, /obj/item/stack/rods))
				...
				if (R.use(1))
					...
					user.visible_message("<span class='notice'>[user] аккуратно устанавливает [chambered.loaded_projectile] в [src].</span>" , \
                                         "<span class='notice'>Аккуратно устанавливаю [chambered.loaded_projectile] в [src].</span>")
		else
			to_chat(user, "<span class='warning'>Стоит натянуть тетиву перед установкой снаряда!</span>")
	else
		to_chat(user, "<span class='warning'>Здесь уже есть [chambered.loaded_projectile] внутри!</span>")
...
/obj/item/gun/ballistic/crossbow/attack_self(mob/living/user)
	if (!chambered)
		...
		user.visible_message("<span class='notice'>[user] достаёт [chambered.loaded_projectile] из [src].</span>" , \
							"<span class='notice'>Достаю [chambered.loaded_projectile] из [src].</span>")
