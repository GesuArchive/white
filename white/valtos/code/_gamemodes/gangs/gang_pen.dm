...
/obj/item/pen/gang/attack(mob/living/M, mob/user, stealth = TRUE)
	...
	if(!..())
		return
	if(cooldown)
		to_chat(user, span_warning("[capitalize(src.name)] needs more time to recharge before it can be used."))
		return
	if(!M.client || !M.mind)
...
/obj/item/pen/gang/proc/cooldown()
	...
	var/mob/M = loc
	if(istype(M))
		to_chat(M, span_notice("[icon2html(src, M)] [src][(loc == M)?(""):(" in your [loc]")] vibrates softly. It is ready to be used again."))
