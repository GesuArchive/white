			...
			if(0)
				...
				for(var/datum/mind/bloodsucker_minds as anything in get_antag_minds(/datum/antagonist/bloodsucker))
					...
					if(bloodsuckerdatum.altar_uses > 0)
						to_chat(bloodsuckerdatum, "<span class='notice'>Your Altar uses have been reset!</span>")
						bloodsuckerdatum.altar_uses = 0
				warn_daylight(4, "<span class='userdanger'>Solar flares bombard the station with deadly UV light!<br><span class = ''>Stay in cover for the next [TIME_BLOODSUCKER_DAY] seconds or risk Final Death!</span>", \
...
/obj/effect/sunlight/proc/warn_daylight(danger_level = 0, vampwarn = "", vassalwarn = "", hunteralert = "")
	for(var/datum/mind/bloodsucker_minds as anything in get_antag_minds(/datum/antagonist/bloodsucker))
		...
		if(bloodsucker_minds.current)
			switch(danger_level)
				if(1)
					bloodsucker_minds.current.playsound_local(null, 'sound/runtime/chatter/griffin_3.ogg', 50 + danger_level, 1)
				if(2)
					bloodsucker_minds.current.playsound_local(null, 'sound/runtime/chatter/griffin_5.ogg', 50 + danger_level, 1)
				if(3)
	...
	for(var/datum/mind/bloodsucker_minds as anything in get_antag_minds(/datum/antagonist/bloodsucker))
		...
		if(istype(bloodsucker_minds.current.loc, /obj/structure))
			...
			if(COOLDOWN_FINISHED(bloodsuckerdatum, bloodsucker_spam_sol_burn)) // Closets offer SOME protection
				to_chat(bloodsucker_minds, "<span class='warning'>Your skin sizzles. [bloodsucker_minds.current.loc] doesn't protect well against UV bombardment.</span>")
			...
			if(COOLDOWN_FINISHED(bloodsuckerdatum, bloodsucker_spam_sol_burn))
				if(bloodsuckerdatum.bloodsucker_level > 0)
					to_chat(bloodsucker_minds, "<span class='userdanger'>The solar flare sets your skin ablaze!</span>")
				else
					to_chat(bloodsucker_minds, "<span class='userdanger'>The solar flare scalds your neophyte skin!</span>")
				COOLDOWN_START(bloodsuckerdatum, bloodsucker_spam_sol_burn, BLOODSUCKER_SPAM_SOL) //This should happen twice per Sol
