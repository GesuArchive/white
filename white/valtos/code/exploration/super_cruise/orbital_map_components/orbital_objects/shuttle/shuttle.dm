...
/datum/orbital_object/shuttle/proc/strand_shuttle()
	...
	if(!docking_target)
		...
		//Create a new orbital waypoint to drop at
			...
			shuttle_location.name = "Заблудший [name]"
...
/datum/orbital_object/shuttle/proc/perform_interdiction()
	...
	if(!length(interdicted_shuttles))
		...
		return FALSE
	SEND_SIGNAL(src, COMSIG_ORBITAL_BODY_MESSAGE, "Перехват активирован, замедляемся...")
	...
	priority_announce("Перехват круиза обнаружен, перехваченные шаттлы видны на местных GPS маячках. Источник: [name]")
