...
/obj/machinery/computer/locator
	name = "триангуляционный компьютер"
	desc = "Компьютерная консоль, которую можно использовать для триангуляции положения орбитальных маяков."
	icon_screen = "shuttle"
...
/obj/machinery/computer/locator/ui_act(action, params)
	...
	//To many pings
	if(length(pings) > 30)
		say("Локальная память заполнена, удалите некоторые результаты")
		return
	//Check cooldown
	if(world.time < next_ping_cooldown)
		say("Пожалуйста, подождите не менее 3 секунд между каждым пингом.")
		return
	...
	if(!istype(current_location))
		current_location = SSorbits.assoc_z_levels["[z]"]
	if(!istype(current_location))
		say("Не удалось пропинговать, неверное местоположение.")
		return
	//Perform action
