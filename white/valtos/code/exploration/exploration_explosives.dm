...
/obj/item/grenade/exploration/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/exploration_detonator))
		...
		to_chat(user, "<span class='notice'>Связаываю [src] и [W].</span>")
		return
	. = ..()
...
/obj/item/grenade/exploration/afterattack(atom/movable/AM, mob/user, flag)
	...
	if(!length(attached_detonators))
		to_chat(user, "<span class='notice'>[src] должен быть связан с детонатором сначала!</span>")
		return
	...
	to_chat(user, "<span class='notice'>Начинаю устанавливать [src].</span>")
	...
	if(do_after(user, 30, target = AM))
		...
		to_chat(user, "<span class='notice'>Устанавливаю заряд.</span>")
...
/obj/item/exploration_detonator/attack_self(mob/user)
	...
	if(is_station_level(T) && !(obj_flags & EMAGGED))
		to_chat(user, "<span class='warning'>Файрволл станции блокирует подрыв.</span>")
		return
	...
	for(var/obj/item/grenade/exploration/exploration in linked_explosives)
		if(get_dist(exploration.target, user) <= range)
			...
			explosives_trigged ++
	to_chat(user, "<span class='notice'>[explosives_trigged] зарядов было активировано.</span>")
