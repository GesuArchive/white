...
/obj/item/organ/internal/shadowtumor/on_find(mob/living/finder)
	. = ..()
	finder.visible_message(span_danger("[finder] opens up [owner] skull, revealing a pulsating black mass, with red tendrils attaching it to [owner.ru_ego()] brain."))
...
/obj/item/organ/internal/shadowtumor/Remove(mob/living/carbon/M, special)
	...
	M.update_sight()
	M.remove_thrall()
	M.visible_message(span_warning("A strange black mass falls from [M] head!"))
	return ..()
