...
/obj/effect/proc_holder/spell/self/shadowling_hatch/cast(list/targets,mob/user = usr)
	...
	var/hatch_or_no = tgui_alert(H,"Are you sure you want to hatch? You cannot undo this!",,"Yes","No")
	switch(hatch_or_no)
		if("No")
		...
		if("Yes")
			H.notransform = TRUE
			H.visible_message(span_warning("[H] things suddenly slip off. They hunch over and vomit up a copious amount of purple goo which begins to shape around them!") , \
			...
			if(!do_mob(H,H,100,1))
				return
			H.visible_message(span_warning("<b>The skin on [H] back begins to split apart. Black spines slowly emerge from the divide.</b>") , \
...
/obj/effect/proc_holder/spell/self/shadowling_ascend/cast(list/targets,mob/user = usr)
	...
	if(!shadowling_check(H))
		return
	var/hatch_or_no = tgui_alert(H,"It is time to ascend. Are you sure about this?",,"Yes","No")
	switch(hatch_or_no)
		if("No")
		...
		if("Yes")
			H.notransform = 1
			H.visible_message(span_warning("[H] things suddenly slip off. They gently rise into the air, red light glowing in their eyes.") , \
			...
			if(!do_mob(H,H,50,1))
				return
			H.visible_message(span_warning("[H] skin begins to crack and harden.") , \
							span_shadowling("Your flesh begins creating a shield around yourself."))
			if(!do_mob(H,H,100,1))
				return
			H.visible_message(span_warning("The small horns on [H] head slowly grow and elongate.") , \
								span_shadowling("Your body continues to mutate. Your telepathic abilities grow.")) //y-your horns are so big, senpai...!~
			if(!do_mob(H,H,90,1))
				return
			H.visible_message(span_warning("[H] body begins to violently stretch and contort.") , \
			...
			for(var/obj/machinery/power/apc/A in GLOB.apcs_list)
				A.overload_lighting()
			//SSachievements.unlock_achievement(/datum/achievement/greentext/slingascend, H.client) -- nahuj?
