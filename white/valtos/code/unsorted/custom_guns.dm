...
/obj/item/gun/proc/change_skin(datum/source, mob/user)
	if(ishuman(user) && (user?.ckey in GLOB.custom_skin_donators) && !custom_skin_name)
		...
		temp_icon = temp_icon.Blend(icon('white/valtos/icons/custom_guns.dmi', custom_skin_name), ICON_ADD)
		lefthand_file = mutable_appearance(temp_icon, icon_state)

		temp_icon = icon(righthand_file, icon_state, , 1)
		temp_icon = temp_icon.Blend(icon('white/valtos/icons/custom_guns.dmi', custom_skin_name), ICON_ADD)
		righthand_file = mutable_appearance(temp_icon, icon_state)

		temp_icon = icon(worn_icon, icon_state, , 1)
		temp_icon = temp_icon.Blend(icon('white/valtos/icons/custom_guns.dmi', custom_skin_name), ICON_ADD)
		worn_icon = mutable_appearance(temp_icon, icon_state)
