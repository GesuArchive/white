...
/obj/structure/flippedtable/AltClick(mob/user)
	...
	if(!istype(user) || !user.can_interact_with(src))
		return FALSE
	user.visible_message("<span class='danger'><b>[user]</b> начинает ставить на место <b>[src.name]</b>!</span>", "<span class='notice'>Начинаю ставить на место <b>[src.name]</b>!</span>")
	if(do_after(user, max_integrity/4))
		var/obj/structure/table/T = new table_type(src.loc)
		T.update_integrity(src.get_integrity())
		user.visible_message("<span class='danger'><b>[user]</b> ставит на место <b>[src.name]</b>!</span>", "<span class='notice'>Ставлю на место <b>[src.name]</b>!</span>")
...
/obj/structure/table/AltClick(mob/living/user)
	...
	if(!istype(user) || !user.can_interact_with(src) || isobserver(user))
		return
	if(can_flip)
		user.visible_message("<span class='danger'><b>[user]</b> начинает переворачивать <b>[src.name]</b>!</span>", "<span class='notice'>Начинаю переворачивать <b>[src.name]</b>!</span>")
		if(do_after(user, max_integrity/4))
			...
			user.visible_message("<span class='danger'><b>[user]</b> переворачивает <b>[src.name]</b>!</span>", "<span class='notice'>Переворачиваю <b>[src.name]</b>!</span>")
			playsound(src, 'sound/items/trayhit2.ogg', 100)
			qdel(src)
