...
/obj/structure/chair/noose/user_buckle_mob(mob/living/carbon/human/M, mob/user, check_loc = TRUE)
	...
	if(do_mob(user, M, user == M ? 10:200))
		if(buckle_mob(M))
			user.visible_message("<span class='warning'>[user] вешает [M != user ? "[M]" : "себя"]!</span>",\
			...
			M.noosed = TRUE
			return TRUE
	user.visible_message("<span class='warning'>[user] не может накинуть вешалку на шею [M]!</span>",\
