...
/datum/component/glutton/process()
	...
	if(!eat() && attempts_to_eat > 10)
		our_eater.visible_message("<span class='danger'>[our_eater] делает [bites_taken] укусов и лопается!</span>", "<span class='userdanger'>Лопаюсь после [bites_taken] укусов!</span>")
		our_eater.gib()
...
/datum/component/glutton/proc/eat()
	...
	if(tasty_thing)
		...
		if(E)
			our_eater.visible_message("<span class='notice'>[our_eater] кушает [tasty_thing].</span>", "<span class='notice'>Кушаю [tasty_thing].</span>")
