	...
	if(contents.len)
		if(istype(contents[contents.len], /obj/item/ingot))
			...
			if(N.progress_current == N.progress_need + 1)
				...
				if(istype(O, /obj/item/katanus) || istype(O, /obj/item/dagger) || istype(O, /obj/item/cep) || istype(O, /obj/item/zwei))
...
/obj/item/cep/afterattack(atom/target, mob/user, proximity)
	. = ..()
	if(!proximity)
		return
