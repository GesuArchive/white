...
/datum/action/bloodsucker/feed/CheckCanUse(display_error)
	...
	//	to_chat(owner, span_warning("You aren't grabbing anyone!"))
	//	return FALSE
...
/datum/action/bloodsucker/feed/ActivatePower()
	...
	if (amSilent)
		//if (!iscarbon(target))
		//	user.visible_message(span_notice("[user] shifts [target] closer to [user.ru_ego()] mouth.") ,
		//					 	 span_notice("You secretly slip your fangs into [target] flesh.") ,
		...
		user.visible_message(span_notice("[user] puts [target] wrist up to [user.ru_ego()] mouth.") , \
						 	 span_notice("You secretly slip your fangs into [target] wrist.[deadmessage]") ,
		...
		user.visible_message(span_warning("[user] closes [user.ru_ego()] mouth around [target] neck!") , \
						 span_warning("You sink your fangs into [target] neck."))
	...
	while (bloodsuckerdatum && target && active)
		...
		if (!do_mob(user, target, 20, 0, 0, extra_checks=CALLBACK(src, .proc/ContinueActive, user, target)))
			...
			if (amSilent)
				...
				user.visible_message(span_danger("[user] is ripped from [target] throat. [target.ru_ego(TRUE)] blood sprays everywhere!") , \
						 			 span_userdanger("Your teeth are ripped from [target] throat. [target.ru_ego(TRUE)] blood sprays everywhere!"))
		...
		if (target.stat >= DEAD)
			...
			if (!warning_target_dead)
				to_chat(user, span_notice("Your victim is dead. [target.ru_ego(TRUE)] blood barely nourishes you."))
	...
	if (amSilent)
		...
		user.visible_message(span_warning("[user] unclenches their teeth from [target] neck.") , \
