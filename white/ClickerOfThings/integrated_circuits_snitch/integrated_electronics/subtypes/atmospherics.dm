...
#define INTEGRATED_PUMP_MAX_PRESSURE (ONE_ATMOSPHERE*24)
...
/obj/item/integrated_circuit_old/atmospherics/Initialize(mapload)
...
/obj/item/integrated_circuit_old/atmospherics/pump/Initialize(mapload)
	air_contents = new(volume)
	extended_desc += " Use negative pressure to move air from target to source. \
					...
					The pressure limit for circuit pumps is [round(INTEGRATED_PUMP_MAX_PRESSURE)] kPa."
	. = ..()
...
/obj/item/integrated_circuit_old/atmospherics/pump/volume/move_gas(datum/gas_mixture/source_air, datum/gas_mixture/target_air)
	...
	if((source_air.return_pressure() < 0.01) || (target_air.return_pressure() >= INTEGRATED_PUMP_MAX_PRESSURE))
		return

	//The second part of the min caps the pressure built by the volume pumps to the max pump pressure
	var/transfer_ratio = min(transfer_rate,target_air.return_volume()*INTEGRATED_PUMP_MAX_PRESSURE/source_air.return_pressure())/source_air.return_volume()
...
/obj/item/integrated_circuit_old/atmospherics/connector/Initialize(mapload)
	air_contents = new(volume)
...
/obj/item/integrated_circuit_old/atmospherics/pump/filter/do_work()
	...
	if(contaminated_air.return_pressure() >= INTEGRATED_PUMP_MAX_PRESSURE || filtered_air.return_pressure() >= INTEGRATED_PUMP_MAX_PRESSURE)
		return
	...
	for(var/filtered_gas in removed.get_gases())
		...
		if(filtered_out.gases[filtered_gas][GAS_META][META_GAS_NAME] in wanted)
...
/obj/item/integrated_circuit_old/atmospherics/pump/filter/Initialize(mapload)
	air_contents = new(volume)
	. = ..()
...
/obj/item/integrated_circuit_old/atmospherics/pump/mixer/do_work()
	...
	if(output_gases.return_pressure() >= INTEGRATED_PUMP_MAX_PRESSURE)
		return
...
/obj/item/integrated_circuit_old/atmospherics/tank/Initialize(mapload)
	air_contents = new(volume)
	START_PROCESSING(SSobj, src)
...
/obj/item/integrated_circuit_old/atmospherics/cooler/Initialize(mapload)
	air_contents = new(volume)
	START_PROCESSING(SSobj, src)
	. = ..()
...
/obj/item/integrated_circuit_old/input/tank_slot/Initialize(mapload)
	START_PROCESSING(SSobj, src)
	. = ..()
...
#undef INTEGRATED_PUMP_MAX_PRESSURE
#undef INTEGRATED_PUMP_MAX_VOLUME
