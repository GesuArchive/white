...
/obj/item/integrated_circuit_old/power/transmitter/do_work()
	...
	if(cell)
		...
		if(A.Adjacent(B))
			...
			var/list/U=A.get_all_contents(/obj/item/integrated_circuit_old/power/transmitter)
...
/obj/item/integrated_circuit_old/power/transmitter/large/do_work()
	if(..()) // If the above code succeeds, do this below.
		...
		if(prob(20))
			...
			s.set_up(12, 1, src)
			s.start()
			acting_object.visible_message(span_warning("<b>[capitalize(acting_object)]</b> makes some sparks!"))
	return TRUE
