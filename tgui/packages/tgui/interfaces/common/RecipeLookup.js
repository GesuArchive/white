...
export const RecipeLookup = (props, context) => {
  ...
  if (!recipe) {
    return <Box>Не выбрана реакция!</Box>;
  }
        ...
        <LabeledList>
          <LabeledList.Item label="Оптимальная зона pH">
            <Box position="relative">
              <Tooltip content="Если реакция будет проходить в рамках этого pH, то чистота будет 100%">
          ...
          {!!recipe.inversePurity && (
            <LabeledList.Item label="Инверсная чистота">
              <Box position="relative">
                <Tooltip content="Если чистота ниже этого процента, то 100% реагентов конвертирует в инверсивный реагент при реакции.">
          ...
          {!!recipe.minPurity && (
            <LabeledList.Item label="Минимальная чистота">
              <Box position="relative">
                <Tooltip content="Если чистота ниже этого параметра, это вызовет негативные эффекты и если реакция пройдёт до конца с такими условиями, то получится провальный реагент.">
      ...
      <LabeledList.Item bold label="Динамика">
        <LabeledList>
          <LabeledList.Item label="Оптимальная скорость">
            <Tooltip content="Максимальная скорость прохождения реакции. Можете определить её по плато на графике выше.">
              <Box position="relative">{recipe.thermoUpper + 'е/с'}</Box>
            </Tooltip>
          </LabeledList.Item>
        </LabeledList>
        <Tooltip content="Тепло, которое генерируется при реакции - экзотермическая производит тепло, эндотермическая поглощает тепло.">
