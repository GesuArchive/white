  ...
  return (
    <>
      {sortedAlarms.map((category) => (
        <Section key={category.name} title={category.name + ' Тревоги'}>
          <ul>
            {category.alerts.length === 0 && (
              <li className="color-good">Системы в норме</li>
            )}
                ...
                {!!cameraView && (
                  <Stack.Item>
                    <Button
                      ...
                      content={
                        alert.cameras === 1
                          ? alert.cameras + ' Камера'
                          : alert.cameras > 1
                            ? alert.cameras + ' Камеры'
                            : 'Нет камер'
                      }
                      onClick={() =>
                        act('select_camera', {
