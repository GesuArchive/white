      ...
      {has_beaker ? (
        !beaker_empty ? (
          has_blood ? (
            ...
            <Box color="bad">Не обнаружено крови</Box>
          )
        ) : (
          <Box color="bad">Пробирка пуста</Box>
        )
      ) : (
        <NoticeBox>Нет пробирки</NoticeBox>
      )}
    </Section>
  );
};
  ...
  return viruses.map((virus) => {
    ...
    return (
      <Section
        ...
        {!!virus.is_adv && (
          <>
            <Section title="Статистика" level={2}>
              <Grid>
                <Grid.Column>
                  <LabeledList>
            ...
            <Section title="Симптомы" level={2}>
  ...
  return (
    <Section
      ...
      <Grid>
        ...
        <Grid.Column>
          <LabeledList>
            <LabeledList.Item label="Уровень">{level}</LabeledList.Item>
            <LabeledList.Item label="Сопротивление">
              {resistance}
            </LabeledList.Item>
            <LabeledList.Item label="Скрытность">{stealth}</LabeledList.Item>
            <LabeledList.Item label="Скорость">{stage_speed}</LabeledList.Item>
            <LabeledList.Item label="Передача">{transmission}</LabeledList.Item>
          </LabeledList>
        </Grid.Column>
      </Grid>
      {thresholds.length > 0 && (
        <Section title="Задержки" level={3}>
          <LabeledList>
