          ...
          <Box overflowY="hidden" overflowX="hidden" height="100%">
            ...
            <Section title="Управление полётом">
              ...
              {recall_docking_port_id !== '' ? (
                ...
                <>
                  <NoticeBox>Пристыкованы, ожидаем запуск.</NoticeBox>
                  <Button
                    content="ЗАПУСК"
                    textAlign="center"
                    fontSize="30px"
  ...
  return (
    <Fragment>
      <NoticeBox position="absolute" color="red">
        ...
        <Box ml={1}>Местные шаттлы отмечены на карте.</Box>
      </NoticeBox>
      <Button
        position="absolute"
        icon="search-plus"
...
export const ShuttleControls = (props, context) => {
  ...
  return (
    <>
      ...
      />
      <Box mt={1}>Ускорение будет синхронизированно с ускорением объекта.</Box>
      <ShuttleMap />
      <NoticeBox color="purple" mt={2}>
        Нажми на основной дисплей чтобы лететь.
      </NoticeBox>
      <Box bold>Скорость</Box>
      <Box>Ускорение: {shuttleThrust}</Box>
      <Box bold mt={2}>
        Угол ускорения
      </Box>
      <Box>Угол: {shuttleAngle}</Box>
      {!display_fuel || (
        <>
          <Box bold mt={2}>
            Топлива осталось
          </Box>
          <ProgressBar value={fuel}>{fuel} моль.</ProgressBar>
        </>
      )}
      <Table mt={2}>
      ...
      />
      {!(canDock && !isDocking) || (
        <Button mt={2} content="Начать стыковку" onClick={() => act('dock')} />
      )}
      <Button
        mt={2}
