  ...
  return (
    <Window title="Слежка" width={350} height={700}>
      <Window.Content scrollable>
        ...
        <Section title={`Живые - (${alive.length})`}>
          {alive
            .filter(searchFor(searchText))
            .sort(compareNumberedText)
            .map((thing) => (
        ...
        <Section title={`Призраки - (${ghosts.length})`}>
          {ghosts
            .filter(searchFor(searchText))
            .sort(compareNumberedText)
            .map((thing) => (
        ...
        <BasicSection title="NPC" source={npcs} searchText={searchText} />

        <BasicSection title="Разное" source={misc} searchText={searchText} />
      </Window.Content>
    </Window>
  );
};
