...
/proc/SliceNDice(dmifile as file)
	...
	switch(states.len)
		if(0 to (STATE_COUNT_NORMAL - 1))
			var/cont = tgui_alert(usr, "Too few states: [states.len],  expected [STATE_COUNT_NORMAL] (Non-Diagonal) or [STATE_COUNT_DIAGONAL] (Diagonal), Continue?", "Unexpected Amount of States", "Yes", "No")
			if(cont == "No")
				return
		...
		if((STATE_COUNT_DIAGONAL + 1) to A_BIG_NUMBER)
			var/cont = tgui_alert(usr, "Too many states: [states.len],  expected [STATE_COUNT_NORMAL] (Non-Diagonal) or [STATE_COUNT_DIAGONAL] (Diagonal), Continue?", "Unexpected Amount of States", "Yes", "No")
			if(cont == "No")
				return
